...
def _write_fake_venv_python(path: Path, log_path: Path) -> None:
    _make_executable(
        ...
        "  phase_name=\"${3:-}\"\n"
        "  printf 'gza-verify phase=start name=%s\\n' \"$phase_name\"\n"
        "  shift 3\n"
        "  if [[ \"${1:-}\" != \"--\" ]]; then\n"
        "    printf 'verify_phase: invalid forwarded arguments\\n' >&2\n"
        "    exit 2\n"
        "  fi\n"
        "  shift\n"
        "  set +e\n"
        ...
        "  set -e\n"
        "  if [[ \"$status\" -eq 0 ]]; then\n"
        "    printf 'gza-verify phase=passed name=%s duration_seconds=0.000001\\n' \"$phase_name\"\n"
        "    exit 0\n"
        "  fi\n"
        "  printf 'gza-verify phase=failed name=%s duration_seconds=0.000001\\n' \"$phase_name\"\n"
        "  exit \"$status\"\n"
        "fi\n",
    )
...
def test_full_verify_clamps_xdist_worker_count_to_a_single_core_machine(tmp_path: Path) -> None:
    ...
    fake_bin = fixture_root / "fake-bin"
    fake_bin.mkdir()
    _write_fake_getconf(fake_bin / "getconf", cpu_count=1)
...
def test_full_verify_uses_project_venv_for_test_latency_when_available(tmp_path: Path) -> None:
    ...
    tool_invocations = tool_log.read_text(encoding="utf-8")
    assert "python -m gza.tools.verify_phase unit -- ./bin/test-unit --summary -- tests/ -n 7 --dist load --durations=25 -o faulthandler_timeout=60" in tool_invocations
    assert "test-unit --summary -- tests/ -n 7 --dist load --durations=25 -o faulthandler_timeout=60" in tool_invocations
    assert (
...
def test_full_verify_falls_back_to_uv_for_test_latency_without_project_venv(tmp_path: Path) -> None:
    ...
    assert "uv run python -m gza.tools.verify_phase unit -- ./bin/test-unit --summary -- tests/ -n 7 --dist load --durations=25 -o faulthandler_timeout=60" in uv_invocations
    assert (
        "uv run python -m gza.tools.verify_phase functional -- uv run python -m gza.test_functional_rerun --summary -- tests_functional/ -n 7 --dist loadscope --durations=25 -o faulthandler_timeout=60"
        in uv_invocations
    )
...
def test_full_verify_only_runs_integration_suite_with_integration_flag(tmp_path: Path) -> None:
    ...
    assert (
        "python -m gza.test_functional_rerun --summary -- tests_functional/ -n 3 --dist loadscope --durations=25 -o faulthandler_timeout=60"
...
@pytest.mark.timeout(30, method="signal")
def test_quick_verify_runs_ruff_phase_against_src_gza_with_project_venv(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    fixture_root = _setup_verify_script_fixture(tmp_path)
    tool_log = fixture_root / "venv-tools.log"
    ...
    venv_bin.mkdir(parents=True)
    _write_fake_venv_python(venv_bin / "python", tool_log)
    _write_fake_passthrough_tool(venv_bin / "ruff", tool_log, "ruff")
