...
def test_full_verify_defaults_to_ci_parity_xdist_worker_count_on_high_core_machine(tmp_path: Path) -> None:
    ...
    _write_fake_passthrough_tool(fixture_root / "bin" / "test-functional", tool_log, "test-functional")
    ...
    assert "test-functional -- tests_functional/ -n 2 --dist loadscope --durations=25 -o faulthandler_timeout=60" in tool_invocations
...
def test_full_verify_uses_project_venv_for_test_latency_when_available(tmp_path: Path) -> None:
    ...
    assert (
        "python -m gza.tools.verify_phase functional -- ./bin/test-functional -- tests_functional/ -n 7 --dist loadscope --durations=25 -o faulthandler_timeout=60"
        in tool_invocations
    )
    assert "test-functional -- tests_functional/ -n 7 --dist loadscope --durations=25 -o faulthandler_timeout=60" in tool_invocations
...
def test_full_verify_falls_back_to_uv_for_test_latency_without_project_venv(tmp_path: Path) -> None:
    ...
    assert (
        "uv run python -m gza.tools.verify_phase functional -- ./bin/test-functional -- "
        "tests_functional/ -n 7 --dist loadscope --durations=25 -o faulthandler_timeout=60"
    ) in uv_invocations
...
def test_full_verify_only_runs_integration_suite_with_integration_flag(tmp_path: Path) -> None:
    ...
    assert "test-functional -- tests_functional/ -n 3 --dist loadscope --durations=25 -o faulthandler_timeout=60" in default_invocations
