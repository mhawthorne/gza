...
@pytest.mark.timeout(60, method="signal")
def test_parallel_only_functional_failure_passes_via_serial_rerun_and_preserves_phase_line(tmp_path: Path) -> None:
    suite_dir = tmp_path / "parallel_only_functional"
    suite_dir.mkdir()
    (suite_dir / "test_parallel_only_functional.py").write_text(
        "import os\n"
        "import time\n"
        "import pytest\n\n"
        "@pytest.mark.timeout(1, method='signal')\n"
        "def test_parallel_only_functional_probe():\n"
        "    if int(os.environ.get('PYTEST_XDIST_WORKER_COUNT', '0')) > 1:\n"
        "        time.sleep(2)\n",
        encoding="utf-8",
    )
