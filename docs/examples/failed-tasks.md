...
Fully recovered failed ancestors are omitted from this report entirely. Once a retry/resume descendant completes, normal `uv run gza advance` and `uv run gza watch` treat that completed descendant as the actionable node for merge/review/rebase decisions instead of repeating a permanent recovery skip on the failed ancestor.
