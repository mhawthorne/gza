...
|--------|------|---------|-------------|
| `project_id` | String | *(local DB: `default`; shared DB: generated by `gza init` for new projects, otherwise derived from project path/name when omitted)* | Project identity used to scope rows in shared DB mode. In shared mode, `project_id: default` is invalid. `gza init` persists a stable `project_id` for new projects; older configs that omit it derive an identity until persisted (for example via `gza migrate --import-local-db`). |
...
| `db_path` | String | `.gza/gza.db` | Task DB path. Shared DB mode is opt-in via an explicit path (for example `~/.gza/gza.db`). If a legacy local `.gza/gza.db` exists when using shared mode, import it explicitly with `gza migrate --import-local-db`. `GZA_DB_PATH` overrides this value at runtime. |
...
| `recommend_rebase_behind_commits` | Integer | `1` | Advisory stale-branch threshold; `0` disables behind-target `recommend_rebase` detection |
...
When shared DB mode is active (explicit `db_path`) and a legacy local `.gza/gza.db` is detected, task commands stop with an explicit message until you run `gza migrate --import-local-db`.
