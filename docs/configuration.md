...
`gza land` is the orchestration surface for task-backed rebase, source verify, review, guarded judgment, ordinary follow-up task materialization, deferred blockers, merge, and post-merge target verification. It proves dependency readiness and project scope before any rebase/provider work, re-proves them after every source-head change, and refuses with one typed `LandBlocked` reason when a required phase cannot be run or exactly reused. If initial resolution proves `merged`, `empty`, or `redundant`, the command returns an idempotent terminal result without refreshing verification, source verify, review, judgment, materialization, mark-merged, merge-state mutation, or merge side effects. Writable reconciliation may persist exactly one `unmerged` to `empty`/`redundant` transition; after that terminal no-work write, the same downstream no-activity rule applies. Dry-run reports known terminal states query-only and never performs that reconciliation mutation. `empty` and `redundant` are reported as terminal no-work, not as landed. Successful landing paths create or reuse ordinary review `FOLLOWUP` tasks before merge or mark-merged success. Guarded `CHANGES_REQUESTED` escalation creates or reuses both deferred `BLOCKER` tasks and ordinary `FOLLOWUP` tasks as urgent PR-required work before any merge-state mutation; exact-key reuse must validate or reconcile those properties. The current parser surface is present for the implementation slice; see `specs/behavior/lifecycle-engine.md` §8a for the normative gate matrix and refusal precedence.
