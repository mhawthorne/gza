...
| `on_max_cycles` | String | `merge_and_defer` | Policy for implementation review/improve loops that reach `max_review_cycles`: `merge_and_defer` merges current green capped reviews and creates mandatory blocker follow-ups; `park` is the rollback switch for manual attention |
