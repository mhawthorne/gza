...
| `--force` | Override parked needs-attention lifecycle gates for the local merge path; does not bypass git conflicts, rebase/verify/improve work, red verify gates, or open review `BLOCKER`s by itself, and cannot be combined with `--rebase --resolve` |
