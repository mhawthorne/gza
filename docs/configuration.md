...
| `on_max_cycles` | String | `park` | Policy for implementation review/improve loops that reach `max_review_cycles`: `park` stops for manual attention; `merge_and_defer` is accepted for the staged opt-in merge-and-defer rollout |
...
| `--watch-config PATH` | Load a version 1 watch supervisor manifest. One path-backed selected project can execute today; more than one selected project acquires/rolls back watch leases in selector order and is then refused before dispatch |
| `--watch-project NAME=PATH_OR_PROJECT_ID` | Replace any manifest project list with an ordered CLI project list. Path selectors can execute when exactly one project is selected; multi-project selections acquire/roll back watch leases before the current dispatch-disabled refusal |
...
The current execution surface is intentionally narrower than the version 1 schema: a single path-backed selected project can run through the existing watch runtime. A manifest or CLI selection with more than one project resolves the selected runtime set, acquires watch-supervisor leases for every currently healthy project in selector order, rolls back on any healthy-project lease conflict, releases acquired leases on the feature-disabled refusal, and exits with `multi-project watch supervisor dispatch is not enabled yet`. Invalid projects are reported with their selector, typed reason, and available project/root identity, then remain disabled and unleased until a later validation path can acquire their lease first. A selector that only names a registry `project_id` exits before runtime construction with a registry-ID limitation message; use `NAME=PATH` for the currently executable form.
