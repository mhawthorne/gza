...
| `db_path` | String | `.gza/gza.db` | Task DB path. Shared DB mode is opt-in via an explicit path (for example `~/.gza/gza.db`). If a legacy local `.gza/gza.db` exists when using shared mode, import it explicitly with `gza migrate --import-local-db`. |
