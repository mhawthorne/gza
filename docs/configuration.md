...
The drafted extract prompt leads with the best available description of the work itself. Task-based extraction prefers specific source-task prompt content when available, branch-based extraction falls back to selected diff/file context, and commit-based extraction uses commit subjects when they provide a clearer summary. Source task IDs, branch/base refs, and commit SHAs remain in the prompt as secondary provenance context.
