...
- `Warning: Could not repair changed-diff rebase provenance before lifecycle rendering because the task database is read-only; lifecycle may reflect stale resolution metadata.` when `show` detects changed-diff provenance repair candidates but cannot run the writable repair path against a read-only task database.
- `Warning: Could not complete changed-diff rebase provenance repair before lifecycle rendering: ...` when `show` cannot finish the pre-render writable provenance repair and the lifecycle block may therefore reflect stale or partially repaired resolution metadata.
