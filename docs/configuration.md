...
`gza land` is the orchestration surface for task-backed rebase, source verify, review, guarded judgment, ordinary follow-up task materialization, deferred blockers, merge, and post-merge target verification. It proves dependency readiness and project scope before any rebase/provider work, re-proves them after every source-head change, and refuses with one typed `LandBlocked` reason when a required phase cannot be run or exactly reused. An already `merged` unit returns an idempotent terminal result without refreshing verification, source verify, review, judgment, materialization, mark-merged, merge-state mutation, or merge side effects. Stored `empty` and `redundant` units with no recorded head do the same; stored no-work units with a recorded head are first revalidated against that recorded-head patch and the unit target. If the patch is represented, or the proof is unavailable and therefore fail-closed, `land` reports the stored terminal no-work result with no downstream activity. If the patch is positively missing, writable `land` restores the unit to `unmerged`; dry-run reports the repair-needed nonterminal result without mutating. Writable terminal reconciliation may persist exactly one proof-bound `unmerged` to `merged`/`empty`/`redundant` transition, but it re-reads and re-proves immediately before the atomic identity-conditioned update and returns any fresh authoritative terminal state observed after the write. Dry-run reports known terminal states query-only and never performs that reconciliation mutation. `empty` and `redundant` are reported as terminal no-work, not as landed. Successful landing paths create or reuse ordinary review `FOLLOWUP` tasks before merge or mark-merged success. Guarded `CHANGES_REQUESTED` escalation creates or reuses both deferred `BLOCKER` tasks and ordinary `FOLLOWUP` tasks as urgent PR-required work before any merge-state mutation; exact-key reuse must validate or reconcile those properties. The current parser surface is present for the implementation slice; see `specs/behavior/lifecycle-engine.md` §8a for the normative gate matrix and refusal precedence.
