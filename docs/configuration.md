...
| `review_diff_char_limit` | Integer | `400000` | Maximum characters of diff/excerpt content inlined in a review prompt before falling back to targeted excerpts or truncating excerpt output |
...
Bare `gza rebase <task_id>` creates a pending `rebase` child task. Use `--run` to execute that task-backed rebase in the foreground or `--background` to detach a worker. Use this command for a standalone rebase only; use `gza land <task_id>` when the goal is to drive the merge unit through rebase, review, judgment if needed, merge, and post-merge verification.
