...
When a completed changed-diff rebase row has lost or partially lost its persisted provenance block, a writable maintenance/lifecycle path first re-derives that provenance from local reflogs plus the surviving branch/target refs and writes the repaired `review_scope` back to the rebase row. When a required post-rebase refresh review already exists but its persisted resolution metadata is blank, ordinary, or otherwise inconsistent with that authoritative rebase context, writable lifecycle then re-derives the canonical resolved head/target SHAs, rewrites the review row with structured resolution metadata, and continues through the normal review/merge path. Query-only surfaces such as `gza show`, incomplete-lineage rendering, dispatch preview, and watch scope-gap analysis may apply those same proven repairs in memory, but they must not write; if reconstruction is unavailable in memory, they fail closed and leave durable repair for the next write-capable lifecycle path. Rows that still claim malformed rebase or resolution metadata after those deterministic repair attempts remain fail-closed and park with `resolution-review-metadata-invalid`.
...
| `gza merge` | Advance merges directly only after the current pre-merge verify gate and current review gate are both green; manual `gza merge <id>` uses the shared direct verify prerequisite executor for verify reruns and verify-evidence reconciliation before it reevaluates whether merge is allowed |
