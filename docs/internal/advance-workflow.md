...
| Current verify gate for the implementation owner is red before any review can be created/refreshed, and no same-epoch `verify_fix` exists yet | `create_verify_fix` — create one same-branch verify-fix lane keyed by the reviewed branch, reviewed head SHA, and normalized verify command; timeout settings are recorded as run provenance, not freshness identity |
...
| Current verify gate for the implementation owner is still red after one completed same-epoch `verify_fix` | `needs_discussion` — reason=`verify-fix-failed`; stop after one completed verify-fix attempt for that epoch; deterministic test failures and unknown structured failure origins remain blocking |
