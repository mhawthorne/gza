...
| Current verify gate for the implementation owner is red before any review can be created/refreshed, and the recorded verify base is a proven strict ancestor of the current target head before any not-yet-running `verify_fix` worker path | `needs_rebase` — rebase onto the target before spawning, running, or retrying `verify_fix`; leave the old same-epoch lane unlaunched, require a fresh verify gate on the rebased head, and fall through to the normal `verify_fix` path only when durable provenance from a completed same-branch rebase affirmatively produced the failed verify branch/head and records a resolved target SHA equal to the failed verify result's base/default SHA, with no later code-changing descendant before verify capture |
| Current verify gate for the implementation owner is red before any review can be created/refreshed, and target/base freshness cannot prove exact current equality or strict stale ancestry | `needs_discussion` — reason=`verify-base-freshness-unverified`; fail closed with the diagnostic instead of creating, running, or retrying `verify_fix` |
| Current verify gate for the implementation owner is red before any review can be created/refreshed, no same-epoch `verify_fix` exists yet, and the recorded verify base is proven current | `create_verify_fix` — create one same-branch verify-fix lane keyed by the reviewed branch and reviewed head SHA; the normalized verify command and timeout settings are recorded as run provenance, not freshness identity |
