...
| Consecutive completed no-op improves for the latest `(impl, review)` pair >= `max_noop_improve_cycles`, lineage is not tagged `allow-noop-improve` | `needs_discussion` — park with `improve-no-op` unless runner-owned current passing verify evidence already cleared the review before lifecycle evaluation |
