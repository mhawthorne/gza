...
1. **Unmerged tasks**: `store.get_unmerged()` provides the initial completed `merge_status='unmerged'` compatibility set, then `_collect_advance_completed_tasks()` narrows completed merge-unit-backed work to the active advance target branch. In the no-explicit-task path, `cmd_advance()` passes `git.current_branch()` as that target, so completed work only participates when its merge unit targets the currently checked-out branch. Legacy rows with no resolvable merge unit remain compatibility-oriented fallback candidates instead of being branch-target-filtered away. Improve and rebase tasks that have a parent (`based_on IS NOT NULL`) are still excluded because they operate on the parent's branch.
