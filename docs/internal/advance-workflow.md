...
When runner context can parse the latest review structure, the atomic improve input is the
union of parsed review blockers and unresolved `feedback` comments. A parseable review with
zero blockers still renders those comments as the `## Atomic Blocker Set`; the structured
parse warning path is reserved for genuinely unparseable or malformed blocker prose.
