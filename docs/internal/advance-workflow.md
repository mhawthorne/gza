...
| Verdict = `CHANGES_REQUESTED` AND durable-progress-epoch cycles >= `max_review_cycles` with no stale-review refresh path | `max_cycles_reached` — manual intervention; with `on_max_cycles=merge_and_defer`, eligible ordinary current-head candidates first traverse the existing pre-merge verify gate, so missing/stale verify runs and red/unavailable verify stays on the verify-fix or attention path |
