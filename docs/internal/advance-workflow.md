...
| Branch cannot merge into the resolved target branch AND rebase child is `failed`, and shared recovery classification says that failure is manual/conflict | `needs_discussion` — manual intervention required unless later local post-resolution proof exists |
...
| Current verify gate for the implementation owner is red before any review can be created/refreshed, and no same-epoch `verify_fix` exists yet | `create_verify_fix` — create one same-branch verify-fix lane keyed by the exact verify epoch |
...
| Current verify gate for the implementation owner is still red after one completed same-epoch `verify_fix` | `needs_discussion` — reason=`verify-fix-failed`; stop after one completed verify-fix attempt for that epoch |
...
When a completed changed-diff rebase row has lost or partially lost its persisted provenance block, a writable maintenance/lifecycle path first re-derives that provenance from local reflogs plus the surviving branch/target refs and writes the repaired `review_scope` back to the rebase row before query-only lifecycle rendering depends on it. When a required post-rebase refresh review already exists but its persisted resolution metadata is blank, ordinary, or otherwise inconsistent with that authoritative rebase context, lifecycle then re-derives the canonical resolved head/target SHAs, rewrites the review row with structured resolution metadata, and continues through the normal review/merge path. Rows that still claim malformed rebase or resolution metadata after those deterministic repair attempts remain fail-closed and park with `resolution-review-metadata-invalid`.
...
| `verify_gate` | Runs the lifecycle-owned verify gate for the current implementation owner/head before review creation or merge. In `watch`/`iterate`, this still routes through the shared advance executor rather than bypassing the gate with direct review/merge side effects. |
...
| `gza merge` | Advance merges directly only after the current pre-merge verify gate and current review gate are both green; manual `gza merge <id>` remains the narrower operator path described above |
