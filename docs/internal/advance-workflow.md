...
`gza land <task-id>` is the explicit operator orchestration path for one selected merge
unit. It is separate from `advance`/`watch`: it can run a task-backed rebase, obtain the
one required post-rebase review, run source verification, request a guarded landing
judgment when policy permits, materialize audit tasks, merge, and refresh the post-merge
target checkpoint in one synchronous attempt. `land --policy strict` preserves ordinary
lifecycle gates; default `land --policy guarded` can defer only exact judged
plain-full/resolution review blockers and eligible review-churn parks. It cannot defer
dependency/scope failures, verify failures, spec-coherence blockers, unsafe
conflict-resolution defects, missing source/target proof, failed recovery/rebase work, or
`NEEDS_DISCUSSION` review states.
...
1. **Lineage owner rows**: `src/gza/lineage_query.py::query_lineage_owner_rows(...)` builds one in-memory lineage index, groups tasks by branch/merge ownership, evaluates one lineage-resolved predicate, and returns one canonical row per unresolved owner. The same owner-row query now feeds `gza incomplete`, `gza advance`, and the `gza watch --restart-failed` recovery queue. Broad-index tag-scoped query shapes still materialize the full `store.get_all()` snapshot for lineage/recovery context, while eligible recovery-unit-scoped query shapes use the narrower tag-aware recovery-unit loader before falling back to the broad loader for legacy/no-unit stores. After whichever index is loaded, ordinary out-of-scope owners are pruned before branch priming and lifecycle/recovery action resolution; terminal owners keep a conservative failed-leaf reroot exception so a tag-matching failed leaf with distinct unresolved work can still become the emitted row owner. Explicit `gza advance <task-id>` queries still build the shared snapshot, but failed-task recovery classification is scoped to the selected task's canonical owner/active merge-unit lineage so unrelated failed leaves are not enumerated before the selected row is evaluated. When the selected task is a skipped same-branch member that resolves the visible row through its implementation root, the recovery scope maps back to that root owner and includes the root owner's ordinary and skipped members. Ordinary `depends_on` parents or dependents are not pulled into that recovery scope unless existing ownership metadata has already attached them to the same owner lineage. Same-branch orphan rebase descendants stay available for the explicit `no-descendant-on-the-impl-branch` attention signal, but they are excluded before lifecycle action selection so they cannot be planned for merge/rebase/review work.
...
|-------|---------|-------------|
| `require_review_before_merge` | `true` | Implement tasks must have a valid current review before merge, except for the narrow `system-main-verify` remediation lane after current green verify, mandatory spec-coherence, red-freeze authorization, and ordinary non-review merge safety gates pass |
...
| Current non-budget red verify gate for the implementation owner is red before any review can be created/refreshed | Before creating or reusing any same-epoch `verify_fix`, first guard same-branch rebase mutation: a pending or in-progress rebase skips with `reason=verify-gate-preflight-rebase`; unavailable target-tip or live-head proof parks with `reason=verify-gate-proof-unavailable`; target/base freshness that cannot prove exact current equality or strict stale ancestry parks with `reason=verify-base-freshness-unverified`; a branch proven to lack the target tip selects `needs_rebase` with `reason=verify-gate-preflight-rebase`; a completed preflight rebase whose durable provenance matches the implementation owner, red verify epoch, and current target identity parks instead of looping even when a later same-epoch red artifact is newest |
...
| Red verify-gate rebase preflight is satisfied and no same-epoch `verify_fix` exists yet | `create_verify_fix` — create one same-branch verify-fix lane keyed by the reviewed branch and reviewed head SHA; the normalized verify command and timeout settings are recorded as run provenance, not freshness identity |
| Red verify-gate rebase preflight is satisfied and same-epoch `verify_fix` is `pending` / `in_progress` | `run_verify_fix` / `wait_verify_fix` — reuse the existing lane instead of creating another |
...
| Completed `system-main-verify` remediation implementation | Bypass ordinary code-review-derived routing and, after current green verify plus any mandatory spec-coherence review and ordinary non-review merge safety gates, `merge` |
...
| Completed `system-main-verify` remediation, even when `require_review_before_merge=true` and ordinary review state already exists | If the current pre-merge verify gate is green and any mandatory spec-coherence review, red-freeze exemption authorization, local merge source proof, scope/dependency/execution-status checks, and conflict/rebase safety gates pass, `merge`; otherwise stay on the relevant non-review safety gate |
...
| `gza merge` | Advance normally merges directly only after the current pre-merge verify gate and current review gate are both green. Narrow exceptions are `system-main-verify` remediation tasks, which self-enforce review scope in their prompt and merge directly only after current green verify evidence, mandatory spec-coherence, red-freeze authorization, and ordinary non-review merge safety gates, and `on_max_cycles=merge_and_defer`, where an ordinary capped `CHANGES_REQUESTED` review can emit an annotated merge only after current-head proof, live merge-source proof, readable deterministic blocker content, fresh green verify evidence, and mandatory deferred-blocker payload validation. Manual `gza merge <id>` uses the shared direct verify prerequisite executor for verify reruns and verify-evidence reconciliation before it reevaluates whether merge is allowed |
| `gza land` | Not used by advance. Operators use `gza land <task-id>` for full rebase/review/judgment/merge orchestration of one canonical merge unit; use `gza rebase <task-id> --run` only for a standalone rebase |
...
When watch checks the local target integration verify gate, it emits concise `INFO` progress lines before the checkpoint check, before each bounded merge-halting rerun attempt, and after completion. The pre-check line says the suite will run if the checkpoint is stale or if a red result needs bounded confirmation, so a cached checkpoint is not described as an active long-running verify. Rerun progress labels the preceding evidence truthfully: `red` only for an actual failed gate verdict, `unavailable` for unavailable evidence, and `unknown` for malformed evidence. The completion line reports `green` only for a passed configured gate, `red` for a merge-halting failure with all structured failed phases when known, `cached <status>` when no verify ran, and non-green statuses such as `disabled`, `unavailable`, `launch-failed`, or `unknown` for no-gate, environment, or malformed evidence failures. Red-main remediation rows use one canonical plural phase identity such as `phases:ruff,unit`; prompt evidence is partitioned per failed phase, and green cleanup retires a scoped remediation only after every covered phase has affirmative pass evidence or the full gate passes. When a remediation owner is deliberately dropped as redundant or moot, watch routes the transition through the shared scope-aware drop operation so same-unit `verify_fix` completion children are dropped with it, while ambiguous or distinct-unit descendants remain visible. Raw suite stdout is suppressed for the main-verify subprocess when watch is in `--quiet` or `--yes` mode, but the structured watch progress lines remain visible unless `--quiet` suppresses all watch stdout.
...
At the start of each watch pass, watch also fingerprints the installed `gza` Python package on disk. If the package contents drift from what the process started with, watch emits one loud `WARNING` line for that newly observed fingerprint. In the default mode, the warning explicitly says watch will re-exec itself at the next safe phase checkpoint without waiting for running or pending work to drain; detached workers stay alive and the replacement process reconciles them after it auto-resumes, skipping the first-pass confirmation gate because the session was already approved. A safe checkpoint is reached only after the current phase or dispatch unit has completed and its bookkeeping is settled, so the old process does not start later reconciliation, lifecycle, recovery, or pending-dispatch work once restart is pending. `--no-auto-restart-on-drift` switches back to the warn-only manual-restart message.
...
Default `gza watch` uses the same bounded shared recovery policy as the explicit failed-task recovery queue, but it exposes that policy through recovery and pending lanes plus a narrow main-verify remediation emergency lane. `watch.recovery_slots` (default `1`) reserves that many worker slots per watch pass for worker-consuming failed-task recovery before pending pickup, and leaves the remaining `batch - recovery_slots` worker slots for pending work. The rule is uniform for worker-consuming recovery: batch-1 plain watch gives the single slot to worker-consuming recovery first; `--pending-only` or `watch.recovery_slots: 0` are the escape hatch for operators who intentionally want single-slot pending-only behavior. `--recovery-only` is the other extreme (`recovery_slots = batch` in single-project watch, or every free supervisor dispatch slot in multi-project watch) and suppresses ordinary pending pickup unconditionally. The only pending carve-out is the exact active watch-owned `system-main-verify` remediation selected through the emergency lane; queue position is ordering metadata, not authorization.
