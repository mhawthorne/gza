...
- If rebuild fails, watch skips later merge actions in that watch pass rather than falling back to `config.project_dir`.
...
- **implement**: Creates a new worktree with a new branch based on `_select_worktree_base_ref()`.
  ...
  If another concurrent task creates the selected branch after slug probing but before
  `git worktree add -b` claims it, startup treats the failure as a bounded branch-creation
  collision: it rechecks live branch state, regenerates a suffixed slug/branch pair, persists
  it, and retries setup up to a fixed small limit before failing closed.
...
- Live cleanup is limited to configured Gza-managed roots (`config.worktree_path` plus
  `interactive_worktree_dir` when set). If the branch is attached in a foreign worktree,
  checkout/review/improve/rebase fail closed and instruct the operator to run
