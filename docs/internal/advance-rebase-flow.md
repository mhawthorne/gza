...
3. **Decide action based on rebase child status**:
   ...
   - once a merge candidate is selected, no rebase child (or only a stale completed rebase) and the branch does not already contain the target tip → create a new rebase task (`needs_rebase` action)
...
If a rebase already published successfully, or the runner can still publish the branch after a pre-PR `gh` lookup/availability failure, the task now completes and surfaces a non-fatal publication note; the missing PR no longer blocks lifecycle progress. Only branch-publish rejection during that publication step records a runner failure, now `BRANCH_UNPUSHABLE`, for later reconcile-and-republish recovery. Legacy `PR_REQUIRED` rows are lazily reclassified to `BRANCH_UNPUSHABLE` the first time recovery planning touches them; branchless legacy/corrupted rows still fail closed into needs-attention because there is no branch to reconcile. The old explicit `work --pr` retry remains only as a targeted PR-only completion retry for rows that still carry a branch.
