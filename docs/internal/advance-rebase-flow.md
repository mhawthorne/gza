...
2. **Plan the next lifecycle action**: advance chooses the next review / improve / verify-fix / verify-gate / merge action for the merge unit, then runs one shared rebase-before-dispatch check before returning that action.
3. **Decide action based on rebase child status and target freshness**:
   - ordinary queue-wide planning runs the same pre-dispatch rebase check as selected merge planning, but only for rows with an action about to dispatch; idle and needs-attention rows are left alone
   ...
   - `failed` 3 times on the same branch with no intervening successful rebase, review, review-clear proof, or completed code change → `needs_discussion` with reason `rebase-failure-circuit-breaker` (stop creating fresh rebase tasks and surface manual intervention)
   - behind-count probe failure, `None`, or malformed output while checking a cleanly mergeable branch → `needs_discussion` with reason `pre-dispatch-target-freshness-unverified` (fail closed instead of dispatching against an unproven-current tip)
   ...
   - no active rebase child, the branch does not already contain the target tip, a local merge source is resolvable, and the next action would dispatch against the branch while the source cannot merge or is positively known to be behind target → create a new rebase task (`needs_rebase` action, reason `pre-dispatch-rebase`)
   - before creating, running, or retrying a not-yet-running `verify_fix` for a red verify gate, if the failed verify evidence's recorded base/default SHA is proven to be a strict ancestor of the current local target head and the same failed verify result was not already captured after a completed same-branch rebase whose durable provenance produced that failed verify branch/head and records the failed verify base/default SHA as its resolved target → create a new rebase task (`needs_rebase` action, reason `verify-fix-stale-base-rebase`), leave the old same-epoch verify-fix lane unlaunched, then rerun the verify gate on the rebased head before considering `verify_fix`; if target/base freshness cannot prove exact current equality or strict stale ancestry, park with `needs_discussion` / `verify-base-freshness-unverified` instead of spawning verify-fix work
   - if the merge unit no longer has any resolvable local merge source, park it with `needs_discussion` / `merge-source-needs-manual-resolution` instead of creating or continuing rebase automation from a remote-only/deleted source ref
   ...
   - mergeable branches continue through the normal review/merge rules only when the branch is not positively known to be behind target
...
- `branch=<implementation branch>` at insertion time, so the singleton guard is keyed by
  the persisted source branch being rebased rather than by the parent row
- `skip_learnings=True`
...
- Resolve the branch via `_resolve_code_task_branch_name`, using the same canonical
  rebase source-branch identity as guarded insertion. Active branchful direct rebases,
  trigger-sourced direct rebases, and active branchful retry/resume recovery rebases
  execute on their persisted `branch`. Only legacy branchless rows, plus terminal
  historical branchful orphan recovery descendants, fall back to walking the
  `based_on` lineage.
...
- Before recording successful completion, the host runner imports the private checkout tip
  ...
  tip, treats a stale expected-old-SHA import as a superseded success only when the live
  branch already contains the exact target captured for that attempt, rejects anomalous
  non-advancing or stale-import rebases without that containment proof, persists
  superseded output as an authoritative no-op rather than the losing isolated summary,
  fails closed on local/remote ref lookup uncertainty, and force-pushes the rebased branch
...
6. On success, the host imports the rewritten private-checkout tip back into the canonical
   ...
   success when the branch already contains the target tip, treats a stale expected-old-SHA
   import as a superseded success only when the live branch already contains the exact
   target captured for that attempt, rejects anomalous non-advancing or stale-import
   rebases without that containment proof, persists superseded output as an authoritative
...
Recovery duplicate messages for rebase singletons name the active child's
persisted branch first, falling back to the attempted row's branch only for
legacy active rows that were created before canonical branch persistence. This
keeps manual retry/resume, advance, iterate, and watch output aligned with the
branch key that actually matched the singleton guard.
...
That refresh review is resolution-scoped only when lifecycle has complete provenance for the narrow optimization. The completed rebase persists pre/post rebase provenance, and lifecycle treats that block as authoritative even if a later stale whole-row task update re-saves inherited review text. If older rows lose that block anyway, writable maintenance/lifecycle paths re-derive it from local reflogs and surviving refs, then persist the rebase repair before validating or repairing the dependent resolution review from that state. Complete recovered provenance is usable for the narrow path; `recovered=True` records how the baseline was obtained and does not invalidate otherwise complete provenance. Plain reviews remain plain and are not rewritten into resolution reviews just because they followed a changed-diff rebase.

When rebase or resolution-review metadata cannot be validated or deterministically repaired, lifecycle falls back to a plain full review at the proven current live implementation head instead of parking permanently on `resolution-review-metadata-invalid`. A completed full review clears the stale rebase only when its stored reviewed head SHA equals that live branch head; a stale full-review approval is rejected and rerun through the normal pre-review verify / review path. Pending full-review rows are reused only when their stored reviewed head already matches the live head, and `run_review` verifies that selected head before launching. Foreground iterate and background iterate startup both create selected `create_review` actions through the shared resolution-aware helper, so the watch-launched detached child preserves the same scope as explicit background preflight. If the live branch head itself cannot be established, lifecycle fails closed with `review-freshness-unverified`; cached merge-unit head metadata is not accepted as live proof. Operator descriptions use resolution-metadata wording for malformed or unavailable metadata and reserve branch-head advancement wording for a proven reviewed-head/live-head mismatch. Runner review prompts reconstruct a focused `git range-diff` between the pre-rebase series and the rebased series for the narrow path when possible. If that focused delta cannot be reconstructed from the persisted refs, the prompt fails closed with explicit `resolution delta unavailable` guidance and still suppresses the ordinary whole-implementation diff so reviewers do not accidentally grade the wrong surface.
