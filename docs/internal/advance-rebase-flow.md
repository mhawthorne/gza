...
That refresh review is resolution-scoped, not a whole-task refresh. The completed rebase now persists pre/post rebase provenance, and lifecycle treats that block as authoritative even if a later stale whole-row task update re-saves inherited review text. If older rows lose that block anyway, writable maintenance/lifecycle paths re-derive it from local reflogs and surviving refs, then persist the rebase repair before validating or repairing the dependent resolution review from that state. Foreground iterate and background iterate startup both create selected `create_review` actions through the shared resolution-aware helper, so the watch-launched detached child preserves the same scope as explicit background preflight. Read-only query paths may apply those same proven repairs in memory, but must never write; if in-memory reconstruction is unavailable or ambiguous, lifecycle fails closed with `resolution-review-metadata-invalid` and leaves durable repair for the next write-capable lifecycle path. Runner review prompts then reconstruct a focused `git range-diff` between the pre-rebase series and the rebased series when possible. If that focused delta cannot be reconstructed from the persisted refs, the prompt fails closed with explicit `resolution delta unavailable` guidance and still suppresses the ordinary whole-implementation diff so reviewers do not accidentally grade the wrong surface.
