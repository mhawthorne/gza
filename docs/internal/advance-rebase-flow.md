...
If a rebase already published successfully, or the runner can still publish the branch after a pre-PR `gh` lookup/availability failure, the task now completes and surfaces a non-fatal publication note; the missing PR no longer blocks lifecycle progress. Only branch-publish rejection during that publication step records a runner failure, now `BRANCH_UNPUSHABLE`, for later reconcile-and-republish recovery. Legacy `PR_REQUIRED` rows are compatibility-routed through that same reconcile-and-republish path when they still carry a branch; branchless legacy/corrupted rows now fail closed into needs-attention because there is no branch to reconcile. The old explicit `work --pr` retry remains only as a targeted PR-only completion retry for rows that still carry a branch.
