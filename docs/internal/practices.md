...
the lane's rerun cap fail the phase without masking. Operators can disable the
bridge with `GZA_UNIT_SERIAL_RERUN=0` or `GZA_FUNCTIONAL_SERIAL_RERUN=0` when
debugging suspected parallel-only real failures. Treat the
`unit-rerun:` / `functional-rerun:` `PARALLEL-ONLY FAILURE (passed serially)`
