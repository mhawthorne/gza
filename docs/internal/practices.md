...
the per-phase `GZA_<PHASE>_RERUN_CAP` fail the phase without masking.
Operators can disable the bridge with `GZA_<PHASE>_SERIAL_RERUN=0` when
debugging suspected parallel-only real failures. Treat the
`<phase>-rerun: PARALLEL-ONLY FAILURE (passed serially)` lines as a real
