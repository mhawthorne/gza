...
defaulting its xdist worker count to two. The unit lane uses `--dist load` so
large module-level files do not strand one worker, and the functional lane keeps
`--dist loadscope` for its subprocess-heavy file grouping. Operators can still
pin alternate worker counts for local debugging by setting
...
as interim arbiters for bounded parallel-only flakes. Each bridge keeps the
normal parallel xdist pass, then reruns only the captured failing node IDs
serially when, and only when, the entire failure set is a bounded set of
per-test failures. Collection errors, internal pytest errors, unattributable
non-zero exits, or failure sets above the lane's rerun cap fail the phase
without masking. Operators can disable the bridges with
`GZA_UNIT_SERIAL_RERUN=0` or `GZA_FUNCTIONAL_SERIAL_RERUN=0` when debugging
suspected parallel-only real failures. Treat the `unit-rerun:` and
`functional-rerun:` `PARALLEL-ONLY FAILURE (passed serially)` lines as a real
signal to follow up, not as noise to suppress.
