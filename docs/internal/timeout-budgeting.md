...
## Verify-lane parallel-only rerun bridges
...
- the parallel xdist pass runs first with `--maxfail=GZA_<PHASE>_RERUN_CAP+1`, so
...
- if that pass fails only because a bounded set of per-test node IDs failed,
  ...
  timeout and logs each `PARALLEL-ONLY FAILURE (passed serially)` line under a
  phase-specific `<phase>-rerun:` prefix;
- collection errors, internal errors, unattributable exits, or failure sets
  over the cap fail immediately with `<phase>-rerun: NOT masking - ...`.
...
budgeting work. `GZA_<PHASE>_SERIAL_RERUN=0` disables the bridge for operator
triage, and `GZA_<PHASE>_RERUN_CAP` keeps broad failure sets fail-closed.
