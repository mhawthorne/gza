...
- the parallel xdist pass runs first with `--maxfail=<RERUN_CAP>+1`, so the
...
budgeting work. `GZA_UNIT_SERIAL_RERUN=0` or `GZA_FUNCTIONAL_SERIAL_RERUN=0`
disables the respective bridge for operator triage, and
`GZA_UNIT_RERUN_CAP` / `GZA_FUNCTIONAL_RERUN_CAP` keep broad failure sets
fail-closed.
