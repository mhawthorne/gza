...
Copy `.gza/learnings.md` (and eventually the full learnings directory) into the worktree in task context before launching the provider. The copy is read-only from the agent's perspective; learnings are regenerated by gza-owned `internal` tasks in task context. No need to copy back.
