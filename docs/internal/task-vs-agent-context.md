...
Verify subprocesses use a separate disposable writable task DB copy for each verify attempt. The copy is fresh per attempt, so a rerun never inherits SQLite mutations from an earlier failed verify command, and the host control plane records only explicit verify result evidence after the subprocess exits. Native verify snapshots stay owner-private. Docker verify snapshots are still disposable copies: the container-visible path components are group-traversable, the per-snapshot temp directory is group-writable, and the database file is group-writable for the snapshot hierarchy's resolved group. Docker launch adds that resolved group to the container process, so the non-root container user can create SQLite sidecars and commit to the copy without granting world access to the live database. Temporary traversal grants on pre-existing checkout path components are shared leases across overlapping Docker verify snapshots, including separate gza processes, and are restored to their exact original modes only after the last dependent snapshot exits or the surviving lease cleanup observes dead holders.
