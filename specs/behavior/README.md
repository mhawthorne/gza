...
| [main-verify-self-heal.md](main-verify-self-heal.md) | North-star convergence contract for non-green configured local-target integration verify: classification-first target verify taxonomy, rerun-before-reuse, bounded red lifetime, deterministic-red repair, unavailable escalation, and no merge-stall-to-launch-stall cascade. |
| [shared-schema-isolation.md](shared-schema-isolation.md) | Shared durable database schema migration isolation: evaluated code cannot advance shared schema, only proven canonical landed code may migrate it, isolated copies may migrate independently, and schema-runtime skew is unavailable rather than code-red. |
