...
| `max_cycles_reached` — review churn | Current-head review→improve cycles hit the bound (`max_review_cycles`) and no stale-review refresh path is available. | Take over: review and fix inline, or redirect the work. | Better improve quality; raise/redesign the bound. |
