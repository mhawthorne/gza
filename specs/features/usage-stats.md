...
One table, append-only, one row per window per fetch:
...
CREATE INDEX idx_provider_usage_latest ON provider_usage_samples (provider, fetched_at DESC);
```

Append-only is the whole point: it costs nothing extra now and gives us burn-rate
analysis later (percent consumed per hour, projected exhaustion before reset)
without a second migration. A retention prune (drop samples older than N days,
default 30) runs opportunistically on write.

Failures are *not* stored as samples. A failed fetch leaves the last good sample
in place and records a separate lightweight failure marker (provider,
`last_error`, `last_error_at`) so consumers can render "stale, last error: …"
instead of silently showing a week-old number as current.
...
Compact (watch header, one line):

```
usage  codex 25% used · resets in 3h12m   (cached 4m ago)
```

Multiple windows collapse to the *most consumed* one in compact form, with the
rest available in the full form. Stale or failed:

```
usage  codex 25% used · resets in 3h12m   (stale 41m, last fetch failed)
...
- Samples accumulate in `provider_usage_samples`, oldest pruned by retention.
