...
from gza.cli.git_ops import (
    ...
    _coerce_merge_single_task_result,
...
def _assert_failed_merge_status_is_not_success(result: Any) -> None:
    assert result.rc != 0
    assert result.status not in {"merged", "already_merged"}


@pytest.mark.parametrize(
    ("scenario", "expected_status"),
    [
        ("dirty_checkout", "blocked_dirty_checkout"),
        ("merge_conflict", "merge_conflict"),
        ("already_merged_refusal", "already_merged_refused"),
        ("deferred_materialization_none", "deferred_blocker_materialization_failed"),
        ("mandatory_side_effect_failure", "merge_side_effect_materialization_failed"),
        ("merge_failure", "merge_failed"),
        ("merge_cleanup_failure", "merge_cleanup_failed"),
    ],
)
def test_manual_merge_execution_failure_statuses_are_never_success(
    tmp_path: Path,
    scenario: str,
    expected_status: str,
) -> None:
    setup_config(tmp_path)
    store = make_store(tmp_path)
    task = _completed_merge_task(store, f"Failure status {scenario}", f"feature/failure-status-{scenario}")
    assert store.get_or_create_merge_unit_for_task(task) is not None

    git = SimpleNamespace(
        repo_dir=tmp_path,
        is_merged=MagicMock(return_value=scenario == "already_merged_refusal"),
        default_branch=MagicMock(return_value="main"),
        has_changes=MagicMock(return_value=scenario == "dirty_checkout"),
        can_merge=MagicMock(return_value=scenario != "merge_conflict"),
        merge=MagicMock(),
        merge_abort=MagicMock(),
        reset_hard_head=MagicMock(),
    )
    hooks = _manual_merge_service_hooks()
    if scenario == "deferred_materialization_none":
        hooks = replace(hooks, materialize_deferred_blockers=lambda _task: None)
    elif scenario == "mandatory_side_effect_failure":
        hooks = replace(hooks, materialize_followups=lambda _task: (_ for _ in ()).throw(RuntimeError("boom")))
    elif scenario == "merge_failure":
        git.merge.side_effect = GitError("merge exploded")
    elif scenario == "merge_cleanup_failure":
        git.merge.side_effect = GitError("merge exploded")
        git.merge_abort.side_effect = GitError("abort failed")
        git.reset_hard_head.side_effect = GitError("reset failed")
    ...
    _assert_failed_merge_status_is_not_success(result)


@pytest.mark.parametrize(
    "factory",
    [
        lambda: ManualMergeExecutionResult(rc=1, status="merged"),
        lambda: ManualMergeExecutionResult(rc=1, status="already_merged"),
        lambda: _MergeSingleTaskResult(rc=1, status="merged"),
        lambda: _MergeSingleTaskResult(rc=1, status="already_merged"),
        lambda: _MergeActionResult(
            rc=1,
            status="merged",
            created_followups=[],
            reused_followups=[],
...
def test_merge_result_models_reject_failure_success_status(factory: Any) -> None:
    with pytest.raises(ValueError):
        factory()


def test_merge_single_task_integer_failure_coercion_is_not_success() -> None:
    result = _coerce_merge_single_task_result(2)
...
@pytest.mark.parametrize(
    ("scenario", "expected_status"),
    [
        ("removed_option", "invalid_merge_options"),
        ("missing_task", "merge_subject_not_found"),
        ("invalid_execution_state", "invalid_execution_state"),
        ("merge_source_warning", "merge_source_warning"),
        ("no_source", "no_resolvable_merge_source"),
    ],
)
def test_merge_single_task_early_refusals_are_not_success_status(
    tmp_path: Path,
    scenario: str,
    expected_status: str,
) -> None:
    setup_config(tmp_path)
    store = make_store(tmp_path)
    git = SimpleNamespace(default_branch=MagicMock(return_value="main"))
    args = _manual_merge_args()
    task_id = "testproject-404"
    resolved = None

    if scenario == "removed_option":
        args.rebase = True
    elif scenario != "missing_task":
        task = _completed_merge_task(store, f"Early refusal {scenario}", f"feature/early-{scenario}")
        task_id = task.id
        execution_task = replace(task)
        merge_source_warning = None
        merge_branch = task.branch
        merge_source_ref = task.branch
        if scenario == "invalid_execution_state":
            execution_task.status = "failed"
        elif scenario == "merge_source_warning":
            merge_source_warning = "source ref is ambiguous"
        elif scenario == "no_source":
            merge_branch = None
            merge_source_ref = None
...
@pytest.mark.parametrize(
    ...
    [
        ("invalid_execution_state", "invalid_execution_state"),
        ("wrong_target_checkout", "blocked_lifecycle_gate"),
        ("merge_source_warning", "merge_source_warning"),
    ],
)
def test_execute_merge_action_isolated_preflight_refusals_are_not_success_status(
    tmp_path: Path,
    scenario: str,
    expected_status: str,
) -> None:
    setup_config(tmp_path)
    store = make_store(tmp_path)
    task = _completed_merge_task(store, f"Isolated refusal {scenario}", f"feature/isolated-refusal-{scenario}")
    execution_task = replace(task)
    merge_source_warning = None
    execution_branch = "main"
    if scenario == "invalid_execution_state":
        execution_task.status = "failed"
    elif scenario == "wrong_target_checkout":
        execution_branch = "feature/not-main"
    elif scenario == "merge_source_warning":
        merge_source_warning = "source ref is ambiguous"
    resolved = _ResolvedMergeSubject(
        trigger_task=task,
    ...
    repo_git = SimpleNamespace(repo_dir=tmp_path)
    merge_git = SimpleNamespace(repo_dir=tmp_path / "isolated")

    with patch("gza.cli.git_ops._resolve_merge_subject", return_value=resolved):
...
def _landing_authorization(**overrides: Any) -> MergeLandingAuthorization:
    values: dict[str, Any] = {
        ...
        "authoritative_scope_identity": "scope:a",
        ...
        "live_judgment_identity": "judgment:a",
        "review_id": "testproject-2",
        "reviewed_head": "source-sha",
        "review_mode": "plain_full",
        "review_verdict": "CHANGES_REQUESTED",
        "blocker_identities": (
            '{"class":"out_of_scope","deferrable":true,"finding_id":"B1",'
...
def _landing_blocker_identity(review_id: str, finding_id: str = "B1", fingerprint: str = "blocker:a") -> str:
    return (
        '{"class":"out_of_scope","deferrable":true,'
        f'"finding_id":"{finding_id}","fingerprint":"{fingerprint}",'
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {"authoritative_scope_identity": "scope:b"},
        {"policy_version": "guarded.v2"},
        {"schema_version": "landing_judge.v2"},
        {"live_judgment_identity": "judgment:b"},
        {"representative_task_id": "testproject-9"},
        {"member_task_ids": ("testproject-1", "testproject-9")},
        {"merge_unit_id": "mu-2"},
        {"blocker_identities": ('{"class":"adjacent","deferrable":true,"finding_id":"B1"}',)},
        {"blocker_fingerprints": ("blocker:b",)},
        {"followup_identities": ('{"finding":"F2","review":"testproject-2"}',)},
...
def test_manual_merge_boundary_stable_landing_authorization_materializes_before_merge_and_state(
    ...
    assert task.id is not None
    assert store.get_or_create_merge_unit_for_task(task) is not None
    blocker = _deferred_blocker_task(store, task, "testproject-2", "B1")
...
@pytest.mark.parametrize(
    ...
    [
        ("empty", ("B1",), lambda store, task, review_id: ([], [])),
        ("partial", ("B1", "B2"), lambda store, task, review_id: ([_deferred_blocker_task(store, task, review_id, "B1")], [])),
        ("wrong_identity", ("B1",), lambda store, task, review_id: ([_deferred_blocker_task(store, task, review_id, "B2")], [])),
        ("non_urgent", ("B1",), lambda store, task, review_id: ([_deferred_blocker_task(store, task, review_id, "B1", urgent=False)], [])),
        ("non_pr", ("B1",), lambda store, task, review_id: ([_deferred_blocker_task(store, task, review_id, "B1", create_pr=False)], [])),
    ],
)
def test_manual_merge_boundary_refuses_incomplete_guarded_deferred_blockers_before_merge(
    tmp_path: Path,
    scenario: str,
    ...
    setup_config(tmp_path)
    store = make_store(tmp_path)
    task = _completed_merge_task(store, f"Guarded deferred {scenario}", "feature/landing-auth")
    assert task.id is not None
    assert store.get_or_create_merge_unit_for_task(task) is not None
    review_id = "testproject-2"
    blocker_identities = tuple(
        _landing_blocker_identity(review_id, finding_id, f"blocker:{finding_id.lower()}")
    ...
    git = SimpleNamespace(
        ...
        has_changes=MagicMock(return_value=False),
        can_merge=MagicMock(return_value=True),
        merge=MagicMock(side_effect=AssertionError("merge must not run without exact deferred blocker set")),
        rev_parse_if_exists=MagicMock(
            side_effect=lambda ref: {
                task.branch: authorized.source_sha,
                "main": authorized.target_sha,
    ...
    hooks = replace(
        _manual_merge_service_hooks(),
        load_landing_authorization=lambda: authorized,
        materialize_deferred_blockers=lambda _task: materialized(store, task, review_id),
        materialize_followups=MagicMock(side_effect=AssertionError("follow-ups must not materialize after deferred refusal")),
    )
    request = replace(
        _manual_merge_service_request(tmp_path, store, git, task, merge_source="manual_land_escalated"),
    ...
    assert result.status == "deferred_blocker_materialization_failed"
    assert "guarded deferred blocker materialization did not match authorization" in (result.block_reason or "")
    git.merge.assert_not_called()


def test_manual_merge_boundary_accepts_exact_guarded_deferred_blocker_set_before_merge(
    tmp_path: Path,
) -> None:
    setup_config(tmp_path)
    store = make_store(tmp_path)
    task = _completed_merge_task(store, "Guarded exact deferred set", "feature/landing-auth")
    assert task.id is not None
    assert store.get_or_create_merge_unit_for_task(task) is not None
    review_id = "testproject-2"
    authorized = _landing_authorization(
        ...
        review_id=review_id,
        blocker_identities=(
            _landing_blocker_identity(review_id, "B1", "blocker:b1"),
    ...
    order: list[str] = []
    blockers = [
        _deferred_blocker_task(store, task, review_id, "B1"),
        _deferred_blocker_task(store, task, review_id, "B2"),
    ]
    git = SimpleNamespace(
        repo_dir=tmp_path,
        is_merged=MagicMock(return_value=False),
    ...
    hooks = replace(
        _manual_merge_service_hooks(),
        load_landing_authorization=lambda: authorized,
        materialize_deferred_blockers=lambda _task: order.append("deferred") or ([blockers[0]], [blockers[1]]),
        materialize_followups=lambda _task: order.append("followups") or ([], []),
    )
    request = replace(
        _manual_merge_service_request(tmp_path, store, git, task, merge_source="manual_land_escalated"),
        landing_authorization=authorized,
...
def test_manual_merge_boundary_refuses_guarded_review_race_empty_materialization_before_merge(
    tmp_path: Path,
) -> None:
    setup_config(tmp_path)
    store = make_store(tmp_path)
    task = _completed_merge_task(store, "Guarded review race", "feature/landing-auth")
    assert task.id is not None
    assert store.get_or_create_merge_unit_for_task(task) is not None
    review = store.add("Current review", task_type="review", based_on=task.id, depends_on=task.id)
    assert review.id is not None
    review.status = "completed"
    ...
    authorized = _landing_authorization(
        owner_task_id=task.id,
        review_id=review.id,
        blocker_identities=(_landing_blocker_identity(review.id, "B1", "blocker:b1"),),
        blocker_fingerprints=("blocker:b1",),
    )
    git = SimpleNamespace(
        repo_dir=tmp_path,
        is_merged=MagicMock(return_value=False),
        has_changes=MagicMock(return_value=False),
        can_merge=MagicMock(return_value=True),
        merge=MagicMock(side_effect=AssertionError("merge must not run after review race")),
        rev_parse_if_exists=MagicMock(
            side_effect=lambda ref: {
                task.branch: authorized.source_sha,
                "main": authorized.target_sha,
    ...
    def materialize_after_review_race(_task: DbTask) -> tuple[list[DbTask], list[DbTask]]:
        review.output_content = "## Verdict\n\nVerdict: APPROVED\n"
        store.update(review)
        return ([], [])
    ...
    hooks = replace(
        _manual_merge_service_hooks(),
        load_landing_authorization=lambda: authorized,
        materialize_deferred_blockers=materialize_after_review_race,
        materialize_followups=MagicMock(side_effect=AssertionError("follow-ups must not materialize after review race")),
    )
    request = replace(
        _manual_merge_service_request(tmp_path, store, git, task, merge_source="manual_land_escalated"),
        landing_authorization=authorized,
...
def test_manual_merge_execution_api_late_already_merged_refusal_is_not_success_status(
    ...
    assert result.rc == 1
    assert result.status == "already_merged_refused"
    assert result.status != "merged"
...
@pytest.mark.parametrize("path", ["nonisolated", "isolated"])
...
def test_execute_merge_action_materializes_fresh_inner_replan_identity_before_merge(
    ...
    def _merge_side_effect(*_args: object, **kwargs: object) -> _MergeSingleTaskResult:
        ...
        return _MergeSingleTaskResult(rc=0, status="merged", authorized_merge_action=dict(fresh_action))
...
@pytest.mark.parametrize("action_family", ["ordinary"])
@pytest.mark.parametrize(
...
def test_execute_merge_action_ordinary_merge_persists_state_and_followups_once(
    ...
    with (
        patch("gza.cli.git_ops._build_auto_merge_args", return_value=argparse.Namespace()),
        patch("gza.cli.git_ops._merge_single_task", return_value=_MergeSingleTaskResult(rc=0, status="merged")),
        patch("gza.cli.git_ops._create_or_reuse_capped_review_blocker_tasks") as materialize,
    ):
        result = _execute_merge_action(
            config,
...
def test_execute_merge_action_isolated_candidate_verify_red_blocks_promotion(
    ...
    with (
        ...
        patch(
            "gza.cli.git_ops._merge_single_task",
            return_value=_MergeSingleTaskResult(rc=0, status="merged", authorized_source_ref_sha="isolated-merge-oid"),
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("blank-verify-source-moved", "isolated-single"),
    ],
)
...
def test_execute_merge_action_isolated_followup_source_move_during_materialization_preserves_child_without_promotion(
    ...
    with (
        patch(
            "gza.cli.git_ops._merge_single_task",
            return_value=_MergeSingleTaskResult(rc=0, status="merged", authorized_source_ref_sha="same-sha"),
        ),
        patch("gza.cli.git_ops._promote_isolated_merge_to_target_branch") as promote,
    ):
        result = _execute_merge_action(
