...
class TestEditCommand:
    ...
    def test_edit_add_tag_single_mutation_succeeds(self, tmp_path: Path):
        ...
        assert result.returncode == 0
        assert f"Updated tags for task {task.id}: backend, release-1.2" in result.stdout
...
class TestRetagCommand:
    ...
    def test_edit_pr_and_add_tag_apply_both_mutations(self, tmp_path: Path):
        ...
        assert result.returncode == 0
        assert f"Enabled PR creation/reuse request for successful completion of task {task.id}" in result.stdout
        assert f"Updated tags for task {task.id}: cli" in result.stdout
    ...
    def test_edit_combines_prompt_metadata_and_tag_mutations_atomically(self, tmp_path: Path) -> None:
        setup_config(tmp_path)
        store = make_store(tmp_path)
        task = store.add("Original prompt text", tags=("existing",))
        assert task.id is not None
        ...
        assert result.returncode == 0
        assert f"✓ Updated tags for task {task.id}: existing, new" in result.stdout
        assert "Added tags" not in result.stdout
        updated = store.get(task.id)
        assert updated is not None
        assert updated.prompt == "Updated combined prompt"
        assert updated.create_review is True
        assert updated.tags == ("existing", "new")
        assert updated.status == "pending"
    ...
    def test_edit_completed_prompt_with_tag_rejects_without_partial_mutation(
        ...
        setup_config(tmp_path)
        store = make_store(tmp_path)
        task = store.add("Original completed prompt", tags=("existing",))
        assert task.id is not None
        task.status = "completed"
        task.completed_at = datetime.now(UTC)
        store.update(task)
        ...
        assert persisted is not None
        assert persisted.prompt == "Original completed prompt"
        assert persisted.tags == ("existing",)
        assert persisted.status == "completed"
        assert persisted.completed_at == task.completed_at
...
class TestIterateCommand:
    ...
    def _make_failed_root_with_completed_recovery_descendant(self, store, *, branch: str):
        failed_root = store.add("Implement recovered feature", task_type="implement")
        assert failed_root.id is not None
        failed_root.status = "failed"
        failed_root.failure_reason = "MAX_TURNS"
        failed_root.session_id = "sess-recovered-root"
        failed_root.branch = branch
        failed_root.completed_at = datetime.now(UTC)
        store.update(failed_root)

        completed_descendant = store.add(
            failed_root.prompt,
            task_type="implement",
            based_on=failed_root.id,
        )
        assert completed_descendant.id is not None
        completed_descendant.status = "completed"
        completed_descendant.session_id = failed_root.session_id
        completed_descendant.branch = failed_root.branch
        completed_descendant.has_commits = True
        completed_descendant.merge_status = "unmerged"
        completed_descendant.completed_at = datetime.now(UTC)
    ...
    def _make_review_task(
        ...
        *,
        ...
        review = store.add("Review recovered feature", task_type="review", depends_on=impl.id)
        assert review.id is not None
        review.status = status
        if verdict is not None:
            if verdict == "CHANGES_REQUESTED":
                review.output_content = (
                    ...
                    "## Blockers\n\n"
                    "### B1 Broken behavior\n"
                    "Evidence: current behavior is wrong.\n"
                    "Required fix: correct the behavior.\n"
                    "Required tests: add a regression.\n\n"
                    "## Follow-Ups\n\nNone.\n\n"
                    "## Verdict\n\n"
                    "Verdict: CHANGES_REQUESTED\n"
                )
            elif verdict == "APPROVED_WITH_FOLLOWUPS":
                review.output_content = (
                    "## Summary\n\nLooks good with follow-up work.\n\n"
                    "## Blockers\n\nNone.\n\n"
                    "## Follow-Ups\n\n"
                    "### F1 Add coverage\n"
                    "Recommended follow-up: add broader coverage.\n\n"
                    "## Verdict\n\n"
                    "Verdict: APPROVED_WITH_FOLLOWUPS\n"
                )
            else:
                review.output_content = f"## Verdict\n\nVerdict: {verdict}\n"
        if status == "completed":
            review.completed_at = completed_at or datetime.now(UTC)
        store.update(review)
        return review

    def _make_iterate_mock_config(self, tmp_path: Path, **overrides: object) -> object:
        from unittest.mock import MagicMock
    ...
    def _persist_current_failed_verify_with_completed_verify_fix(
        ...
        *,
        ...
        verify_fix.completed_at = captured_at + timedelta(minutes=5)
        verify_fix.branch = impl.branch
        verify_fix.has_commits = True
        store.update(verify_fix)
        return config, verify_fix

    def _persist_timeout_verify_with_completed_verify_fix(
        self,
        store,
        tmp_path: Path,
        impl,
        *,
        head_sha: str,
        invalid_canonical_outcome: str | None = None,
        ...
        verify_fix.review_verify_head_sha = head_sha
        if invalid_canonical_outcome is not None:
            verify_fix.verify_fix_completion_outcome_json = invalid_canonical_outcome
        if invalid_legacy_scope is not None:
            verify_fix.verify_fix_completion_outcome_json = None
            verify_fix.review_scope = invalid_legacy_scope
            verify_fix.changed_diff = None
            verify_fix.review_verify_head_sha = None
        store.update(verify_fix)
        return config, verify_fix

    def _patch_fresh_verify_gate(
        self,
        stack: ExitStack,
        *,
        config,
        ...
        def _git_for_path(path):
            ...
            worktree_git.rev_parse_if_exists.side_effect = lambda ref: head_sha if ref == "HEAD" else None
            return worktree_git

        def _run_verify(*args, **kwargs):
            verify_calls.append(SimpleNamespace(args=args, kwargs=kwargs))
        ...
        stack.enter_context(patch("gza.cli.unstick.Git", return_value=mock_git))
        stack.enter_context(patch("gza.cli.watch.Git", return_value=mock_git))
        stack.enter_context(patch("gza.cli.advance_executor.Git", side_effect=_git_for_path))
        stack.enter_context(patch("gza.db.Git", return_value=mock_git, create=True))
        stack.enter_context(patch("gza.cli.advance_executor._resolve_review_verify_base_sha", return_value="base-head"))
        stack.enter_context(patch("gza.runner._run_review_verify_command", side_effect=_run_verify))
        ...
        stack.enter_context(patch("gza.cli.unstick.get_concurrency_snapshot", return_value=snapshot))
        stack.enter_context(patch("gza.cli.watch.get_concurrency_snapshot", return_value=snapshot))
        ...
        stack.enter_context(patch("gza.cli.watch.reconcile_stale_watch_no_progress_parks", return_value=0))
        ...
        stack.enter_context(patch("gza.cli.watch._maybe_file_main_verify_remediation", return_value=None))
        stack.enter_context(
            patch(
        ...
        stack.enter_context(patch("gza.cli.watch._settle_watch_dispatch_starts", return_value=[]))
    ...
    def _approved_plan_review_output(self, source_task_id: str, *, source_task_type: str = "plan") -> str:
        return (
            "## Verdict\n"
            "Verdict: APPROVED\n\n"
            "## Slice Manifest\n"
            "```json\n"
            "{"
            f"\"schema_version\":1,\"source_task_id\":\"{source_task_id}\","
            f"\"source_task_type\":\"{source_task_type}\","
            "\"verdict\":\"APPROVED\","
            "\"slice_quality\":{\"fits_single_task_budget\":true,\"timeout_budget_minutes\":30,"
            "\"max_expected_files_changed_per_slice\":8,\"rationale\":\"Bounded.\"},"
            "\"slices\":["
    ...
    def test_iterate_verify_only_noop_recovery_clears_even_when_verify_evidence_predates_review_completion(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import MagicMock, patch
        ...
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store)
        impl.branch = "feature/iterate-verify-only-stale-evidence"
        store.update(impl)
        ...
        prior_review.completed_at = datetime(2026, 6, 27, 18, 30, tzinfo=UTC)
        ...
        noop_improve.completed_at = datetime(2026, 6, 27, 18, 35, tzinfo=UTC)
        ...
        mock_config = MagicMock(
            ...
            use_docker=False,
            project_prefix="testproject",
            verify_command="uv run pytest tests/unit -q",
        )
        mock_git = MagicMock()
        mock_git.current_branch.return_value = "main"
        mock_git.branch_exists.return_value = True
        ...
        assert "Iterate complete: BLOCKED" in output
    ...
    def test_cmd_iterate_routes_plan_tasks_and_rejects_unsupported_types(self, tmp_path: Path) -> None:
        ...
        explore_task = store.add("An explore", task_type="explore")
        ...
        result = cmd_iterate(explore_args)
        assert result == 1


    def test_pending_plan_iterate_runs_source_then_materializes(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        ...
        plan_task = store.add("Pending plan", task_type="plan")
        assert plan_task.id is not None
        ...
        def fake_run_foreground(_config, task_id, **_kwargs):
            ...
            if task.task_type == "plan_review":
                task.output_content = self._approved_plan_review_output(plan_task.id)
            store.update(task)
            return 0
        ...
        assert rc == 0
        assert run_calls[0] == plan_task.id
        assert "Running pending plan source" in output
        assert "Created implement task" in output

    def test_failed_plan_iterate_requires_restart_flag(self, tmp_path: Path) -> None:
        import argparse
        ...
        failed_plan = store.add("Failed plan", task_type="plan")
        assert failed_plan.id is not None
        failed_plan.status = "failed"
        failed_plan.failure_reason = "TIMEOUT"
        failed_plan.session_id = "plan-session-1"
        store.update(failed_plan)

        args = argparse.Namespace(
            impl_task_id=failed_plan.id,
            max_iterations=1,
            dry_run=False,
            project_dir=tmp_path,
            no_docker=True,
        ...
        assert cmd_iterate(args) == 1

    def test_plan_iterate_rejects_resume_for_non_failed_plan(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        ...
        plan_task = store.add("Completed plan", task_type="plan")
        assert plan_task.id is not None
        plan_task.status = "completed"
        plan_task.completed_at = datetime.now(UTC)
        store.update(plan_task)
        ...
        assert cmd_iterate(args) == 1
        assert "only valid for failed tasks" in capsys.readouterr().out

    def test_failed_plan_iterate_resume_runs_recovery_then_materializes(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import patch
        ...
        def fake_run_foreground(_config, task_id, **_kwargs):
            ...
            if task.task_type == "plan_review":
                ...
                task.output_content = self._approved_plan_review_output(task.depends_on)
            store.update(task)
            return 0
        ...
        output = capsys.readouterr().out
        resume_children = [
            task for task in store.get_all() if task.task_type == "plan" and task.based_on == failed_plan.id
        ]

        assert rc == 0
        assert len(resume_children) == 1
        assert run_calls[0] == resume_children[0].id
        assert "Resuming failed plan source" in output
        assert "Created implement task" in output

    def test_failed_plan_iterate_resume_recovery_exhaustion_uses_shared_attention(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import patch
        ...
        assert failed_plan.id is not None
        failed_plan.status = "failed"
        failed_plan.failure_reason = "MAX_STEPS"
        failed_plan.session_id = "plan-session-1"
        store.update(failed_plan)
        ...
        def fake_run_foreground(_config, task_id, resume=False, **_kwargs):
            task = store.get(task_id)
            assert task is not None
            task.status = "failed"
            task.failure_reason = "MAX_STEPS"
            task.session_id = "plan-session-1"
            store.update(task)
            return 1
        ...
        recovery_children = store.get_based_on_children(failed_plan.id)
    ...
    def test_failed_held_plan_iterate_retry_preserves_hold_and_stops_for_human(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import patch
        ...
        failed_plan = store.add("Held failed plan", task_type="plan", auto_implement=False)
        assert failed_plan.id is not None
        failed_plan.status = "failed"
        failed_plan.failure_reason = "TIMEOUT"
        store.update(failed_plan)
        ...
        assert retry_children[0].auto_implement is False
        assert len(plan_reviews) == 0
        assert len(implement_tasks) == 0
        assert "Retrying failed plan source" in output
        assert f"use uv run gza implement {retry_children[0].id}".lower() in output.lower()

    def test_plan_iterate_changes_requested_runs_plan_improve_then_materializes(
        self, tmp_path: Path
    ) -> None:
        import argparse
        ...
        def fake_run_foreground(_config, task_id, **_kwargs):
            ...
            if task.task_type == "plan_review":
                ...
                if review_runs == 1:
                    ...
                    assert task.depends_on is not None
                    task.output_content = self._approved_plan_review_output(
                        task.depends_on,
    ...
    def test_plan_iterate_accepts_latest_revision_after_plan_review_cycle_cap(
        ...
        with (
            ...
            patch("gza.cli._run_foreground", side_effect=fake_run_foreground),
            patch(
                "gza.cli.execution.execute_advance_action",
                side_effect=fake_execute_advance_action,
            ),
        ):
            rc = cmd_iterate(args)
    ...
    def test_plan_iterate_reports_blocked_states(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        wait_plan = store.add("Plan with running review", task_type="plan")
        wait_plan.status = "completed"
        wait_plan.completed_at = datetime.now(UTC)
        store.update(wait_plan)
        running_review = store.add("Review", task_type="plan_review", depends_on=wait_plan.id)
        running_review.status = "in_progress"
        store.update(running_review)

        disabled_plan = store.add("Disabled plan", task_type="plan")
        disabled_plan.status = "completed"
        disabled_plan.completed_at = datetime.now(UTC)
        store.update(disabled_plan)

        held_args = argparse.Namespace(
            impl_task_id=held_plan.id,
            max_iterations=1,
            dry_run=False,
            project_dir=tmp_path,
            no_docker=True,
        ...
        wait_args = argparse.Namespace(**{**vars(held_args), "impl_task_id": wait_plan.id})
        disabled_args = argparse.Namespace(**{**vars(held_args), "impl_task_id": disabled_plan.id})

        assert cmd_iterate(held_args) == 3
        held_output = capsys.readouterr().out
        assert f"use uv run gza implement {held_plan.id}".lower() in held_output.lower()

        assert cmd_iterate(wait_args) == 3
        wait_output = capsys.readouterr().out
        assert "already in progress" in wait_output.lower()

        config = Config.load(tmp_path)
        config.require_plan_review_before_implement = False
        with patch("gza.cli.execution.Config.load", return_value=config):
    ...
    def test_plan_iterate_invalid_approved_manifest_blocks_with_detail(
        ...
        plan_task = store.add("Invalid manifest plan", task_type="plan")
        assert plan_task.id is not None
        plan_task.status = "completed"
        plan_task.completed_at = datetime.now(UTC)
        store.update(plan_task)
        ...
        output = capsys.readouterr().out
        assert "invalid slice manifest" in output

    def test_plan_iterate_materialize_action_falls_back_to_latest_review_inputs(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import patch
        ...
        plan_task = store.add("Fallback manifest plan", task_type="plan")
        assert plan_task.id is not None
        plan_task.status = "completed"
        plan_task.completed_at = datetime.now(UTC)
        store.update(plan_task)
        ...
        with patch(
            ...
            return_value={"type": "materialize_plan_slices", "description": "materialize"},
        ):
            assert cmd_iterate(args) == 0
    ...
    def test_plan_iterate_repair_plan_slice_materialization_executes_action(
        ...
        partial_impl.status = "pending"
        store.update(partial_impl)
    ...
    def test_plan_iterate_background_bypasses_impl_preflight(
        ...
        plan_task = store.add("Background plan", task_type="plan")
        assert plan_task.id is not None
        plan_task.status = "completed"
        plan_task.completed_at = datetime.now(UTC)
        store.update(plan_task)
        ...
        with (
            patch("gza.cli.execution.Git", side_effect=AssertionError("implementation preflight should not run")),
            patch("gza.cli.execution._spawn_background_iterate_worker", return_value=0) as spawn_background,
        ):
            assert cmd_iterate(args) == 0

        spawn_background.assert_called_once()

    def test_plan_iterate_dry_run_and_already_materialized_skip(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        ...
        plan_task = store.add("Materialized plan", task_type="plan")
        assert plan_task.id is not None
        plan_task.status = "completed"
        plan_task.completed_at = datetime.now(UTC)
        store.update(plan_task)

        review = store.add("Review materialized plan", task_type="plan_review", depends_on=plan_task.id)
        review.status = "completed"
        review.completed_at = datetime.now(UTC)
        review.output_content = self._approved_plan_review_output(plan_task.id)
        store.update(review)
        ...
        assert review_outcome.manifest is not None
        ...
        run_args = argparse.Namespace(**{**vars(dry_run_args), "dry_run": False})

        assert cmd_iterate(dry_run_args) == 0
        dry_run_output = capsys.readouterr().out
        assert "[dry-run] Would iterate plan" in dry_run_output

        assert cmd_iterate(run_args) == 0
        output = capsys.readouterr().out
        implement_tasks = [task for task in store.get_all() if task.task_type == "implement"]

        assert len(implement_tasks) == len(materialization.tasks)
        assert "already materialized" in output.lower()
    ...
    def test_iterate_first_pass_approved_with_followups_creates_followup_and_rerun_reuses(
        ...
        assert followups_after_second[0].id == followups_after_first[0].id
        assert "Iterate complete: APPROVED (approved_with_followups)" in output_second
    ...
    def test_iterate_force_recovered_lineage_invalid_verify_fix_proof_remains_stopped(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
        proof_kind: str,
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    ...
                    side_effect=AssertionError("invalid verify-fix proof must not run workers"),
                )
            )
            result = cmd_iterate(args)
        output = capsys.readouterr().out
        ...
        assert "verify-fix-proof-unavailable" in output
        assert "Next action: verify_gate" not in output

    def test_iterate_force_recovered_lineage_different_verify_attention_reason_remains_stopped(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        from gza.cli import cmd_iterate
        ...
        with ExitStack() as stack:
            verify_calls = self._patch_fresh_verify_gate(
                ...
                mock_git=mock_git,
                impl=head,
                head_sha="recovered-unavailable-head",
                status="passed",
            )
            stack.enter_context(
                patch(
                    "gza.cli.execution._run_foreground",
                    side_effect=AssertionError("non verify-fix-failed park must not run workers"),
                )
            )
            result = cmd_iterate(args)
        output = capsys.readouterr().out
        ...
        assert result == 3
        assert len(verify_calls) == 0
        assert "verify-unavailable" in output
        assert "Next action: verify_gate" not in output
    ...
    def test_iterate_background_force_prepares_verify_gate_and_inner_loop_refreshes(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        from gza.cli import cmd_iterate
        ...
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store)
        impl.branch = "feature/iterate-background-force-verify-fix-failed"
        store.update(impl)
        config, _verify_fix = self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            impl,
            head_sha="background-force-head",
        )
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("gza.cli.execution._spawn_background_iterate_worker", side_effect=fake_spawn))
            result = cmd_iterate(parent_args)
        parent_output = capsys.readouterr().out

        assert result == 0
        assert len(verify_calls) == 0
        assert spawned
        assert spawned[0].args.force is True
        assert spawned[0].kwargs["prepared_task_id"] == impl.id
        assert spawned[0].kwargs["prepared_phase"] == "iteration"
        assert spawned[0].kwargs["prepared_action_type"] == "verify_gate"
        assert spawned[0].kwargs["prepared_verify_owner_task_id"] == impl.id
        assert "NEEDS_ATTENTION" not in parent_output

        self._persist_current_green_verify(
            store,
            tmp_path,
            impl,
            head_sha="background-force-head",
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("gza.cli.execution._run_foreground", return_value=3))
    ...
    def test_iterate_force_recovered_lineage_real_cap_keeps_maxed_out_exit(
        ...
        def fake_run_foreground(config, task_id, **kwargs):
            ...
            task = store.get(task_id)
            assert task is not None
            run_order.append(task.task_type)
            task.status = "completed"
            task.completed_at = datetime.now(UTC)
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("gza.cli.execution._run_foreground", side_effect=fake_run_foreground))
            result = cmd_iterate(args)
        output = capsys.readouterr().out

        assert result == 2
        assert len(verify_calls) == 1
        assert run_order == ["improve", "review"]
        ...
        assert "Max iterations (1) reached." in output

    def test_iterate_prepared_verify_gate_without_force_rejects_without_mutation(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        from gza.cli import cmd_iterate
        ...
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store)
        impl.branch = "feature/prepared-verify-gate-no-force"
        store.update(impl)
        config, _verify_fix = self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            impl,
            head_sha="prepared-no-force-head",
        )
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "gza.cli.execution._run_foreground",
                    side_effect=AssertionError("unforced prepared verify_gate must not continue"),
                )
            )
            result = cmd_iterate(args)
        output = capsys.readouterr().out
        ...
        assert result == 1
        assert len(verify_calls) == 0
        assert mock_git.worktree_add_existing.call_count == 0
        assert decision.state == "failed"
        assert "prepared verify_gate iterate startup requires --force" in output

    def test_iterate_prepared_verify_gate_rejects_foreign_owner_without_mutation(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        from gza.cli import cmd_iterate
        from gza.review_verify_state import resolve_verify_gate_decision
        ...
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store)
        impl.branch = "feature/prepared-verify-gate-owner"
        store.update(impl)
        config, _verify_fix = self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            impl,
            head_sha="prepared-owner-head",
        )
        foreign = self._make_completed_impl(store, prompt="Unrelated implementation")
        foreign.branch = "feature/unrelated-owner"
        store.update(foreign)
        self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            foreign,
            head_sha="foreign-owner-head",
        )
        ...
        args = argparse.Namespace(
            ...
            prepared_phase="iteration",
            prepared_action_type="verify_gate",
            prepared_verify_owner_task_id=foreign.id,
            prepared_review_task_id=None,
        )
        mock_git = MagicMock()
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "gza.cli.execution._run_foreground",
                    side_effect=AssertionError("foreign prepared verify owner must not continue"),
                )
            )
            result = cmd_iterate(args)
        output = capsys.readouterr().out

        impl_decision = resolve_verify_gate_decision(store, impl, config=config, git=mock_git)
        mock_git.rev_parse_if_exists.side_effect = lambda ref: "foreign-owner-head" if ref == foreign.branch else None
        foreign_decision = resolve_verify_gate_decision(store, foreign, config=config, git=mock_git)
        assert result == 1
        assert len(verify_calls) == 0
        assert mock_git.worktree_add_existing.call_count == 0
    ...
    def test_iterate_background_force_prepared_recovered_lineage_no_review_goes_merge_ready(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "gza.cli.execution._run_foreground",
                    side_effect=AssertionError("no-review force must not run review or improve workers"),
                )
            )
            stack.enter_context(
                patch(
                    "gza.cli.execution._create_review_task",
                    side_effect=AssertionError("no-review force must not create a review"),
                )
            )
            result = cmd_iterate(inner_args)
        inner_output = capsys.readouterr().out
        ...
        assert "run_review" not in inner_output
        assert "Iterate complete: MERGE_READY" in inner_output
    ...
    def test_unstick_run_by_tag_invokes_fresh_verify_for_verify_fix_failed_rearm(
        ...
        setup_config(tmp_path)
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store, prompt="Tagged verify-fix park")
        impl.branch = "feature/unstick-tag-verify-fix-failed"
        impl.tags = ("verify-refresh", "ops")
        store.update(impl)
        config, _verify_fix = self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            impl,
            head_sha="unstick-tag-head",
        )
    ...
    @pytest.mark.parametrize(
        ("patch_target", "failure_text"),
        (
            ("gza.cli.advance_executor._run_lifecycle_verify", "verify process exploded"),
            ("gza.cli.advance_executor._persist_lifecycle_verify_execution", "verify persistence exploded"),
        ),
    )
    def test_unstick_run_reports_fresh_verify_exception_as_direct_blocked(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
        patch_target: str,
        ...
        store = make_store(tmp_path)
        impl = self._make_completed_impl(store)
        impl.branch = f"feature/unstick-{failure_text.replace(' ', '-')}"
        store.update(impl)
        config, _verify_fix = self._persist_current_failed_verify_with_completed_verify_fix(
            store,
            tmp_path,
            impl,
            head_sha="unstick-exception-head",
        )
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch(patch_target, side_effect=RuntimeError(failure_text)))
            result = cmd_unstick(args)
        output = capsys.readouterr().out
    ...
    def test_iterate_run_improve_success_without_terminal_state_fails_closed(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import MagicMock, patch
        ...
        with (
            ...
            patch(
                ...
                return_value={"type": "run_improve", "description": "Run improve", "improve_task": improve},
            ),
        ...
        assert result == 3
        run_foreground.assert_called_once()
        assert "Iterate blocked: run_improve reported success but task" in output
        assert "remained pending; stop instead of looping on stale state." in output
    ...
    def test_background_iterate_failed_start_prepares_before_spawn_and_child_reuses_prepared_task(
        ...
        assert captured_spawn == {
            ...
            "prepared_action_type": None,
            "prepared_verify_owner_task_id": None,
            "prepared_review_task_id": None,
        }
    ...
    def test_failed_task_resume_duplicate_active_review_backed_improve_returns_phase1_error(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import MagicMock, patch
        ...
        active_resume = store.add(
            "Active improve recovery",
            task_type="improve",
        ...
        root_decision = FailedRecoveryDecision(
            task_id=impl.id,
            action="skip",
            reason_code="retry_limit_reached",
            reason_text="blocked by newer failed recovery descendant",
            launch_mode="none",
            attempt_index=1,
            attempt_limit=1,
        )
        descendant_decision = FailedRecoveryDecision(
            task_id=failed_improve.id,
            action="resume",
            reason_code="timeout_style_failure",
            reason_text="resume failed improve",
            launch_mode="iterate",
            attempt_index=1,
            attempt_limit=1,
        )
        ...
        with (
            ...
            patch("gza.cli.execution.get_store", return_value=store),
            patch(
                "gza.cli.execution.decide_failed_task_recovery",
                side_effect=[root_decision, descendant_decision],
            ),
            patch(
                "gza.cli.execution.get_failed_recovery_needs_attention_reason",
                return_value="newer-recovery-descendant-needs-attention",
            ),
            patch(
                "gza.cli.execution.get_manual_resume_override_descendant",
                return_value=failed_improve,
            ),
            patch(
                "gza.cli.execution._create_resume_task",
                side_effect=DuplicateActiveChildError(active_resume),
            ),
            patch(
                "gza.cli.execution._run_foreground",
                side_effect=AssertionError("iterate should not start foreground execution"),
            ),
        ):
            result = cmd_iterate(args)
        ...
        assert result == 1
        assert (
            f"Error: review-backed improve already pending/in progress for {failed_improve.id}: {active_resume.id}"
            in output
        )
    ...
    @pytest.mark.parametrize(
        ("recovery_action", "creator_name"),
        [
            ("resume", "_create_resume_task"),
            ("retry", "_create_retry_task"),
        ],
    )
    def test_iterate_duplicate_failed_improve_recovery_releases_permit_and_blocks(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
        recovery_action: str,
        ...
        assert active_recovery.id is not None
        active_recovery.status = "pending"
        store.update(active_recovery)
        ...
        decision = FailedRecoveryDecision(
            ...
            reason_code="timeout_style_failure",
            reason_text=f"{recovery_action} failed improve",
            launch_mode="iterate",
            attempt_index=1,
            attempt_limit=1,
        )
        ...
        with (
            ...
            patch(
                "gza.cli.execution.resolve_improve_action",
                return_value=(recovery_action, failed_improve, decision),
            ),
            patch(
                "gza.cli.execution.build_improve_needs_attention_result",
                return_value=None,
            ),
            patch(
                f"gza.cli.execution.{creator_name}",
                side_effect=DuplicateActiveChildError(active_recovery),
            ),
            patch(
                "gza.cli._run_foreground",
                side_effect=AssertionError("foreground iterate should not run duplicate recovery"),
            ),
        ):
            result = cmd_iterate(args)
        ...
        assert result == 3
        assert (
            f"Error creating improve task: review-backed improve already pending/in progress "
            f"for {failed_improve.id}: {active_recovery.id}"
        ) in output
        assert "Iterate blocked: improve_failed." in output
        permit = launch_permit(config, store)
        permit.release()
    ...
    def test_iterate_background_duplicate_active_rebase_releases_capacity_and_leaves_no_prepared_task(
        self, tmp_path: Path
    ) -> None:
        import argparse
        from unittest.mock import MagicMock, patch
        ...
        with (
            ...
            patch(
                "gza.cli.execution._create_rebase_task",
    ...
    def test_iterate_retry_foreground_duplicate_resolved_failed_rebase_returns_phase1_error_and_releases_capacity(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        ...
        with (
            ...
            patch("gza.cli.execution.Git", return_value=mock_git),
            patch(
                "gza.cli.execution.resolve_recovery_planning_task",
                return_value=failed_rebase,
            ),
            patch(
    ...
    def test_iterate_retry_background_duplicate_resolved_failed_rebase_skips_spawn_and_releases_capacity(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        ...
        with (
            ...
            patch(
                "gza.cli.execution._create_retry_task",
                side_effect=DuplicateActiveChildError(active_retry),
            ) as create_retry,
            patch(
                "gza.cli.execution._spawn_background_iterate",
                side_effect=AssertionError("duplicate retry should not spawn background iterate"),
            ),
        ):
            result = cmd_iterate(args)

        assert result == 0
        create_retry.assert_called_once()
        _ = capsys.readouterr()
        permit = launch_permit(config, store)
        permit.release()
    ...
    def test_iterate_needs_rebase_duplicate_active_rebase_releases_capacity_and_continues(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        import argparse
        from unittest.mock import MagicMock, patch
        ...
        assert result == 0
        captured = capsys.readouterr()
        assert f"Skipping rebase: rebase already pending/in progress for {impl.id}: {active_rebase.id}." in captured.out
        permit = launch_permit(config, store)
        permit.release()
    ...
    def test_background_iterate_duplicate_failed_improve_recovery_releases_permit_and_skips_spawn(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
        recovery_action: str,
        ...
        assert result == 0
        assert (
            f"Skipping improve recovery: review-backed improve already pending/in progress "
            f"for {failed_improve.id}: {active_recovery.id}."
        ) in output
        permit = launch_permit(config, store)
        permit.release()
...
class TestRunForeground:
    ...
    def test_auto_rebase_before_resume_duplicate_active_rebase_returns_clean_error(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """Resume preflight should fail closed when an active rebase child already exists."""
        from gza.cli import _auto_rebase_before_resume, _create_rebase_task
        ...
        active_rebase = _create_rebase_task(
            store,
            task.id,
            task.branch,
            "release",
        ...
        with patch("gza.cli.git_ops._run_task_backed_rebase") as mock_rebase:
            rc = _auto_rebase_before_resume(config, task.id)
...
class TestForegroundInvocationContextWiring:
    ...
    def test_cmd_retry_rejects_duplicate_active_rebase_retry_and_releases_permit(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
    ...
    def test_cmd_resume_rejects_duplicate_active_rebase_resume_and_releases_permit(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
...
class TestRunAsWorker:
    ...
    def test_run_as_worker_resume_duplicate_active_rebase_skips_run_without_worker_error(
        self,
        tmp_path: Path,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """Worker resume should surface duplicate active rebases as a controlled startup failure."""
        from gza.cli import _create_rebase_task

        setup_config(tmp_path)
        config = Config.load(tmp_path)
        store = SqliteTaskStore(config.db_path)
        task = store.add("Worker resume target", task_type="implement")
        assert task.id is not None
        task.status = "failed"
        task.session_id = "resume-session"
        task.branch = "feature/worker-resume-target"
        store.update(task)
        unit = store.create_merge_unit(
            source_branch=task.branch,
            target_branch="release",
        ...
        registry = self._register_current_worker(config, task.id, "w-worker-resume-duplicate")
        args = argparse.Namespace(task_ids=[task.id], resume=True)
        ...
        worker = registry.get("w-worker-resume-duplicate")
        assert worker is not None
        assert worker.status == "failed"
        assert worker.exit_code == 1
        assert worker.completion_reason == "startup failure before task claim"
...
class TestRecoveryTaskScopeCloning:
    @pytest.mark.parametrize("creator_name", ["_create_resume_task", "_create_retry_task"])
    @pytest.mark.parametrize("task_kind", ["review", "rebase", "review_backed_improve"])
    def test_singleton_recovery_creators_reject_duplicate_active_children(
        self,
        tmp_path: Path,
        creator_name: str,
        task_kind: str,
    ) -> None:
        from gza.cli._common import _create_resume_task, _create_retry_task

        setup_config(tmp_path)
        store = make_store(tmp_path)

        impl = store.add("Implement scoped slice", task_type="implement")
        assert impl.id is not None

        if task_kind == "review":
            original = store.add("Review scoped slice", task_type="review", based_on=impl.id)
        elif task_kind == "rebase":
            original = store.add(
                "Rebase scoped slice",
                task_type="rebase",
                based_on=impl.id,
        ...
        assert original.id is not None
        original.status = "failed"
        if creator_name == "_create_resume_task":
            original.session_id = f"session-{task_kind}"
        store.update(original)

        creator = _create_resume_task if creator_name == "_create_resume_task" else _create_retry_task
        first = creator(store, original, trigger_source="manual")
        assert first.id is not None

        with pytest.raises(DuplicateActiveChildError) as exc_info:
            creator(store, original, trigger_source="manual")

        assert exc_info.value.active_child.id == first.id

    @pytest.mark.parametrize("creator_name", ["_create_resume_task", "_create_retry_task"])
    def test_comments_only_improve_recovery_creators_allow_multiple_active_children(
        self,
        tmp_path: Path,
        creator_name: str,
    ) -> None:
        ...
        original = store.add("Improve from comments", task_type="improve", same_branch=True)
        assert original.id is not None
        original.status = "failed"
        if creator_name == "_create_resume_task":
            original.session_id = "comments-session"
        store.update(original)

        creator = _create_resume_task if creator_name == "_create_resume_task" else _create_retry_task
        first = creator(store, original, trigger_source="manual")
        second = creator(store, original, trigger_source="manual")
    ...
    def test_resume_task_preserves_held_plan_auto_implement(self, tmp_path: Path):
        from gza.cli._common import _create_resume_task
        ...
        assert resumed.auto_implement is False

    def test_retry_task_preserves_held_plan_auto_implement(self, tmp_path: Path):
        from gza.cli._common import _create_retry_task
        ...
        assert retried.auto_implement is False
