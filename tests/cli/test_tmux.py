...
class TestClaudeProviderTmuxMode:
    ...
    def test_spawn_passes_prompt_file_to_proxy(self, tmp_path: Path):
        ...
        assert result == 0
        # Find the new-session call and verify --prompt-file is in the proxy command
        tmux_new_session_calls = [
            c for c in mock_run.call_args_list
            if c[0][0][0] == "tmux" and "new-session" in c[0][0]
        ]
        assert tmux_new_session_calls, "tmux new-session must be called"
        new_session_cmd = tmux_new_session_calls[0][0][0]
        cmd_str = " ".join(new_session_cmd)
        assert "--prompt-file" in cmd_str, (
            "_spawn_background_worker must pass --prompt-file to the proxy "
            "so the proxy can deliver the prompt via PTY"
        )
