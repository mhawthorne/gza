...
from gza.review_verdict import ParsedReviewReport, ReviewBlockerSummary, ReviewFinding, parse_review_report
...
from gza.runner import (
    ...
    _build_single_project_verify_aggregate_details,
...
class _FakeGit:
    ...
    def resolve_refs(self, refs: tuple[str, ...], peel: str = "commit") -> dict[str, str | None]:
        ...
        return {ref: self._resolved_tree_shas.get(ref) for ref in refs}
...
def _add_review_blocker_resolution_artifact(
    ...
    *,
    ...
    state: str,
    created_at: datetime,
    reason: str = "already_satisfied",
    evidence: str = "The guard already exists on the current branch tip.",
    current_state_citation: str = "`src/api.py:12-18`",
    fingerprint_title: str = "missing api guard",
    fingerprint_anchor: str = "src/api.py:12-18",
    dispute_source_task_id: str | None = None,
    disputed_artifact_id: int | None = None,
    ...
    if disputed_artifact_id is not None:
        metadata["disputed_artifact_id"] = disputed_artifact_id
    ...
    return store.add_artifact(
        ...
        path=f".gza/artifacts/{state.lower()}-{finding_id.lower()}.txt",
        byte_size=0,
        sha256=("0" if state == "disputed" else "1") * 64,
        status=state,
        exit_status=reason,
        head_sha=head_sha,
        metadata=metadata,
...
def _branch_introduced_failed_verify_output(*, tree_fingerprint: str) -> str:
    return (
        f"gza-verify phase=failed name=unit duration_seconds=0.20 tree_fingerprint={tree_fingerprint}\n"
...
def test_completed_plan_with_partial_unrecorded_plan_materialization_needs_repair(tmp_path: Path) -> None:
    ...
    assert action["type"] == "repair_plan_slice_materialization"
    assert action["plan_source_task"].id == plan.id
    assert action["plan_review_task"].id == review.id
    assert action["partial_task_ids"] == (partial.id,)
    assert action["manifest_digest"] == plan_review_manifest_digest(action["manifest"])
    assert action["repair_trigger_source"] == "manual"
...
def test_completed_plan_improve_with_partial_unrecorded_materialization_needs_repair(tmp_path: Path) -> None:
    ...
    assert action["type"] == "repair_plan_slice_materialization"
    assert action["plan_source_task"].id == revised_plan.id
    assert action["plan_review_task"].id == revised_review.id
    assert action["partial_task_ids"] == (partial.id,)
    assert action["manifest_digest"] == plan_review_manifest_digest(action["manifest"])
    assert action["repair_trigger_source"] == "manual"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            lambda task: setattr(task, "status", "completed"),
            id="non-pending-partial",
        ),
        pytest.param(
            lambda task: setattr(task, "branch", "feature/partial-slice"),
            id="branch-bearing-partial",
        ),
        pytest.param(
            lambda task: setattr(task, "review_scope", "Mutated review scope"),
            id="mutated-partial-metadata",
        ),
    ],
)
def test_completed_plan_with_ambiguous_partial_slice_descendants_still_needs_discussion(
    tmp_path: Path,
    mutator,
) -> None:
    store = _make_store(tmp_path)
    ...
    action = evaluate_advance_rules(config, store, _FakeGit(can_merge=True), plan, "main")
    ctx = resolve_advance_context(config, store, _FakeGit(can_merge=True), plan, "main")

    assert action["type"] == "needs_discussion"
    assert action["needs_attention_reason"] == "plan-review-materialization-repair-needed"
    assert ctx.plan_materialization_state is not None
    assert ctx.plan_materialization_state.partial_slice_descendants_detected is True


def test_completed_plan_with_prompt_mutated_partial_slice_descendant_needs_discussion(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    assert partial.id is not None
    partial.prompt = f"{partial.prompt}\n\nOperator note: prompt was edited after materialization."
    store.update(partial)
    ...
    assert ctx.plan_materialization_state is not None
    assert ctx.plan_materialization_state.partial_slice_descendants_detected is True
    assert ctx.plan_materialization_state.partial_repair_candidate is None
...
def test_completed_plan_with_recorded_materialization_and_extra_pending_partial_repairs(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    assert action["type"] == "repair_plan_slice_materialization"
    assert action["partial_task_ids"] == (extra_partial.id,)
    assert action["repair_trigger_source"] == "plan-review"
    assert ctx.plan_materialization_state is not None
    assert ctx.plan_materialization_state.materialized is False
    assert ctx.plan_materialization_state.partial_repair_candidate is not None
    assert {task.id for task in created_tasks if task.id is not None} == set(ctx.plan_materialization_state.task_ids)


def test_completed_plan_with_recorded_materialization_and_unsafe_extra_partial_needs_discussion(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    assert extra_partial.id is not None
    extra_partial.branch = "feature/partial-slice"
    store.update(extra_partial)
...
def test_head_advanced_stale_review_refresh_preempts_max_cycles(tmp_path: Path, monkeypatch) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-head-advanced",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
...
@pytest.mark.parametrize(
    ("changed_diff", "expected_reason_text"),
    [
        (True, "changed diff"),
        (None, "change unknown"),
    ],
)
def test_rebase_stale_review_reason_precedence_over_branch_head_advance(
    tmp_path: Path,
    monkeypatch,
    refresh_review_status: str | None,
    expected_action_type: str,
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch=f"feature/stale-review-rebase-precedence-{refresh_review_status or 'create'}-{changed_diff}",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
    ...
    assert ctx.review_invalidated_by_progress is True
    assert ctx.review_invalidated_by_rebase is not None
    assert ctx.review_invalidated_by_rebase.id == rebase.id
    assert ctx.latest_reviewed_head_sha == "reviewed-sha"
    assert ctx.current_review_head_sha == "current-sha"
...
def test_live_branch_head_preempts_stale_merge_unit_head_for_capped_review(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-merge-unit-cache",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_review_freshness_probe_failure_parks_approved_review_instead_of_merging(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-probe-failure-approved",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_review_freshness_probe_failure_preempts_review_max_cycles(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-probe-failure-changes-requested",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_review_freshness_probe_failure_parks_cleared_review_instead_of_merging(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-probe-failure-cleared",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_head_advanced_stale_review_refresh_reuses_active_review(
    tmp_path: Path,
    monkeypatch,
    refresh_review_status: str,
    expected_action_type: str,
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch=f"feature/stale-review-head-advanced-{refresh_review_status}",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_head_advanced_stale_review_manual_refresh_with_auto_create_disabled(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-head-manual-refresh",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_missing_reviewed_head_sha_does_not_infer_stale_review(tmp_path: Path, monkeypatch) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-missing-reviewed-sha",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.output_content = "## Verdict\n\nVerdict: CHANGES_REQUESTED\n"
    ...
    assert ctx.latest_reviewed_head_sha is None
...
def test_capped_review_candidate_with_missing_verify_runs_pre_merge_verify_before_cap(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-missing-verify",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
...
def _assert_malformed_capped_review_stays_on_attention_path(
...
def test_capped_review_candidate_with_zero_parsed_blockers_stays_on_attention_path(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-zero-blockers",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    review.output_content = _review_output_with_findings("CHANGES_REQUESTED")
    store.update(review)
...
def test_capped_review_candidate_with_missing_report_stays_on_unavailable_attention_path(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-missing-output",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
...
def test_capped_review_candidate_with_malformed_blocker_id_stays_on_attention_path(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-blank-blocker-id",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    review.output_content = _review_output_with_findings("CHANGES_REQUESTED", blockers=("B1",))
    store.update(review)
    blank_id_finding = ReviewFinding(
        id="",
        severity="BLOCKER",
    ...
    monkeypatch.setattr(advance_engine_module, "parse_review_report", lambda _content: parsed_report)
    monkeypatch.setattr(review_tasks_module, "parse_review_report", lambda _content: parsed_report)
    monkeypatch.setattr(
        advance_engine_module,
        "summarize_review_blockers",
        lambda _content: ReviewBlockerSummary(1, 0, 0, 1),
    )
...
def test_capped_review_candidate_with_duplicate_blocker_ids_stays_on_attention_path(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-duplicate-blockers",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    review.output_content = _review_output_with_findings("CHANGES_REQUESTED", blockers=("B1", "B1"))
    store.update(review)
...
def test_capped_review_candidate_with_summary_disagreement_stays_on_attention_path(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-summary-disagreement",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    ...
    monkeypatch.setattr(
        advance_engine_module,
        "summarize_review_blockers",
        lambda _content: ReviewBlockerSummary(0, 0, 0, 0),
    )
...
def test_capped_review_candidate_with_stale_verify_runs_pre_merge_verify_before_cap(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-stale-verify",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
...
def test_opt_in_on_max_cycles_with_green_verify_emits_annotated_merge_action(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-green-verify",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
...
def test_capped_review_candidate_with_unavailable_verify_blocks_merge_and_attention_path(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-unavailable-verify",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    ...
    assert action["type"] == "verify_gate"
    assert action["verify_gate_phase"] == "pre_merge"
    assert action["verify_gate_state"] == "unavailable"
    assert action.get("max_cycles_merge_and_defer") is not True


def test_capped_review_candidate_with_newer_red_verify_uses_pre_merge_verify_fix_path(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/capped-review-newer-red-verify",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review.review_verify_head_sha = "current-sha"
    review.output_content = _review_output_with_findings("CHANGES_REQUESTED", blockers=("B1",))
    store.update(review)
    for status, exit_status, captured_at in (
        ("passed", "0", datetime(2026, 5, 10, 10, 5, tzinfo=UTC)),
        ("failed", "1", datetime(2026, 5, 10, 10, 10, tzinfo=UTC)),
    ):
        persist_verify_gate_artifact(
            store,
            config,
            ...
            result=ReviewVerifyResult(
                ...
                reviewed_base_sha="base-sha",
                working_directory=str(tmp_path),
                failure="pytest failed" if status == "failed" else None,
            ),
            verify_timeout_seconds=120,
            verify_timeout_grace_seconds=5.0,
            producer="test",
...
def test_merge_and_defer_stale_review_still_refreshes_before_capped_verify(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/merge-defer-stale-review-refresh",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "old-sha"
    review.output_content = _review_output_with_findings("CHANGES_REQUESTED", blockers=("B1",))
    store.update(review)
    _add_completed_improve_for_review(
        store,
    ...
    action = evaluate_advance_rules(
        ...
        _FakeGit(
            ...
            name_status_by_range={
                f"main...{impl.branch}": "M\tspecs/behavior/lifecycle-engine.md\n",
            },
        ),
        impl,
        "main",
...
def test_new_reviewed_head_does_not_reset_review_cycle_cap(tmp_path: Path, monkeypatch) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-review-current-head-epoch",
        when=datetime(2026, 5, 10, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC))
    review1.review_verify_head_sha = "old-sha"
...
def test_preserved_noop_rebase_only_refreshes_when_head_advances_independently(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/preserved-rebase-independent-head-advance",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_changed_rebase_completed_approved_resolution_review_with_blank_pre_rebase_shas_is_backfilled(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/rebase-resolution-completed-blank-pre-rebase",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    rebase = _add_completed_rebase(
        store,
...
def test_changed_rebase_completed_resolution_review_missing_pre_rebase_target_degrades_to_full_review(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/rebase-resolution-completed-missing-target-proof",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    rebase = _add_completed_rebase(
        store,
...
def test_live_branch_head_blocks_merge_when_merge_unit_head_is_stale(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/stale-approved-review-merge-unit-cache",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 5, 10, 11, 0, tzinfo=UTC))
    review.review_verify_head_sha = "reviewed-sha"
...
def test_changed_rebase_completed_resolution_review_with_missing_resolved_shas_degrades_to_full_review(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/rebase-review-missing-resolved-shas",
        when=datetime(2026, 5, 10, 10, 0, tzinfo=UTC),
    )
    rebase = _add_completed_rebase(
        store,
...
def test_disputed_noop_improve_routes_to_review_blocker_adjudication(tmp_path: Path) -> None:
    ...
    assert "review-blocker adjudication" in action["description"]
    candidate = action["review_blocker_adjudication_candidate"]
    assert candidate.finding.id == "B1"
    assert action["review_task"].id == review.id


def test_invalid_adjudication_clears_review_and_surfaces_invalid_status(tmp_path: Path) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    from gza.runner import REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-invalid",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    adjudication = store.add(
        build_review_blocker_adjudication_prompt(
            ...
            {
                "source_task_id": improve.id,
                "disputed_artifact_id": dispute_artifact.id,
                "reason": "already_satisfied",
                "evidence": "The guard already exists on the current branch tip.",
                "current_state_citation": "src/api.py:12-18",
            },
    ...
    adjudication.completed_at = datetime(2026, 5, 14, 12, 0, tzinfo=UTC)
    store.update(adjudication)
    assert adjudication.id is not None

    store.add_artifact(
        review.id,
        kind=REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND,
        label="invalid-B1",
        path=".gza/artifacts/invalid-b1.txt",
        byte_size=0,
        sha256="1" * 64,
        status="invalid",
        exit_status="already_satisfied",
        metadata={
            ...
            "review_task_id": review.id,
            "impl_task_id": impl.id,
            "source_task_id": adjudication.id,
            "source_task_type": "internal",
            "finding_id": "B1",
            "dispute_source_task_id": improve.id,
            "disputed_artifact_id": dispute_artifact.id,
            "reason": "already_satisfied",
            "evidence": "The guard already exists on the current branch tip.",
            "current_state_citation": "`src/api.py:12-18`",
...
def test_invalid_code_adjudication_does_not_clear_mixed_review_without_current_verify_provenance(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-mixed-review-still-blocked",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    review.completed_at = datetime(2026, 5, 14, 10, 0, tzinfo=UTC)
    review.output_content = (
        "## Summary\n\n- One code blocker was disputed, but verify still fails.\n\n"
        "## Blockers\n\n"
        "### B1 Missing API guard\n"
        "Evidence: the current code still accepts empty IDs.\n"
        "Open-state citation: `src/api.py:12-18`\n"
        ...
        "Required fix: reject empty IDs before calling the service.\n"
        "Required tests: add regression coverage for empty IDs.\n\n"
        "### B2 verify_command failure: flaky unit lane\n"
        "Evidence: verify_command failed with exit status 1.\n"
        "Impact: autonomous verify fails.\n"
        "Required fix: rerun verify_command on the current tip.\n"
        "Required tests: rerun verify_command.\n\n"
    ...
    assert action["type"] == "needs_discussion"
    assert action["needs_attention_reason"] == "improve-no-op"
    assert "review-blocker-invalid" not in action["description"]


def test_invalid_code_adjudication_clears_mixed_review_once_current_verify_provenance_exists(
    tmp_path: Path,
) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    from gza.runner import REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-mixed-review-cleared",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
...
def test_valid_adjudication_returns_to_normal_improve(tmp_path: Path) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-valid",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    store.add_artifact(
        ...
        kind=REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND,
        label="valid-B1",
        path=".gza/artifacts/valid-b1.txt",
        byte_size=0,
        sha256="2" * 64,
        status="valid",
        exit_status="already_satisfied",
    ...
    assert ctx.consecutive_noop_improves == 0
    assert action["type"] == "improve"
...
def test_unparseable_adjudication_parks_with_specific_attention_reason(tmp_path: Path) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-unparseable",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    adjudication = store.add(
        (
            ...
            f"Dispute source task: {improve.id}\n"
            f"Dispute artifact id: {dispute_artifact.id}\n"
        ),
        task_type="internal",
        based_on=review.id,
        depends_on=impl.id,
    ...
    assert ctx.review_blocker_adjudication_needed is True
    assert action["type"] == "needs_discussion"
    assert action["needs_attention_reason"] == "review-blocker-adjudication-needed"
    assert "review-blocker-adjudication-needed" in action["description"]


def test_stale_completed_adjudication_does_not_suppress_newer_dispute(tmp_path: Path) -> None:
    from gza.runner import REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-fresh-dispute",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    old_improve = _add_completed_improve_for_review(
        store,
        impl,
        review,
        when=datetime(2026, 5, 14, 11, 0, tzinfo=UTC),
    ...
    stale_adjudication = store.add(
        f"Adjudicate blocker B1 from review {review.id} for task {impl.id}: Missing API guard\n\n"
        f"Dispute source task: {old_improve.id}\n"
    ...
    stale_adjudication.status = "completed"
    stale_adjudication.completed_at = datetime(2026, 5, 14, 12, 0, tzinfo=UTC)
    store.update(stale_adjudication)

    new_improve = _add_completed_improve_for_review(
        store,
        impl,
        review,
    ...
    assert old_improve.id is not None
    assert new_improve.id is not None

    store.add_artifact(
        review.id,
        kind=REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND,
        label="disputed-B1-old",
        path=".gza/artifacts/disputed-b1-old.txt",
        byte_size=0,
        sha256="0" * 64,
        status="disputed",
        exit_status="already_satisfied",
        ...
        metadata={
            ...
            "review_task_id": review.id,
            "impl_task_id": impl.id,
            "source_task_id": old_improve.id,
            "source_task_type": "improve",
            "head_sha": "old-sha",
            "finding_id": "B1",
            "reason": "already_satisfied",
            "evidence": "Old dispute evidence.",
            "current_state_citation": "`src/api.py:12-18`",
            "finding_fingerprint": {
                "title": "missing api guard",
                "anchor": "src/api.py:12-18",
        ...
        created_at=old_improve.completed_at,
    )
    store.add_artifact(
        review.id,
        kind=REVIEW_BLOCKER_RESOLUTION_ARTIFACT_KIND,
        label="disputed-B1-new",
        path=".gza/artifacts/disputed-b1-new.txt",
        byte_size=0,
        sha256="1" * 64,
        status="disputed",
        exit_status="already_satisfied",
        ...
        metadata={
            ...
            "review_task_id": review.id,
            "impl_task_id": impl.id,
            "source_task_id": new_improve.id,
            "source_task_type": "improve",
            "head_sha": "new-sha",
            "finding_id": "B1",
            "reason": "already_satisfied",
            "evidence": "New dispute evidence.",
            "current_state_citation": "`src/api.py:12-18`",
            "finding_fingerprint": {
                "title": "missing api guard",
                "anchor": "src/api.py:12-18",
        ...
        created_at=new_improve.completed_at,
    )

    action = evaluate_advance_rules(config, store, _FakeGit(can_merge=True), impl, "main")

    assert action["type"] == "create_review_adjudication"
    assert action["review_blocker_adjudication_candidate"].dispute_artifact.label == "disputed-B1-new"


def test_existing_pending_blocker_adjudication_is_reused(tmp_path: Path) -> None:
    from gza.review_tasks import (
        build_review_blocker_adjudication_prompt,
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-reuse",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    dispute_artifact = _add_review_blocker_resolution_artifact(
        store,
        review=review,
        impl=impl,
        source_task=improve,
    ...
    adjudication = store.add(
        build_review_blocker_adjudication_prompt(
            ...
            build_review_blocker_dispute_metadata(dispute_artifact),
        ),
        task_type="internal",
        based_on=review.id,
        depends_on=impl.id,
    ...
    assert action["type"] == "run_review_adjudication"
    assert action["review_adjudication_task"].id == adjudication.id


def test_stale_pending_adjudication_with_same_source_and_head_is_not_reused_for_newer_dispute(
    tmp_path: Path,
) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-stale-artifact-id",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    first_dispute = _add_review_blocker_resolution_artifact(
        store,
        review=review,
        impl=impl,
        source_task=improve,
    ...
    stale_adjudication = store.add(
        build_review_blocker_adjudication_prompt(
            ...
            ),
            {
                "source_task_id": improve.id,
                "head_sha": "same-sha",
                "disputed_artifact_id": first_dispute.id,
                "reason": "already_satisfied",
                "evidence": "Earlier dispute evidence.",
                "current_state_citation": "src/api.py:12-18",
            },
        ),
        task_type="internal",
    ...
    stale_adjudication.status = "pending"
    store.update(stale_adjudication)

    second_dispute = _add_review_blocker_resolution_artifact(
        store,
        review=review,
        impl=impl,
        source_task=improve,
        finding_id="B1",
        state="disputed",
        created_at=improve.completed_at + timedelta(seconds=1),
        head_sha="same-sha",
        reason="unreproducible",
        evidence="Latest dispute evidence.",
    )
    ...
    assert action["type"] == "create_review_adjudication"
    candidate = action["review_blocker_adjudication_candidate"]
    assert candidate.dispute_artifact.id == second_dispute.id
    assert candidate.dispute_artifact.id != first_dispute.id


def test_replacement_blocker_adjudication_is_run_on_next_advance_pass(tmp_path: Path) -> None:
    from gza.review_tasks import (
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/disputed-blocker-replacement-reroute",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    first_noop = _add_completed_improve_for_review(
        store,
        impl,
        review,
        when=datetime(2026, 5, 14, 11, 0, tzinfo=UTC),
    ...
    _add_review_blocker_resolution_artifact(
        ...
        review=review,
        impl=impl,
        source_task=first_noop,
        finding_id="B1",
        state="disputed",
        created_at=first_noop.completed_at,
    )
    ...
    second_noop = _add_completed_improve_for_review(
        store,
        impl,
        review,
        when=datetime(2026, 5, 14, 13, 0, tzinfo=UTC),
    ...
    second_dispute = _add_review_blocker_resolution_artifact(
        ...
        review=review,
        impl=impl,
        source_task=second_noop,
        finding_id="B1",
        state="disputed",
        created_at=second_noop.completed_at,
        reason="unreproducible",
        evidence="The latest branch state already rejects empty IDs.",
    )

    replacement, created_now = create_or_reuse_review_blocker_adjudication_task(
        store,
        review_task=review,
        impl_task=impl,
        ...
        dispute_metadata={
            "source_task_id": second_noop.id,
            "disputed_artifact_id": second_dispute.id,
            "reason": "unreproducible",
            "evidence": "The latest branch state already rejects empty IDs.",
            "current_state_citation": "src/api.py:12-18",
        },
    ...
    assert action["type"] == "run_review_adjudication"
    assert action["review_adjudication_task"].id == replacement.id
    assert action["review_adjudication_task"].id != stale_adjudication.id


def test_valid_adjudication_reopens_improve_even_when_noop_limit_is_hit(tmp_path: Path) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/adjudication-valid-reopens",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
...
def test_newer_disputed_noop_reopens_adjudication_after_stale_valid_resolution(
    tmp_path: Path,
) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/adjudication-reopens-on-newer-dispute",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )

    review1 = store.add("Review", task_type="review", depends_on=impl.id)
    ...
    assert review.id is not None
    review.status = "completed"
    review.completed_at = datetime(2026, 5, 14, 14, 0, tzinfo=UTC)
    review.output_content = review1.output_content
    store.update(review)
    ...
    adjudication.status = "completed"
    adjudication.completed_at = datetime(2026, 5, 14, 16, 0, tzinfo=UTC)
    store.update(adjudication)
    _add_review_blocker_resolution_artifact(
        store,
        review=review,
        ...
        state="valid",
        created_at=adjudication.completed_at,
        dispute_source_task_id=first_noop.id,
        disputed_artifact_id=first_dispute.id,
        head_sha="same-sha",
        reason="already_satisfied",
        evidence="Earlier dispute evidence.",
    ...
    second_noop = _add_completed_improve_for_review(
        ...
        impl,
        review,
        when=datetime(2026, 5, 14, 17, 0, tzinfo=UTC),
        changed_diff=False,
    )
    second_dispute = _add_review_blocker_resolution_artifact(
        store,
...
def test_newer_disputed_noop_is_not_cleared_by_stale_invalid_resolution_with_same_source_and_head(
    tmp_path: Path,
) -> None:
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/adjudication-does-not-clear-newer-dispute",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
...
def test_verify_only_noop_improve_does_not_enter_code_adjudication_lane(tmp_path: Path) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/noop-verify-only-no-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    ...
    store.add_artifact(
        ...
        metadata={
            ...
            "finding_id": "B1",
            "reason": "already_satisfied",
            "evidence": "The verify lane was already green on the current branch tip.",
            "current_state_citation": "`src/api.py:12-18`",
            "finding_fingerprint": {
                "title": "verify_command failure: flaky unit lane",
                "anchor": "rerun verify_command on the current tip.",
            },
        },
        created_at=improve.completed_at,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "newer_failed_evidence",
            "failed",
            "same-head-sha",
            datetime(2026, 6, 23, 11, 30, tzinfo=UTC),
        ),
        ("newer_missing_evidence", None, None, None),
        (
            "newer_stale_evidence",
            "passed",
            "same-head-sha",
            datetime(2026, 6, 23, 9, 59, 59, tzinfo=UTC),
        ),
        (
            "newer_head_mismatch",
            "passed",
...
def test_verify_failure_only_noop_improve_does_not_clear_from_older_pass_when_newer_noop_does_not_clear(
    tmp_path: Path,
    label: str,
    newer_status: str | None,
    ...
    if newer_status is not None:
        newer_improve.review_verify_status = newer_status
        newer_improve.review_verify_branch = impl.branch
        newer_improve.review_verify_head_sha = newer_head_sha
    if captured_at is not None:
        newer_improve.review_verify_captured_at = captured_at
...
def test_read_only_verify_failure_only_noop_improve_merge_does_not_persist_review_clearance(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5680-read-only-noop-clear",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
...
def test_off_topic_verify_unblock_disabled_keeps_existing_noop_red_behavior(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-off-topic-disabled",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    ...
    with patch(
        ...
        side_effect=AssertionError("disabled policy must not run off-topic baseline"),
    ):
        action = evaluate_advance_rules(config, store, git, impl, "main")
...
def test_off_topic_verify_unblock_clears_review_and_persists_audit_artifact(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-off-topic-clear",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
...
def test_off_topic_verify_unblock_fails_closed_when_investigation_persistence_fails(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-off-topic-investigation-fails",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    ...
    with (
        ...
        patch.object(store, "_add_artifact_conn", side_effect=RuntimeError("artifact write failed")),
    ):
        action = evaluate_advance_rules(config, store, git, impl, "main")
...
def test_off_topic_verify_unblock_baseline_exception_parks_with_warning_and_no_artifacts(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.advance_off_topic_verify_unblock = True
    tree_fingerprint = "4" * 64

    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-baseline-cwd-failure",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    ...
    with patch(
        "gza.off_topic_verify.run_local_target_baseline_plan",
        side_effect=FileNotFoundError("local-target baseline cwd does not exist: /tmp/missing"),
    ):
        action = evaluate_advance_rules(config, store, git, impl, "main")
...
def test_off_topic_verify_unblock_uses_requested_target_branch(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-release-target",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    ...
    git = _FakeGit(
        can_merge=True,
        existing_branches={impl.branch, "main", "release"},
        ref_shas={
            impl.branch: "same-head-sha",
            "main": "main-head-sha",
            "release": "release-head-sha",
        },
        default_branch_name="main",
...
def test_read_only_off_topic_verify_candidate_skips_baseline_and_keeps_review_blocking(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-read-only-off-topic",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    ...
    with patch(
        "gza.off_topic_verify.run_local_target_baseline_plan",
        side_effect=AssertionError("read-only classification must not run a target baseline"),
    ):
        action = determine_next_action(
            config,
            store,
...
def test_off_topic_verify_unblock_blocks_branch_introduced_failures(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-branch-introduced",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 6, 23, 10, 0, tzinfo=UTC))
    review.output_content = _verify_failure_only_review_report()
    ...
    _persist_review_verify_artifact(
        ...
        store=store,
        task=red_improve,
        output=_branch_introduced_failed_verify_output(tree_fingerprint=tree_fingerprint),
        status="failed",
        exit_status="1",
        tree_fingerprint=tree_fingerprint,
    )
    ...
    git = _FakeGit(
        ...
        default_branch_name="main",
        resolved_tree_shas={"main": "f" * 64},
        name_status_by_range={f"main...{impl.branch}": "M\tsrc/gza/cli/git_ops.py"},
    )

    with patch(
        "gza.off_topic_verify.run_local_target_baseline_plan",
        side_effect=AssertionError("branch-introduced failures must not run a target baseline"),
    ):
        action = evaluate_advance_rules(config, store, git, impl, "main")
...
def test_off_topic_verify_unblock_fails_closed_when_classifier_is_unavailable(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/gza-5236-classifier-unavailable",
        when=datetime(2026, 6, 23, 9, 0, tzinfo=UTC),
    )
    review = _add_completed_review(store, impl, when=datetime(2026, 6, 23, 10, 0, tzinfo=UTC))
    review.output_content = _verify_failure_only_review_report()
    ...
    red_improve.review_verify_branch = impl.branch
    red_improve.review_verify_head_sha = "same-head-sha"
    red_improve.review_verify_command = "uv run mypy src"
    red_improve.review_verify_exit_status = "1"
    red_improve.review_verify_captured_at = review.completed_at + timedelta(minutes=3)
    store.update(red_improve)
    _persist_review_verify_artifact(
    ...
    with patch(
        "gza.off_topic_verify.run_local_target_baseline_plan",
        side_effect=AssertionError("unavailable classifier must fail closed before baseline execution"),
    ):
        action = evaluate_advance_rules(config, store, git, impl, "main")
...
def test_verify_blocked_noop_improves_refresh_review_when_review_sha_is_stale(tmp_path: Path) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.verify_command = "uv run pytest tests/ -q"
...
def test_three_consecutive_identical_primary_blockers_returns_needs_attention(
    ...
    review3.review_verify_head_sha = "reviewed-sha"
    store.update(review3)
    review3.output_content = (
        "## Summary\n\n- Found a blocker.\n\n"
        "## Blockers\n\n"
        "### B1 Missing guard\n"
        "Evidence: the current code still misses the guard.\n"
        "Open-state citation: `src/gza/foo.py:10`\n"
        "Impact: invalid input still reaches the unsafe path.\n"
        "Required fix: add the missing guard.\n"
        "Required tests: cover the empty-input case.\n\n"
        "## Follow-Ups\n\nNone.\n\n"
        "## Verdict\n\nVerdict: CHANGES_REQUESTED\n"
    )
    review3.review_verify_head_sha = "reviewed-sha"
    ...
    candidate = action["review_blocker_adjudication_candidate"]
    assert candidate.finding.id == "B1"
    assert candidate.dispute_metadata["reason"] == "repeated_reviewer_request"
    assert classify_advance_action(action) == "actionable"
...
def test_duplicate_blocker_falls_back_to_required_fix_without_citation(
    ...
    candidate = action["review_blocker_adjudication_candidate"]
    assert candidate.finding.id == "B1"
    assert candidate.dispute_metadata["current_state_citation"] == "return early on empty input"


def test_duplicate_blocker_backstop_preempts_noop_improve_limit_for_non_verify_code_blocker(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
...
def test_duplicate_blocker_pending_adjudication_is_run_after_candidate_reuse(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    from gza.review_tasks import create_or_reuse_review_blocker_adjudication_task
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-pending-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(store, impl, review1, when=datetime(2026, 5, 14, 11, 0, tzinfo=UTC))
    ...
    first_action = evaluate_advance_rules(config, store, _FakeGit(can_merge=True), impl, "main")

    assert first_action["type"] == "create_review_adjudication"
    candidate = first_action["review_blocker_adjudication_candidate"]
    adjudication, created_now = create_or_reuse_review_blocker_adjudication_task(
        store,
        review_task=review3,
        impl_task=impl,
...
def test_duplicate_blocker_in_progress_adjudication_is_waited_after_candidate_reuse(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-in-progress-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(store, impl, review1, when=datetime(2026, 5, 14, 11, 0, tzinfo=UTC))
    ...
    assert action["type"] == "wait_review_adjudication"
    assert action["review_adjudication_task"].id == adjudication.id
...
def test_duplicate_blocker_backstop_preempts_max_review_cycles(
    ...
    review4.review_verify_head_sha = "reviewed-sha"
    store.update(review4)
    review4.review_verify_head_sha = "reviewed-sha"
    store.update(review4)
...
def test_duplicate_blocker_streak_creates_review_adjudication_before_manual_park(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.max_noop_improve_cycles = 10

    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(
    ...
    assert action["type"] == "create_review_adjudication"
    candidate = action["review_blocker_adjudication_candidate"]
    assert candidate.dispute_artifact is None
    assert candidate.finding.id == "B1"
    assert candidate.dispute_metadata["reason"] == "repeated_reviewer_request"
    assert review3.id in candidate.dispute_metadata["evidence"]
    assert action.get("needs_attention_reason") is None


def test_duplicate_blocker_without_head_binding_parks_instead_of_creating_adjudication(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-missing-head-binding",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(
...
def test_duplicate_verify_only_blocker_stays_on_verify_only_lane(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-verify-only-blocker",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(
    ...
    reports = {
        review1.id: parse_review_report(_timeout_only_review_report()),
        review2.id: parse_review_report(_timeout_only_review_report()),
        review3.id: parse_review_report(_timeout_only_review_report()),
    }
    monkeypatch.setattr(
        advance_engine_module,
        "get_review_report",
...
def test_invalid_duplicate_blocker_adjudication_clears_review_and_merges(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from gza import advance_engine as advance_engine_module
    from gza.review_tasks import build_review_blocker_adjudication_prompt
    from gza.runner import _persist_review_blocker_adjudication_for_completed_task
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-invalid-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(
    ...
    initial_action = evaluate_advance_rules(config, store, _FakeGit(can_merge=True), impl, "main")
    assert initial_action["type"] == "create_review_adjudication"
    candidate = initial_action["review_blocker_adjudication_candidate"]
    ...
    adjudication.completed_at = datetime(2026, 5, 14, 15, 0, tzinfo=UTC)
    store.update(adjudication)
...
def test_headless_repeated_review_adjudication_does_not_clear_review_or_merge(
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feat/duplicate-blocker-headless-adjudication",
        when=datetime(2026, 5, 14, 9, 0, tzinfo=UTC),
    )
    review1 = _add_completed_review(store, impl, when=datetime(2026, 5, 14, 10, 0, tzinfo=UTC))
    _add_completed_improve_for_review(
    ...
    adjudication = store.add(
        (
            f"Adjudicate blocker B1 from review {review3.id} for task {impl.id}: Missing guard\n\n"
            f"Dispute source task: {impl.id}\n"
            "Dispute reason: repeated_reviewer_request\n"
            "Dispute evidence: The same blocker repeated across review cycles.\n"
            "Current state citation: `src/gza/foo.py:10`\n"
        ),
        task_type="internal",
        based_on=review3.id,
        depends_on=impl.id,
    ...
    [
        "\n".join(
            (
                ...
                "gza-verify phase=passed name=unit duration_seconds=110.0",
...
def test_pre_review_budget_timeout_accepts_final_in_flight_phase_without_verify_fix(
    tmp_path: Path,
    output: str,
) -> None:
    store = _make_store(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-in-flight-budget-timeout",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    result = ReviewVerifyResult(
        command="./bin/tests",
    ...
    aggregate_details = _build_single_project_verify_aggregate_details(result)
    assert aggregate_details is not None
    assert aggregate_details["expected_phase_names"] == ["ruff", "ty", "mypy", "checks", "unit", "functional"]
    persist_verify_gate_artifact(
        store,
        config,
        owner_task=impl,
...
def test_pre_review_cross_project_budget_timeout_accepts_lone_started_phase_without_verify_fix(
    tmp_path: Path,
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-cross-project-in-flight-budget-timeout",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    project_result = ReviewVerifyResult(
        command="./bin/tests",
        status="failed",
        exit_status="timed out",
        captured_at=datetime(2026, 8, 29, 12, 5, tzinfo=UTC),
    ...
    aggregate_details = _build_cross_project_verify_aggregate_details(
        (
            ProjectVerificationResult(
                project=None,
                scope="libs/bar",
    ...
    assert aggregate_details is not None
    assert aggregate_details["phase_results"] == []
    persist_verify_gate_artifact(
        store,
        config,
        owner_task=impl,
            ...
            "scopes": [
                {
                    ...
                    "phase_diagnostics": {
                        "phase_results": [
                            {"name": "unit", "status": "passed", "duration_seconds": 3.25},
                        ],
                        "started_phase_names": ["unit"],
                        "completed_phase_names": ["unit"],
                        "failed_phase_names": ["unit"],
...
def test_pre_review_legacy_scoped_timeout_contradictions_park_invalid_without_verify_fix(
    tmp_path: Path,
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-legacy-scoped-invalid",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    persist_verify_gate_artifact(
        store,
...
@pytest.mark.parametrize(
    "phase_diagnostics_patch",
    [
        {"completed_count": 0},
        {"failed_count": 1},
    ],
)
def test_pre_review_legacy_scoped_timeout_rejects_contradictory_phase_counts(
    tmp_path: Path,
    phase_diagnostics_patch: dict[str, object],
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-legacy-count-invalid",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    phase_diagnostics = {
...
@pytest.mark.parametrize(
    "aggregate_details_patch",
    [
        {"expected_phase_names": ["libs/bar:not-real"]},
        {
            "scopes": [
                {
                    "scope": "libs/bar",
                ...
                },
                {
                    ...
                    "failure_origin": "timeout",
                    "phase_diagnostics": {
                        "phase_results": [{"name": "lint", "status": "passed", "duration_seconds": 1.0}],
                        "started_phase_names": ["lint"],
                        "completed_phase_names": ["lint"],
                        "failed_phase_names": [],
                        "expected_phase_names": ["lint"],
                        "not_started_phase_names": [],
                    },
                },
            ]
...
def test_pre_review_legacy_scoped_timeout_rejects_aggregate_identity_contradictions(
    tmp_path: Path,
    aggregate_details_patch: dict[str, object],
) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-legacy-aggregate-invalid",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    aggregate_details: dict[str, object] = {
        "schema_version": 1,
        "runnable_count": 1,
        "completed_phase_names": ["libs/bar:unit"],
        "failed_phase_names": [],
        "started_phase_names": ["libs/bar:unit"],
        "not_started_phase_names": [],
        "expected_phase_names": ["libs/bar:unit"],
        "scopes": [
            {
                "scope": "libs/bar",
                "status": "failed",
    ...
    aggregate_details.update(aggregate_details_patch)
    persist_verify_gate_artifact(
        store,
        config,
        owner_task=impl,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "gza-verify phase=start name=unit\ngza-verify phase=passed name=unit duration_seconds=3.25\ngza-verify phase=start name=unit",
        "gza-verify phase=start name=ruff\ngza-verify phase=start name=unit\ngza-verify phase=passed name=ruff duration_seconds=3.25",
    ],
)
def test_pre_review_persisted_production_timeout_preserves_ordered_phase_invalidity(
    tmp_path: Path,
    output: str,
) -> None:
    store = _make_store(tmp_path)
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-ordered-output-invalid",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    result = ReviewVerifyResult(
        command="./bin/tests",
...
@pytest.mark.parametrize(
    ("phase_diagnostics_patch", "expected_reason"),
    [
        ({"never_started": ["functional"]}, PARK_REASON_VERIFY_BUDGET_EXCEEDED),
        (
            {"not_started_phase_names": ["integration"], "never_started": ["functional"]},
            PARK_REASON_VERIFY_PHASE_EVIDENCE_INVALID,
        ),
    ],
)
def test_pre_review_legacy_scoped_timeout_normalizes_never_started_alias(
    tmp_path: Path,
    phase_diagnostics_patch: dict[str, object],
    expected_reason: str,
) -> None:
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-legacy-never-started",
        when=datetime(2026, 8, 29, 12, 0, tzinfo=UTC),
    )
    phase_diagnostics = {
        "phase_results": [{"name": "unit", "status": "passed", "duration_seconds": 110.0}],
        "started_phase_names": ["unit"],
        "completed_phase_names": ["unit"],
        "failed_phase_names": [],
    ...
    persist_verify_gate_artifact(
        ...
        aggregate_details={
            ...
            "failed_phase_names": [],
            "started_phase_names": ["libs/bar:unit"],
            "expected_phase_names": ["libs/bar:unit", "libs/bar:functional"],
            "not_started_phase_names": ["libs/bar:functional"],
            "scopes": [
                {
                    "scope": "libs/bar",
                    "status": "failed",
...
def test_pre_review_failed_verify_waits_for_in_progress_verify_fix_with_timeout_drift(tmp_path: Path) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    config.verify_command = "./bin/tests"
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/pre-review-in-progress-verify-fix",
        when=datetime(2026, 7, 6, 12, 0, tzinfo=UTC),
    )
    recorded_epoch = VerifyEpoch(
        reviewed_branch=impl.branch,
...
def test_green_verify_at_current_head_clears_completed_verify_fix_red_gate(tmp_path: Path) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    config.verify_command = "./bin/tests"
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/current-green-clears-red",
        when=datetime(2026, 7, 6, 12, 0, tzinfo=UTC),
    )
    epoch = VerifyEpoch(
        reviewed_branch=impl.branch,
    ...
    assert action["type"] == "merge"
    assert action.get("needs_attention_reason") != "verify-fix-failed"
    assert "verify gate is still red" not in action["description"]


def test_green_verify_clears_completed_verify_fix_red_gate_with_absent_timeout_provenance(tmp_path: Path) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    config.verify_command = "./bin/tests"
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/current-green-absent-timeout",
        when=datetime(2026, 7, 6, 12, 0, tzinfo=UTC),
    )
    epoch = VerifyEpoch(
        reviewed_branch=impl.branch,
...
def test_new_failed_verify_at_current_head_keeps_completed_verify_fix_gate_red(tmp_path: Path) -> None:
    store = _make_store(tmp_path)
    config = Config.load(tmp_path)
    config.require_review_before_merge = False
    config.verify_command = "./bin/tests"
    ...
    impl = _make_completed_unmerged_impl(
        store,
        branch="feature/current-red-stays-red",
        when=datetime(2026, 7, 6, 12, 0, tzinfo=UTC),
    )
    epoch = VerifyEpoch(
        reviewed_branch=impl.branch,
