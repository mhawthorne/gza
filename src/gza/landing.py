...
class LandingCoordinator:
    ...
    def land(self, request: LandRequest) -> LandTerminalResult | LandBlocked:
        ...
        terminal = self._reconcile_unmerged_terminal_boundary(unit, request=request)
        if terminal is not None:
            return terminal
    ...
    def _fresh_terminal_proof_after_write(
        self,
        unit: MergeUnit,
        *,
        expected_proof: TerminalProof,
    ) -> TerminalProof | LandTerminalResult | LandBlocked:
        collaborators = self.collaborators or LandingCollaborators()
        refreshed = self.store.get_merge_unit(unit.id)
        if refreshed is None:
            return LandBlocked(
        ...
        if refreshed.state != expected_proof.state:
            ...
            return LandBlocked(
                ...
                fact=(
                    f"merge unit {unit.id} terminal reconciliation persisted {expected_proof.state} "
                    f"but refreshed as {refreshed.state}"
                ),
                evidence_refs=(unit.id,),
            )
        proof = collaborators.reconcile_terminal_state(self.store, refreshed)
        if isinstance(proof, LandBlocked):
            self._restore_unmerged_after_stale_terminal_write(refreshed)
            return proof
        proof_state = _reconciled_terminal_state(proof)
        if proof_state != expected_proof.state:
            self._restore_unmerged_after_stale_terminal_write(refreshed)
            return LandBlocked(
                reason_code="merge-proof-changed",
                fact=(
                    f"terminal proof for merge unit {unit.id} changed from {expected_proof.state} "
                    f"to {proof_state or 'unmerged'} after persistence"
                ),
                evidence_refs=(unit.id,),
            )
        post_write_proof = _as_terminal_proof(proof, refreshed, proof_state)
        if (
            post_write_proof.source_sha != expected_proof.source_sha
            or post_write_proof.target_sha != expected_proof.target_sha
        ):
            self._restore_unmerged_after_stale_terminal_write(refreshed)
            return LandBlocked(
                reason_code="merge-proof-changed",
                fact=f"live source or target ref for merge unit {unit.id} changed during terminal persistence",
                evidence_refs=(unit.id,),
            )
        return post_write_proof

    def _restore_unmerged_after_stale_terminal_write(self, unit: MergeUnit) -> None:
    ...
    def _reconcile_unmerged_terminal_boundary(
        self,
        unit: MergeUnit,
        *,
        request: LandRequest,
        ...
        if not persisted:
            return _result_for_concurrent_terminal_refresh(unit, self.store.get_merge_unit(unit.id))
        post_write_proof = self._fresh_terminal_proof_after_write(unit, expected_proof=final_proof)
        if isinstance(post_write_proof, LandBlocked) or isinstance(post_write_proof, LandTerminalResult):
            return post_write_proof
        refreshed = self.store.get_merge_unit(unit.id)
        terminal = (
                    ...
                    if isinstance(terminal, LandTerminalResult):
                        steps.append(
                            LandStep(
                                ...
                                "completed",
                                (
                                    "source reconciled to already landed or equivalent on target"
                                    if terminal.outcome == "merged"
                                    else f"source reconciled to terminal no-work state {terminal.outcome}"
                                ),
                                evidence_refs=_evidence_refs(
                                    identity.owner_task_id,
                                    identity.merge_unit_id,
    ...
    def _final_preflight_block(self, identity: LandingResolvedIdentity) -> LandBlocked | None:
        ...
        if git is None:
            return LandBlocked(
                "identity-proof-unavailable",
                "git proof is unavailable for final landing preflight",
        ...
        current_source_sha = _rev_parse_if_exists(git, identity.source_ref)
        current_target_sha = _rev_parse_if_exists(git, identity.target_branch)
        if current_source_sha != identity.source_sha:
            return LandBlocked(
                "identity-proof-unavailable",
...
def create_production_landing_coordinator(
    *,
    config: Any,
    store: SqliteTaskStore,
    git: Git,
    ...
    """Build the production landing coordinator and all side-effect collaborators."""
    ...
    latest_identity: dict[str, LandingResolvedIdentity] = {}
    latest_facts: dict[str, LandingPolicyFacts] = {}
    coordinator_ref: dict[str, LandingCoordinator] = {}

    def runner(config: Any, task_id: str) -> int:
        return runner_mod.run(config, task_id=task_id)

    def inspect_policy_facts(identity: LandingResolvedIdentity) -> LandingPolicyFacts:
    ...
    def durable_judge() -> LandingJudgment:
        ...
        if (
            review_id is None
            or identity.merge_unit_id is None
            or review.reviewed_head is None
            or identity.source_sha is None
            or identity.target_sha is None
        ):
            return LandingJudgment("BLOCK", blocking_fact="landing policy facts are incomplete for guarded judgment")
        ...
        if review_task is None:
            return LandingJudgment("BLOCK", blocking_fact=f"current review {review_id} is unavailable")
        try:
    ...
    def execute_land_merge(
        identity: LandingResolvedIdentity,
        decision: LandingPolicyDecision,
        provenance: Literal["manual_land", "manual_land_escalated"],
    ) -> ManualMergeExecutionResult:
        coordinator = coordinator_ref["coordinator"]
        authorization = refresh_landing_authorization(coordinator, identity, decision, policy=policy)
        if authorization is None:
        ...
        merge_result = _merge_single_task(
            ...
            load_landing_authorization=lambda: refresh_landing_authorization(
                coordinator, identity, decision, policy=policy
            ),
    ...
    def finalize_land_merge(
        identity: LandingResolvedIdentity,
        _decision: LandingPolicyDecision,
        provenance: Literal["manual_land", "manual_land_escalated"],
    ) -> ManualMergeExecutionResult:
        try:
            mark_merge_subject_merged(
...
def run_production_landing(
    *,
    config: Any,
    store: SqliteTaskStore,
    git: Git,
    request: LandRequest,
) -> LandResult | LandTerminalResult:
    """Run production landing through the public coordinator factory."""
    ...
    unit = store.resolve_merge_unit_subject(request.task_id)
    ...
    if pending_finalization or source_contained:
        ...
        if terminal_result.reason_code == "required-review-unavailable":
            coordinator_collaborators = terminal_collaborators
...
def refresh_landing_authorization(
    coordinator: LandingCoordinator,
    identity: LandingResolvedIdentity,
    decision: LandingPolicyDecision,
    *,
    policy: LandingPolicyName,
) -> MergeLandingAuthorization | None:
    """Re-resolve live landing state and rebuild exact merge authorization."""
    ...
    try:
        current_identity = coordinator._resolve_identity(
            LandRequest(task_id=identity.owner_task_id, policy=policy),
    ...
    if decision.allowed_overrides:
        ...
        if review is None or verify is None or not review.review_id:
            return None
        if (
            current_identity.merge_unit_id is None
            or review.reviewed_head is None
            or current_identity.source_sha is None
            or current_identity.target_sha is None
        ):
            return None
        ...
        refreshed_decision = evaluate_landing_policy(policy=policy, facts=facts, judge=None)
    if not refreshed_decision.allowed:
        return None
    return landing_merge_authorization_from_facts(
...
def _landing_judge_evidence(
    ...
    if facts.review is None or facts.verify is None:
        raise ValueError("landing judge evidence requires current review and verify facts")
    ...
    verify_identity = json.dumps(
        {
            "epoch": facts.verify.epoch if facts.verify is not None else None,
            "gate": facts.verify.gate_identity if facts.verify is not None else None,
            "tree": facts.verify.tree_fingerprint if facts.verify is not None else None,
            "verdict": facts.verify.status if facts.verify is not None else None,
        },
        sort_keys=True,
        separators=(",", ":"),
    )
...
def _landing_diff_context(git: Any, identity: LandingResolvedIdentity) -> str:
