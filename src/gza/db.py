...
@dataclass(frozen=True)
class PendingTaskEditResult:
    """Transaction-observed state around an atomic pending-task edit."""
...
MERGE_SOURCE_MANUAL_LAND = "manual_land"
MERGE_SOURCE_MANUAL_LAND_ESCALATED = "manual_land_escalated"
...
class SqliteTaskStore:
    ...
    def update_pending_task_edit(
        ...
        *,
        ...
        transaction. Only explicitly requested editable columns whose current
        ...
        with self._write_transaction() as conn:
            ...
            if row["prompt"] != prompt:
                ...
                changed_fields.add("prompt")
                meaningful_edit = True

            for field, value in updates.items():
                persisted_value = (1 if value else 0) if field in boolean_fields else value
                if row[field] == persisted_value:
                    continue
                assignments.append(f"{field} = ?")
                values.append(persisted_value)
                changed_fields.add(field)
                meaningful_edit = meaningful_edit or field == "task_type"
            ...
            assert updated_row is not None
            after = self._rows_to_tasks(conn, [updated_row])[0]
            return PendingTaskEditResult(
                before=before,
                after=after,
    ...
    def list_recent_artifacts(self, *, kind: str, limit: int = 25) -> list[TaskArtifact]:
        """Return recent artifacts of one kind across this project, newest first."""
        if not self.supports_task_artifacts():
            return []
        if limit < 1:
            return []
        ...
        *,
        ...
        params.extend(
            [
                self._project_id,
                unit_id,
                expected_identity.source_branch,
                expected_identity.target_branch,
                expected_identity.state,
                expected_identity.owner_task_id,
        ...
        with self._write_transaction() as conn:
            cursor = conn.execute(
                ...
                WHERE project_id = ?
                  AND id = ?
                  AND source_branch IS ?
                  AND target_branch IS ?
                  AND state IS ?
