...
class Task:
    ...
    trigger_source: str | None = None  # None/legacy, manual, watch, or auto-recovery task creation source
...
class ParkedTaskRearmState:
    ...
    last_attempt_target_sha: str | None = None
...
_DB_TIMESTAMP_COLUMNS: dict[str, tuple[str, ...]] = {
    ...
    "parked_task_rearms": ("manual_rearmed_at", "last_attempt_at"),
}
...
# Migration from v39 to v40: explicit completion reason for successful terminal tasks
...
# Migration from v44 to v45: plan auto-implement follow-up flag
MIGRATION_V44_TO_V45 = "ALTER TABLE tasks ADD COLUMN auto_implement INTEGER DEFAULT 1;"

# Migration from v45 to v46: task trigger provenance
MIGRATION_V45_TO_V46 = """
...
# Migration from v58 to v59: extend parked-task rearm state for future auto-rearm budgets
MIGRATION_V58_TO_V59 = """
ALTER TABLE parked_task_rearms ADD COLUMN attempt_count INTEGER NOT NULL DEFAULT 0;
ALTER TABLE parked_task_rearms ADD COLUMN last_attempt_at TEXT;
ALTER TABLE parked_task_rearms ADD COLUMN last_attempt_target_sha TEXT;
"""
...
def _validate_auto_migration_target(conn: sqlite3.Connection, target_version: int) -> None:
    ...
    required_columns_by_version: dict[int, tuple[str, str]] = {
        ...
        59: ("parked_task_rearms", "last_attempt_target_sha"),
    }
    ...
    *,
    ...
    if target_version >= 59:
        for column, alter_sql in (
            ...
            if _table_has_column(conn, "parked_task_rearms", column):
                continue
            try:
                conn.execute(alter_sql)
            except sqlite3.OperationalError as exc:
                if _is_readonly_snapshot_operational_error(exc):
                    raise SchemaIntegrityError(
                        ...
                        f"parked_task_rearms.{column}; use a writable database to complete "
                        "migration to v59, then retry."
                    ) from exc
                raise SchemaIntegrityError(
                    "Schema integrity check failed while repairing required column "
                    f"parked_task_rearms.{column}: use a writable database."
                ) from exc
...
class SqliteTaskStore:
    ...
    def supports_merge_units(self) -> bool:
        ...
        if self._supports_merge_units_cache is not None:
            return self._supports_merge_units_cache
        with self._connect() as conn:
            result = _table_exists(conn, "merge_units") and _table_exists(conn, "merge_unit_tasks")
    ...
    def supports_parked_task_rearms(self) -> bool:
        """Return whether durable parked-task rearm storage is available."""
        if self._open_mode == "query_only":
    ...
    def default_merge_target(self, *, strict: bool = False) -> str:
        ...
        """
        if self._default_merge_target_cache is not None:
            return self._default_merge_target_cache
        ...
        if isinstance(project_root, Path):
            ...
            if isinstance(default_branch, str) and default_branch:
                self._default_merge_target_cache = default_branch
    ...
    def _collect_query_only_warnings(self) -> None:
        ...
        if not self._query_only_has_column("tasks", "auto_implement"):
            self._startup_warnings.append(
                "Query-only DB open detected missing optional column tasks.auto_implement; "
                "plan tasks will default to automatic implementation follow-up."
            )
        if not self._query_only_has_column("tasks", "trigger_source"):
            self._startup_warnings.append(
        ...
        if not self._query_only_supports_parked_task_rearms():
            if self._query_only_table_exists.get("parked_task_rearms", False):
                self._startup_warnings.append(
                    ...
                    "parked-task rearm state will be unavailable."
                )
            else:
                self._startup_warnings.append(
                    "Query-only DB open detected missing required table parked_task_rearms; "
    ...
    def record_parked_task_auto_rearm_attempt(
        ...
        *,
        ...
        """Increment and persist future auto-rearm attempt accounting for one parked subject/reason pair."""
        if not self.supports_parked_task_rearms():
            return None
        now = _format_db_timestamp(datetime.now(UTC))
        assert now is not None
        manual_rearmed_at = _format_db_timestamp(datetime.fromtimestamp(0, tz=UTC))
        assert manual_rearmed_at is not None
        with self._connect() as conn:
            conn.execute(
                """
                ...
                DO UPDATE SET
                    subject_task_id = excluded.subject_task_id,
                    attempt_count = parked_task_rearms.attempt_count + 1,
                    last_attempt_at = excluded.last_attempt_at,
                    last_attempt_target_sha = excluded.last_attempt_target_sha
                """,
                (
                    self._project_id,
                    subject_kind,
    ...
    def get_or_create_merge_unit_for_task(self, task: Task) -> MergeUnit | None:
        """Resolve or backfill the merge unit associated with a task.

        Each task has at most one merge unit (single-unit-per-task model). The
        unit is created against the project's default merge target.
        """
        if not self.supports_merge_units():
            return None
        if task.id is None:
    ...
    def get_unmerged(self) -> list[Task]:
        ...
        if self.supports_merge_units():
            units = self._get_unmerged_merge_units_with_legacy_fallback()
            representatives: list[Task] = []
            for unit in units:
                rep = self.resolve_merge_unit_representative_task(unit, require_actionable=True)
                if rep is not None:
    ...
    def _legacy_unmerged_candidates(self) -> list[Task]:
        """Return legacy code-bearing rows that may still need merge-unit backfill."""
        with self._connect() as conn:
            cur = conn.execute(
                """
                SELECT * FROM tasks
    ...
    def get_canonical_unmerged_candidates(self) -> list[Task]:
        ...
        if self.supports_merge_units():
            for unit in self.get_unmerged_merge_units():
                for task in self.list_tasks_for_merge_unit(unit.id):
                    if task.id is None:
                        continue
                    candidate_key = (task.id, task.branch)
        ...
        for task in self._legacy_unmerged_candidates():
            ...
            if candidate_key in seen_candidate_keys:
                continue
            if self.supports_merge_units() and self.resolve_merge_unit_for_task(task.id) is not None:
                continue
            seen_candidate_keys.add(candidate_key)
            candidates.append(task)
        return candidates
    ...
    def get_sync_candidates(
        ...
        *,
        ...
        if self.supports_merge_units():
            ...
            tasks: list[Task] = []
            seen_ids: set[str] = set()
            for unit in self.list_active_merge_units():
            ...
            for task in self._legacy_sync_candidates(recent_days=recent_days, cooldown_seconds=cooldown_seconds):
                if task.id is None or task.id in seen_ids:
                    continue
                if self.resolve_merge_unit_for_task(task.id) is not None:
                    continue
                seen_ids.add(task.id)
                tasks.append(task)
            return sorted(
        ...
        for row in rows:
            ...
            conn.execute(
                ...
                (
                    ...
                    row["auto_implement"] if "auto_implement" in row.keys() else 1,
                    ...
                    row["trigger_source"] if "trigger_source" in row.keys() else None,
