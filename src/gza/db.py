...
# Migration from v52 to v53: durable workflow stuck-state tracking
...
def _validate_auto_migration_target(conn: sqlite3.Connection, target_version: int) -> None:
    ...
    if target_version >= 53:
        if not _table_exists(conn, "workflow_stuck_state"):
            raise RuntimeError(
                "Auto-migration to v53 incomplete: missing required table workflow_stuck_state"
            )
        if not _index_exists(conn, "idx_workflow_stuck_active_reason"):
            raise RuntimeError(
                "Auto-migration to v53 incomplete: missing required index "
...
def _ensure_required_auto_migration_artifacts(
    ...
    *,
    ...
    if target_version >= 53:
        if not _table_exists(conn, "workflow_stuck_state"):
            try:
                conn.executescript(MIGRATION_V52_TO_V53)
            except sqlite3.OperationalError as exc:
                if _is_readonly_snapshot_operational_error(exc):
                    raise SchemaIntegrityError(
                        "Query-only DB open detected missing required table workflow_stuck_state; "
                        "use a writable database to complete migration to v53, then retry."
                    ) from exc
                raise SchemaIntegrityError(
                    "Schema integrity check failed while repairing required table "
