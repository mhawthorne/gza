...
def _is_strict_finite_number(value: object) -> bool:
    """Return True for real finite int/float scalars (exclude booleans)."""
...
@dataclass
class WatchParkedAutoRearmConfig:
    """Configuration for bounded watch-owned parked-task auto rearm."""
...
class WatchConfig:
    ...
    parked_auto_rearm: WatchParkedAutoRearmConfig = field(default_factory=WatchParkedAutoRearmConfig)
...
class Config:
    ...
    def _build_config_from_merged_data(
        ...
        *,
        ...
        if not isinstance(auto_rearm_enabled, bool):
            raise ConfigError("'watch.parked_auto_rearm.enabled' must be a boolean (true/false)")
        auto_rearm_budget = _validate_positive_int_field(
            watch_parked_auto_rearm_data.get(
                "budget",
                DEFAULT_WATCH_PARKED_AUTO_REARM_BUDGET,
            ),
            "watch.parked_auto_rearm.budget",
        )
        if auto_rearm_budget is None:
            raise ConfigError("'watch.parked_auto_rearm.budget' must be a positive integer")
        auto_rearm_cooldown_hours = _validate_positive_int_field(
            watch_parked_auto_rearm_data.get(
                "cooldown_hours",
        ...
        watch_config = WatchConfig(
            ...
            parked_auto_rearm=WatchParkedAutoRearmConfig(
                enabled=auto_rearm_enabled,
                budget=auto_rearm_budget,
                cooldown_hours=auto_rearm_cooldown_hours,
                require_target_advanced=auto_rearm_require_target_advanced,
            ),
        )
    ...
    def validate(cls, project_dir: Path) -> tuple[bool, list[str], list[str]]:
            ...
            if not isinstance(watch_data, dict):
                ...
                if "parked_auto_rearm" in watch_data:
                    ...
                    if not isinstance(parked_auto_rearm, dict):
                        ...
                        if "enabled" in parked_auto_rearm and not isinstance(
                            parked_auto_rearm["enabled"],
                            bool,
                        ):
                            errors.append(
                                "'watch.parked_auto_rearm.enabled' must be a boolean (true/false)"
                            )
        ...
        if "review_verify_timeout_grace_seconds" in data:
            ...
            elif not _is_strict_finite_number(data["review_verify_timeout_grace_seconds"]):
