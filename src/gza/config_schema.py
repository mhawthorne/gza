...
CONFIG_KEY_REGISTRY: tuple[ConfigKeySpec, ...] = (
    ...
    ConfigKeySpec(
        ...
        "bool",
        False,
        "Enable durable watch-owned parked-task auto-rearm policy wiring; current releases expose only config/state groundwork and do not change watch behavior yet.",
    ),
    ConfigKeySpec(
        "watch.parked_auto_rearm.budget",
        "int",
        2,
        "Per parked subject/reason auto-rearm attempt budget reserved for future watch-owned rearm policy; no attempts are spent yet.",
    ),
    ConfigKeySpec(
        "watch.parked_auto_rearm.cooldown_hours",
        "int",
        12,
        "Cooldown window in hours reserved for future watch-owned parked-task auto rearm; current releases store the setting without changing watch behavior yet.",
    ),
    ConfigKeySpec(
        "watch.parked_auto_rearm.require_target_advanced",
        "bool",
        True,
        "Require target-branch advancement before future watch-owned parked-task auto rearm is eligible; current releases expose the key without changing watch behavior yet.",
    ),
