...
CONFIG_KEY_REGISTRY: tuple[ConfigKeySpec, ...] = (
    ...
    ConfigKeySpec("watch.parked_auto_rearm.enabled", "bool", False, "Enable the watch-owned parked auto-rearm phase for eligible parked owner/reason pairs."),
    ConfigKeySpec("watch.parked_auto_rearm.budget", "int", 2, "Maximum parked auto-rearm attempts `gza watch` may spend per parked subject/reason pair before leaving it parked."),
    ConfigKeySpec("watch.parked_auto_rearm.cooldown_hours", "int", 12, "Cooldown window between parked auto-rearm attempts for the same subject/reason pair."),
    ConfigKeySpec("watch.parked_auto_rearm.require_target_advanced", "bool", True, "Require the target branch SHA to advance since the last parked auto-rearm attempt before watch spends another attempt for the same parked subject/reason pair."),
