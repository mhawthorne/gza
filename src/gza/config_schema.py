...
CONFIG_KEY_REGISTRY: tuple[ConfigKeySpec, ...] = (
    ...
    ConfigKeySpec("behavior_monitor.enabled", "bool", True, "Enable the host-side behavior conformance monitor command by default."),
    ConfigKeySpec("behavior_monitor.interval_seconds", "int", 14400, "Default poll interval for looping `gza behavior-monitor` runs."),
    ConfigKeySpec("behavior_monitor.tag", "str", "behavior-conformance", "Default filing tag applied to monitor-created follow-up tasks."),
    ConfigKeySpec("behavior_monitor.max_new_tasks_per_cycle", "int", 5, "Per-pass cap on newly filed behavior-conformance follow-up tasks."),
    ConfigKeySpec("behavior_monitor.check_timeout_seconds", "int", 3600, "Timeout budget for one behavior-check monitor pass."),
    ConfigKeySpec("behavior_monitor.file_undetermined", "bool", False, "Whether the monitor files follow-up investigation tasks for UNDETERMINED results."),
    ...
    ConfigKeySpec(
        ...
        "str",
        "merge_and_defer",
        "Policy for completed review/improve loops at max_review_cycles: `merge_and_defer` merges current green capped reviews and creates mandatory blocker follow-ups; `park` is the rollback switch for manual-attention stop.",
    ),
    ...
    ConfigKeySpec("provider", "str", None, "Required provider for every project config.", required=True),
    ...
    ConfigKeySpec("watch.failure_backoff_max", "int", 3600, "Maximum cooldown for consecutive non-auto-resumable failures from one watch owner unit."),
    ConfigKeySpec(
    ...
    ConfigKeySpec(
        ...
        "int | null",
        10,
        "Exit or hold `gza watch` after this many distinct owner units retain non-auto-resumable failure backoff.",
    ),
    ...
    ConfigKeySpec("watch.heartbeat_interval_seconds", "int", 60, "Cadence for watch `BUSY` heartbeat lines during long-running verify, merge, rebase, agent execution, and in-process cycle phases."),
    ConfigKeySpec(
