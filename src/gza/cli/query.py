    ...
    if task.task_type == "review":
        ...
        if task.review_verify_status or task.review_verify_markdown or task.review_verify_artifact_file or latest_verify_artifact:
            console.print(
                ...
                f"[{c['value']}]{_escaped(task.review_verify_status or 'unknown')}[/{c['value']}]"
            )
            ...
            if task.review_verify_branch:
                console.print(
                    ...
                    f"[{c['value']}]{_escaped(task.review_verify_branch)}[/{c['value']}]"
                )
            if task.review_verify_head_sha:
                console.print(
                    f"[{c['label']}]Review Verify Head:[/{c['label']}] "
                    f"[{c['value']}]{_escaped(task.review_verify_head_sha)}[/{c['value']}]"
                )
            if task.review_verify_base_sha:
                console.print(
                    f"[{c['label']}]Review Verify Base:[/{c['label']}] "
                    f"[{c['value']}]{_escaped(task.review_verify_base_sha)}[/{c['value']}]"
                )
            if task.review_verify_cwd:
                console.print(
                    f"[{c['label']}]Review Verify Cwd:[/{c['label']}] "
                    f"[{c['value']}]{_escaped(task.review_verify_cwd)}[/{c['value']}]"
                )
            ...
            if verify_artifact_path:
                ...
                console.print(
                    ...
                    f"[{c['value']}]{_escaped(verify_artifact_text)}[/{c['value']}]"
                )
            if task.review_verify_failure:
                console.print(
                    f"[{c['label']}]Review Verify Failure:[/{c['label']}] "
                    f"[{c['value']}]{_escaped(task.review_verify_failure)}[/{c['value']}]"
                )
