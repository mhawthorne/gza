...
import time
import uuid
from collections.abc import Callable, Hashable, Iterable, Iterator, Mapping, Sequence, Set as AbstractSet
...
from ..main_verify_format import (
...
from ..watch_strategies import WATCH_DISPATCH_STRATEGY_REGISTRY, WatchDispatchStrategy, create_watch_dispatch_strategy
...
def _main_verify_remediation_prompt(
    ...
    *,
    ...
    spent_attempts: int | None = None,
    attempt_limit: int = MAIN_VERIFY_REMEDIATION_ATTEMPT_LIMIT,
) -> str:
    ...
    body = [
        ...
        _render_main_verify_remediation_attempts_line(
...
def _render_main_verify_remediation_attempts_line(*, attempts_spent: int, attempt_limit: int) -> str:
    bounded_spent = max(0, min(attempts_spent, attempt_limit))
    return f"{_MAIN_VERIFY_REMEDIATION_ATTEMPTS_PREFIX}{bounded_spent}/{attempt_limit}"


def _describe_main_verify_environment_identity(
    identity: MainIntegrationVerifyEnvironmentIdentity | None,
) -> str | None:
    if identity is None:
        return None
    description = f"{identity.runner_class}/{identity.platform_system}/{identity.platform_machine}"
    python_bits: list[str] = []
    if identity.python_implementation:
        python_bits.append(identity.python_implementation)
    if identity.python_version:
        python_bits.append(identity.python_version)
    if python_bits:
...
def _parse_main_verify_remediation_attempts_line(prompt: str) -> tuple[int, int] | None:
    for line in prompt.splitlines():
        if not line.startswith(_MAIN_VERIFY_REMEDIATION_ATTEMPTS_PREFIX):
            continue
        raw_value = line.removeprefix(_MAIN_VERIFY_REMEDIATION_ATTEMPTS_PREFIX).strip()
        spent_raw, sep, limit_raw = raw_value.partition("/")
        if sep != "/":
            return None
        try:
            spent = int(spent_raw)
...
def _main_verify_remediation_spent_attempts(
    task: DbTask,
    *,
    attempt_limit: int,
    attempt_state: MainVerifyRemediationAttemptState | None = None,
) -> int:
    durable_spent = 0
    if attempt_state is not None:
        durable_spent = max(0, min(attempt_state.consumed_attempt_count, attempt_limit))
    parsed = _parse_main_verify_remediation_attempts_line(task.prompt)
    if parsed is not None:
        spent, _ = parsed
        return max(min(spent, attempt_limit), durable_spent)
    if task.status == "failed":
        return max(min(1, attempt_limit), durable_spent)
    return durable_spent


def _main_verify_remediation_attempt_state_exhausted(
    attempt_state: MainVerifyRemediationAttemptState | None,
    *,
    attempt_limit: int,
) -> bool:
...
def _main_verify_remediation_attempts_from_prompt(
    prompt: str,
    *,
    attempt_limit: int,
) -> int | None:
    parsed = _parse_main_verify_remediation_attempts_line(prompt)
    if parsed is None:
        return None
    spent, _ = parsed
    return max(0, min(spent, attempt_limit))


def _main_verify_remediation_attempt_limit(config: Config) -> int:
    configured_limit = getattr(
        config.watch, "main_verify_remediation_max_attempts", MAIN_VERIFY_REMEDIATION_ATTEMPT_LIMIT
    )
...
def _main_verify_remediation_terminal_epoch_consumption_key(task: DbTask) -> str:
    ...
    if task.started_at is None:
        return f"{task.id}:terminal-epoch:legacy-no-started-at"
    evidence = {
        "id": task.id,
    ...
    return f"{task.id}:terminal-epoch:{digest}"
...
def _main_verify_remediation_task_matches_identity(
    ...
    return existing_identity is not None and _main_verify_remediation_identity_matches(existing_identity, identity)


def _main_verify_remediation_active_owner(
    *,
    store: SqliteTaskStore,
    identity: _MainVerifyRemediationIdentity,
    ...
    clear_invalid: bool,
) -> tuple[DbTask | None, MainVerifyRemediationAttemptState | None]:
    attempt_state = store.get_main_verify_remediation_attempt_state(
        signature=identity.signature,
        tree_fingerprint=ledger_fingerprint,
    )
    if attempt_state is None or attempt_state.active_task_id is None:
        return None, attempt_state
    active = store.get(attempt_state.active_task_id)
    if active is not None and _main_verify_remediation_task_matches_identity(active, identity):
        return active, attempt_state
    if clear_invalid:
        attempt_state = store.clear_main_verify_remediation_active_task(
            signature=identity.signature,
            tree_fingerprint=ledger_fingerprint,
            last_observed_head_sha=head_sha,
            last_observed_failure=failure,
        )
    return None, attempt_state


def _main_verify_remediation_effective_merge_state(store: SqliteTaskStore, task: DbTask) -> str | None:
    if task.id is None:
...
def _find_open_main_verify_remediation_tasks(
    ...
    *,
    ...
    return _select_legacy_open_main_verify_remediation_tasks(
        store,
        identity=identity,
...
def _retire_moot_main_verify_remediation_tasks(
    *,
    store: SqliteTaskStore,
    signature: str,
    reason: str,
...
def _drop_main_verify_remediation_task(
    *,
    store: SqliteTaskStore,
    task_id: str | None,
    reason: str,
...
def _retire_greenlit_main_verify_remediation_active_tasks(
    *,
    ...
    for attempt_state in store.list_main_verify_remediation_attempt_states():
        if attempt_state.active_task_id is None or attempt_state.greenlit_while_in_progress_task_id is None:
            continue
        task = store.get(attempt_state.active_task_id)
        ...
        if task is None:
            ...
            identity = _main_verify_remediation_identity_from_prompt(task.prompt)
            if (
                attempt_state.greenlit_while_in_progress_task_id != attempt_state.active_task_id
                or identity is None
                or identity.signature != attempt_state.signature
            ):
                should_clear_active = True
        ...
        if should_clear_active:
            store.clear_main_verify_remediation_active_task(
                ...
                tree_fingerprint=attempt_state.tree_fingerprint,
                last_observed_head_sha=attempt_state.last_observed_head_sha,
                last_observed_failure=attempt_state.last_observed_failure,
            )
            cleared.append(attempt_state.signature)
    if retired:
        log.emit(
            "REMEDY",
            "retired green-cleared active main-verify remediation rows: " + ", ".join(retired),
        )
    elif cleared:
        log.emit(
            "REMEDY",
...
def _preview_greenlit_main_verify_remediation_active_tasks(
    *,
    ...
    for attempt_state in store.list_main_verify_remediation_attempt_states():
        ...
        if task.status == "in_progress":
            continue
        if _main_verify_remediation_task_is_reusable(store, task):
            assert task.id is not None
            retired.append(task.id)
        cleared.append(attempt_state.signature)
    if retired:
        log.emit(
            "REMEDY",
            "dry-run: would retire green-cleared active main-verify remediation rows: " + ", ".join(retired),
        )
    elif cleared:
        log.emit(
            "REMEDY",
            "dry-run: would clear stale active main-verify remediation state for: " + ", ".join(cleared),
        )


def _mark_greenlit_main_verify_remediation_in_progress_tasks(
    *,
    store: SqliteTaskStore,
    signature: str,
    head_sha: str | None,
    ...
    marked: list[str] = []
    for attempt_state in store.list_main_verify_remediation_attempt_states():
        if attempt_state.signature != signature or attempt_state.active_task_id is None:
            continue
        task = store.get(attempt_state.active_task_id)
...
def _clear_signature_scoped_main_verify_remediation_active_tasks(
    *,
    store: SqliteTaskStore,
    signature: str,
) -> tuple[str, ...]:
...
def _preview_signature_scoped_main_verify_remediation_cleanup(
    *,
    store: SqliteTaskStore,
    log: "_WatchLog",
    signature: str,
    ...
    should_clear_active = False
    for attempt_state in store.list_main_verify_remediation_attempt_states():
        if attempt_state.signature != signature:
            continue
        if attempt_state.active_task_id is not None:
            should_clear_active = True
            task = store.get(attempt_state.active_task_id)
            if task is not None and task.status == "in_progress":
                identity = _main_verify_remediation_identity_from_prompt(task.prompt)
                if identity is not None and identity.signature == signature and task.id is not None:
                    marked.append(task.id)
        task = store.get(attempt_state.active_task_id) if attempt_state.active_task_id is not None else None
        if task is not None and task.status == "in_progress":
            continue
    for task in store.get_all():
        if not _main_verify_remediation_task_is_reusable(store, task):
    ...
    if marked:
        log.emit(
            "REMEDY",
            f"dry-run: would mark greenlit in-progress main-verify remediation rows for {signature}: "
            + ", ".join(marked),
        )
    if retired:
        log.emit(
            "REMEDY",
            f"dry-run: would retire moot main-verify remediation rows for {signature}: " + ", ".join(retired),
        )
    elif should_clear_active:
        log.emit(
            "REMEDY",
            f"dry-run: would clear active main-verify remediation state for {signature}",
        )
...
def _queue_main_verify_remediation_task(
    *,
    ...
    spent_attempts = _main_verify_remediation_spent_attempts(
        task,
        attempt_limit=attempt_limit,
        attempt_state=attempt_state,
    )
    if task.status in {"failed", "completed", "unmerged", "dropped"}:
        desired_prompt = _main_verify_remediation_prompt(
            remediation,
            head_sha=head_sha,
            attempts_spent=min(spent_attempts + 1, attempt_limit),
            attempt_limit=attempt_limit,
        )
        route_probe = copy.copy(task)
        route_probe.tags = desired_tags
        route_probe.prompt = desired_prompt
        require_execution_route_for_task(route_probe, config)
        spent_attempts = min(spent_attempts + 1, attempt_limit)
        ...
        if attempt_state is not None:
            spent_attempts = min(attempt_state.consumed_attempt_count, attempt_limit)
    ...
    if task.prompt != desired_prompt:
        task.prompt = desired_prompt
        store.update(task)
    elif task.status != "failed" and previous_tags != desired_tags:
        store.update(task)
...
def _preflight_main_verify_remediation_route(
    *,
    ...
    desired_prompt = _main_verify_remediation_prompt(
        remediation,
        head_sha=head_sha,
        attempts_spent=min(attempts_spent, attempt_limit),
        attempt_limit=attempt_limit,
    )
    if task is None:
        route_probe = DbTask(
        ...
        route_probe = copy.copy(task)
        route_probe.task_type = "implement"
        route_probe.prompt = desired_prompt
        route_probe.tags = desired_tags
        route_probe.trigger_source = MAIN_INTEGRATION_VERIFY_REMEDIATION_TRIGGER_SOURCE
    require_execution_route_for_task(route_probe, config)


def _mark_main_verify_remediation_task_exhausted(
    *,
    config: Config,
    store: SqliteTaskStore,
    task: DbTask,
...
def _consume_merged_main_verify_remediation_attempt(
    *,
    config: Config,
    store: SqliteTaskStore,
    log: "_WatchLog",
    ...
    if _main_verify_remediation_attempt_state_exhausted(attempt_state, attempt_limit=attempt_budget):
        _mark_main_verify_remediation_task_exhausted(
            ...
            desired_tags=_merge_main_verify_remediation_tags([tuple(task.tags or ())], None),
            spent_attempts=consumed_count,
...
def _ensure_main_verify_remediation_task(
    *,
    ...
    active_owner, attempt_state = _main_verify_remediation_active_owner(
        store=store,
        identity=identity,
        ledger_fingerprint=ledger_fingerprint,
    ...
    if (
        ...
        and _main_verify_remediation_task_is_effectively_merged(store, active_owner)
    ):
        spent_attempts = _main_verify_remediation_spent_attempts(
            active_owner,
            attempt_limit=attempt_limit,
        ...
        _preflight_main_verify_remediation_route(
            ...
            remediation=remediation,
            head_sha=state.head_sha,
            desired_tags=_merge_main_verify_remediation_tags([tuple(active_owner.tags or ())], tags),
    ...
    if (
        ...
        and not _main_verify_remediation_task_is_effectively_merged(store, active_owner)
    ):
        spent_attempts = _main_verify_remediation_spent_attempts(
            active_owner,
            attempt_limit=attempt_limit,
        ...
        attempt_state = _consume_terminal_active_main_verify_remediation_without_requeue(
            store=store,
            task=active_owner,
            remediation=remediation,
            state=state,
        ...
        if attempt_state is None:
            ...
            log.emit(
                "WARN",
                f"{active_owner.id}: could not record terminal {remediation.kind} remediation consumption for "
        ...
        log.emit(
            "REMEDY",
            f"{active_owner.id}: consumed terminal {remediation.kind} remediation attempt for {phase} on "
    ...
    if _main_verify_remediation_attempt_state_exhausted(attempt_state, attempt_limit=attempt_limit):
        _preflight_main_verify_remediation_route(
            ...
            remediation=remediation,
            head_sha=state.head_sha,
            desired_tags=_main_verify_remediation_tags(tags),
    ...
    _preflight_main_verify_remediation_route(
        ...
        head_sha=state.head_sha,
        desired_tags=_main_verify_remediation_tags(tags),
        attempts_spent=0 if attempt_state is None else min(attempt_state.consumed_attempt_count, attempt_limit),
        attempt_limit=attempt_limit,
    )
    task = store.add(
...
def _retire_moot_main_verify_remediations(
    *,
    store: SqliteTaskStore,
    signature: str,
    reason: str,
...
def _apply_main_verify_green_cleanup(
    *,
    store: SqliteTaskStore,
    log: "_WatchLog",
...
def _maybe_file_main_verify_remediation(
    *,
    ...
    if result.outcome == "reused_live":
        log.emit(
            "REMEDY",
            f"{result.task.id}: reused live {remediation.kind} remediation for {phase}; "
            "left existing in_progress row untouched and deferred queue changes",
        )
    elif result.outcome == "merge_ready":
        log.emit(
            "REMEDY",
            f"{result.task.id}: reused merge-ready {remediation.kind} remediation for {phase}; "
            "left completed/unmerged row available for lifecycle merge",
        )
    else:
        log.emit(
            "REMEDY",
...
def _persist_main_verify_remediation_exhausted_attention(
    *,
    ...
    message = (
        f"{base_message}; automatic remediation exhausted after {attempt_limit}/{attempt_limit} "
        f"attempts for {signature_label} on {fingerprint_label}; remediation {exhausted_task_id}; "
        f"signature {remediation.signature}; "
    ...
    log.emit(
        "REMEDY",
        f"automatic remediation exhausted after {attempt_limit}/{attempt_limit} attempts for "
        f"{signature_label} on {fingerprint_label}",
    )
    return persisted_state


def _consume_unmerged_main_verify_remediation_attempt(
    *,
    config: Config,
    store: SqliteTaskStore,
    log: "_WatchLog",
    ...
    reason: str,
    active_task_id: str | None = None,
) -> _MainVerifyRemediationConsumeResult:
    if task.id is None:
        return _MainVerifyRemediationConsumeResult(consumed=False)
    fresh = store.get(task.id)
    if fresh is None:
        return _MainVerifyRemediationConsumeResult(consumed=False)
    if fresh.status not in {"completed", "unmerged", "dropped"}:
        return _MainVerifyRemediationConsumeResult(consumed=False)
    if remediation.kind != "fix":
        return _MainVerifyRemediationConsumeResult(consumed=False)
    if not _is_active_main_verify_fix_remediation_merge(
        fresh,
        remediation,
        active_task_id=active_task_id,
    ):
...
def _consume_terminal_active_main_verify_remediation_without_requeue(
    *,
    store: SqliteTaskStore,
    task: DbTask,
    remediation: MainIntegrationVerifyRemediation,
    ...
    if not _main_verify_remediation_task_matches_identity(task, identity):
        return None
    if task.status not in {"completed", "unmerged", "dropped"}:
        return None
    return store.record_main_verify_remediation_consumed_attempt(
        signature=identity.signature,
        tree_fingerprint=_main_verify_remediation_ledger_fingerprint(identity.tree_fingerprint),
        task_id=task.id,
        consumption_key=_main_verify_remediation_terminal_epoch_consumption_key(task),
    ...
    *,
    ...
    if check.merges_halted and current_remediation is not None and same_identity:
        ...
        log.emit(
            ...
            f"consumed remediation attempt {consumed_count}/{attempt_budget}",
        )
        return
...
def _retire_cleared_main_verify_remediations(
    *,
    store: SqliteTaskStore,
    log: "_WatchLog",
    check: MainIntegrationVerifyCheck,
    ...
    if not isinstance(signature, str) or not signature:
        if dry_run:
            _preview_greenlit_main_verify_remediation_active_tasks(
                store=store,
                log=log,
            )
            return False
    ...
    retired = _retire_moot_main_verify_remediation_tasks(
        store=store,
        signature=signature,
        reason=MAIN_VERIFY_REMEDIATION_MOOT_GREEN_REASON,
    )
    cleared = _clear_signature_scoped_main_verify_remediation_active_tasks(
        store=store,
        signature=signature,
    )
    if marked:
        log.emit(
            "REMEDY",
            f"marked greenlit in-progress main-verify remediation rows for {signature}: {', '.join(marked)}",
        )
    if retired:
        log.emit(
            "REMEDY",
            f"retired moot main-verify remediation rows for {signature}: {', '.join(retired)}",
        )
    elif cleared:
        log.emit(
            "REMEDY",
...
def _resolve_watch_attention_display_task(store: SqliteTaskStore, row: LineageOwnerRow) -> DbTask:
    ...
    fallback_task = (
        ...
        or row.owner_task
        or _resolve_incomplete_owner_task(store, cast(Any, row))
    )
...
def _watch_reexec_argv(args: argparse.Namespace) -> list[str]:
    ...
    if invocation.watch_state_dir is not None:
        argv.extend(["--watch-state-dir", invocation.watch_state_dir])
    ...
    watch_strategy_state = getattr(args, "watch_strategy_state", None)
    if isinstance(watch_strategy_state, str) and watch_strategy_state:
        argv.extend(["--watch-strategy-state", watch_strategy_state])
...
def _serialize_watch_reexec_strategy_state(
    *,
    recovery_strategy: WatchDispatchStrategy[Any],
    pending_strategy: WatchDispatchStrategy[Any],
) -> str:
    return json.dumps(
        {
            "version": 1,
            "recovery": recovery_strategy.serialize_state(),
            "pending": pending_strategy.serialize_state(),
        },
        sort_keys=True,
        separators=(",", ":"),
    )
...
def _watch_reexec_strategy_states_from_args(
    args: argparse.Namespace,
) -> tuple[Mapping[str, object] | None, Mapping[str, object] | None]:
    raw_state = getattr(args, "watch_strategy_state", None)
    if raw_state is None or raw_state == "":
        return None, None
    if not isinstance(raw_state, str):
        raise ValueError("--watch-strategy-state must be a JSON object")
    try:
        decoded = json.loads(raw_state)
    except json.JSONDecodeError as exc:
        raise ValueError(f"--watch-strategy-state is not valid JSON: {exc}") from exc
    if not isinstance(decoded, Mapping):
        raise ValueError("--watch-strategy-state must decode to a JSON object")
    version = decoded.get("version")
    if type(version) is not int or version != 1:
        raise ValueError("--watch-strategy-state field 'version' must be 1")
    recovery_state = decoded.get("recovery")
    pending_state = decoded.get("pending")
    if not isinstance(recovery_state, Mapping):
        raise ValueError("--watch-strategy-state field 'recovery' must be an object")
    if not isinstance(pending_state, Mapping):
        raise ValueError("--watch-strategy-state field 'pending' must be an object")
    return cast(Mapping[str, object], recovery_state), cast(Mapping[str, object], pending_state)
...
def format_red_duration(red_since: datetime, now: datetime) -> str:
    return _shared_format_red_duration(red_since, now)
...
def _emit_main_verify_attention(
    *,
    ...
    if main_verify_state_is_freshness_unavailable(state):
        ...
        if head_status != "current":
            ...
            return
    if main_verify_state_is_red_verdict(state):
        head_status = resolve_main_verify_target_proof(state, git=git, target_branch=target_branch)
...
def _candidate_rework_prompt(
    ...
    *,
    ...
    if batch_context is not None:
        batch_context_lines.append(f"Batch position: {batch_context.failing_position} of {batch_context.batch_size}")
        if batch_context.green_prefix:
            green_prefix_summary = ", ".join(
                (f"{owner_id} ({branch})" if branch else owner_id) for owner_id, branch in batch_context.green_prefix
            )
            batch_context_lines.append(f"Verified green prefix before this failure: {green_prefix_summary}")
    batch_context_block = ""
        ...
        for staged_entry in staged_batch:
            ...
            if staged_entry.merge_event is not None:
                ...
                if (
                    ...
                    log.emit(
                        "MERGE",
                        f"{staged_entry.merge_event.display_task_id} -> {staged_entry.merge_event.target_branch}",
                    )
...
class _WatchLog:
    def __init__(
        ...
        *,
        ...
        self._aggregate_sink_failed = False
        self._project_sink_failed = False
        self._project_sink_warning_emitted = False
        if aggregate_path is not None:
            try:
                aggregate_path.parent.mkdir(parents=True, exist_ok=True)
                aggregate_path.touch(exist_ok=True)
            except OSError as exc:
                self._aggregate_sink_failed = True
                console.print(
                    f"WARN      {console_prefix}aggregate watch log sink disabled for {aggregate_path}: {exc}",
                    soft_wrap=True,
                    highlight=False,
                    markup=False,
                )
    ...
    def transfer_state_from(self, previous: "_WatchLog") -> None:
        with self._lock, previous._lock:
            self._has_emitted_cycle = previous._has_emitted_cycle
            self._skip_keys_prev_cycle = set(previous._skip_keys_prev_cycle)
            self._sticky_attention_prev_cycle = dict(previous._sticky_attention_prev_cycle)

    def begin_cycle(self) -> None:
        with self._lock:
            if self._has_emitted_cycle:
                self._write_project_text("\n")
                if not self.quiet:
                    console.print()
    ...
    def emit(self, event: str, message: str, *, dedupe_key: str | None = None) -> None:
        with self._lock:
            ...
            self._write_project_text(line + "\n")
            self._write_aggregate_line(line)
            if not self.quiet:
                console.print(
                    _render_watch_stdout(_watch_log_line_with_message_prefix(line, self.console_prefix)),
                    soft_wrap=True,
                    highlight=False,
                )

    def _write_project_text(self, text: str) -> None:
        if self._project_sink_failed:
            return
        try:
            with open(self.path, "a") as f:
                f.write(text)
        except OSError as exc:
            self._project_sink_failed = True
            self._emit_project_sink_warning(exc)

    def _write_aggregate_line(self, line: str) -> None:
        if self.aggregate_path is None or self._aggregate_sink_failed:
            return
        try:
            aggregate_line = _watch_log_line_with_message_prefix(line, self.console_prefix)
            with open(self.aggregate_path, "a") as f:
                f.write(aggregate_line + "\n")
        except OSError as exc:
            self._aggregate_sink_failed = True
            console.print(
                f"WARN      {self.console_prefix}aggregate watch log sink disabled for {self.aggregate_path}: {exc}",
                soft_wrap=True,
                highlight=False,
                markup=False,
            )

    def _emit_project_sink_warning(self, exc: OSError) -> None:
        if self._project_sink_warning_emitted:
            return
        self._project_sink_warning_emitted = True
        message = f"WARN      {self.console_prefix}watch log sink disabled for {self.path}: {exc}"
        if self.aggregate_path is not None and not self._aggregate_sink_failed:
            try:
                with open(self.aggregate_path, "a") as f:
                    f.write(message + "\n")
            except OSError as aggregate_exc:
                self._aggregate_sink_failed = True
                console.print(
                    f"WARN      {self.console_prefix}aggregate watch log sink disabled for "
                    f"{self.aggregate_path}: {aggregate_exc}",
                    soft_wrap=True,
                    highlight=False,
                    markup=False,
                )
...
def _watch_log_line_with_message_prefix(line: str, prefix: str) -> str:
    if not prefix:
        return line
    event_prefix_width = len(f"{_format_hms()} {'X' * _WATCH_EVENT_LABEL_WIDTH} ")
    prefixed_lines: list[str] = []
    for raw_line in line.splitlines() or [""]:
        if len(raw_line) >= event_prefix_width and raw_line[:2].isdigit():
            prefixed_lines.append(raw_line[:event_prefix_width] + prefix + raw_line[event_prefix_width:])
        else:
            prefixed_lines.append(" " * event_prefix_width + prefix + raw_line[event_prefix_width:])
    return "\n".join(prefixed_lines)
...
def _watch_supervisor_state_dir(anchor_config: Config, selection: "WatchSupervisorSelection") -> Path:
    if selection.aggregate_state_dir is not None:
        return selection.aggregate_state_dir
    if selection.manifest_path is not None:
        return selection.manifest_path.parent / ".gza" / "watch-supervisor"
    return anchor_config.project_dir / ".gza" / "watch-supervisor"


def _watch_supervisor_aggregate_log_path(anchor_config: Config, selection: "WatchSupervisorSelection") -> Path:
    return _watch_supervisor_state_dir(anchor_config, selection) / "watch-supervisor.log"
...
def _pending_runnable_tasks(
    ...
    *,
    ...
    return [
        ...
        if task.id is not None and (_resolve_failure_owner_task_id(store, str(task.id)) not in excluded_owner_ids)
    ]
...
def _format_watch_duration(seconds: float) -> str:
...
def _resolve_main_verify_target_head(git: Git, target_branch: str, log: "_WatchLog") -> str | None:
    ...
    try:
        ...
        log.emit(
            "WARN",
            f"main verify target SHA unavailable for {target_branch}: {exc}; using unknown",
        )
        return None
    if head_sha is None:
        log.emit(
            "WARN",
            f"main verify target SHA unavailable for {target_branch}: HEAD did not resolve; using unknown",
        )
    return head_sha


def _format_main_verify_target(target_branch: str, head_sha: str | None) -> str:
    short_sha = head_sha[:12] if head_sha else "unknown"
    return f"{target_branch}@{short_sha}"


def _main_verify_completion_verdict(check: MainIntegrationVerifyCheck) -> str:
    state = getattr(check, "state", None)
    status = getattr(state, "verify_status", None)
...
def _main_verify_rerun_result_description(state: object | None) -> str:
    status = getattr(state, "verify_status", None)
    if status == "failed":
        return "red"
    if status == "unavailable":
...
def _run_watch_main_integration_verify(
    *,
    config: Config,
    store: SqliteTaskStore,
    git: Git,
    ...
    log.emit(
        "INFO",
        (
            f"main verify checking checkpoint for {reason} against {target}; "
    ...
    check = _run_with_optional_stdout_suppressed(
        quiet or suppress_verify_stdout,
    ...
    message = (
        f"main verify {cached_prefix}{verdict} for {reason} against {target}{phase_suffix}; "
        f"elapsed {elapsed}{runs_suffix}"
    )
    log.emit(
        "INFO",
        message,
...
def _watch_execution_requires_dispatch_confirmation(result: AdvanceActionExecutionResult) -> bool:
    ...
    return (
        result.worker_consuming
        and result.attempted_spawn
        and result.worker_started
        and result.handled_task_id is not None
    )
...
def _settle_single_watch_dispatch_start(
    *,
    config: Config,
    store: SqliteTaskStore,
    task_id: str,
...
def _emit_deferred_watch_dispatch_outcome(
    *,
    ...
    if settle_result.status is _DispatchSettleStatus.TERMINAL_BEFORE_RUNNING:
        log.emit(
            "START_NOSLOT",
            (f"{deferred.settle.start_label}: dispatch exited before occupying a live slot ({settle_result.reason})"),
...
def _format_expected_start_failed_line(task_id: str, expected_start: "_ExpectedStart") -> str:
    return f"{task_id} {_format_expected_start_annotation(expected_start)}: spawned worker never reached in_progress"
...
@dataclass(frozen=True)
class WatchProjectSelection:
    """Project-local selector state owned by a watch runtime."""
...
class WatchSupervisorSelection:
    ...
    aggregate_state_dir: Path | None = None
...
_WATCH_MANIFEST_TOP_LEVEL_KEYS = frozenset(
    {"version", "projects", "strategy", "batch", "poll", "max_idle", "recovery_slots", "aggregate_state_dir"}
)
...
def _resolve_watch_state_dir(raw: object, *, base_dir: Path, context: str) -> Path:
    if not isinstance(raw, str):
...
def _load_watch_supervisor_manifest(path_arg: object) -> WatchSupervisorSelection:
    ...
    return WatchSupervisorSelection(
        ...
        aggregate_state_dir=(
            _resolve_watch_state_dir(
                loaded["aggregate_state_dir"],
                base_dir=manifest_path.parent,
                context="--watch-config aggregate_state_dir",
            )
            if "aggregate_state_dir" in loaded
            else None
        ),
    )
...
def _capture_watch_reexec_invocation(args: argparse.Namespace) -> _WatchReexecInvocation:
    return _WatchReexecInvocation(
        ...
        watch_state_dir=getattr(args, "watch_state_dir", None),
...
def resolve_watch_supervisor_selection(args: argparse.Namespace) -> WatchSupervisorSelection:
    ...
    return WatchSupervisorSelection(
        ...
        aggregate_state_dir=(
            _resolve_watch_state_dir(
                getattr(args, "watch_state_dir"),
                base_dir=Path.cwd(),
                context="--watch-state-dir",
            )
            if getattr(args, "watch_state_dir", None)
            else manifest.aggregate_state_dir
        ),
    )
...
def _fallback_watch_log(
    ...
    *,
    ...
    log.console_prefix = f"[{project_key}] "
    log.aggregate_path = aggregate_path
    log._aggregate_sink_failed = False
    log._project_sink_failed = True
    log._project_sink_warning_emitted = True
    log.quiet = quiet
    ...
    if aggregate_path is not None:
        try:
            ...
            log._aggregate_sink_failed = True
            console.print(
                f"WARN      [{project_key}] aggregate watch log sink disabled for {aggregate_path}: {aggregate_exc}",
                soft_wrap=True,
                highlight=False,
                markup=False,
            )
...
def _watch_lease_conflict_disabled_projects(exc: WatchLeaseConflict) -> tuple[ExecutionProjectDisabled, ...]:
    return tuple(disabled for disabled in exc.watch_disabled_projects if isinstance(disabled, ExecutionProjectDisabled))
...
def acquire_watch_supervisor_selection_leases(
    *,
    ...
    if existing_lease_set is not None and owner_token is not None and owner_token != existing_lease_set.owner_token:
                ...
                try:
                    ...
                    conflicted = next(
                        (
                            ...
                            and watch_lease_target_identity(
                                WatchLeaseTarget(candidate.project.key, candidate.lease_store)
                            )
    ...
    *,
    ...
    try:
        for candidate in lease_acquisition.activation_candidates:
            ...
            rotate_existing_log = False
            try:
                if isinstance(runtime_state, WatchSupervisorRuntimeState):
                    resolved_log_path, rotate_existing_log = runtime_state.project_log_initialization_rotation(log_path)
                log = _WatchLog(
                    log_path,
                    quiet=quiet,
                    rotate_existing=rotate_existing_log,
                    console_prefix=f"[{project.key}] ",
                    aggregate_path=aggregate_log_path,
                )
            except (OSError, RuntimeError) as exc:
                log = _fallback_watch_log(
                    project.key,
                    log_path,
                    exc,
                    quiet=quiet,
                ...
                if isinstance(runtime_state, WatchSupervisorRuntimeState) and resolved_log_path is not None:
                    runtime_state.mark_project_log_initialized(resolved_log_path)
            try:
...
@dataclass(frozen=True)
class ProjectDispatchCandidate:
    """Candidate head exposed by a project runtime to a later fleet supervisor."""
...
@dataclass(frozen=True)
class WatchSupervisorFleetCycleResult:
    """One fleet supervisor cycle result after global phase barriers."""
    ...
    dispatch_results: WatchSupervisorDispatchResult = field(
        default_factory=lambda: WatchSupervisorDispatchResult((), ())
    )
...
@dataclass
class WatchSupervisorRuntimeState:
    """Persistent runtime-owned observations that survive fleet poll refreshes."""

    runtimes_by_key: dict[str, "WatchProjectRuntime"] = field(default_factory=dict)
    ...
    def transfer_to(self, runtime: "WatchProjectRuntime") -> None:
        ...
        if previous is None or not _watch_runtime_state_matches(previous, runtime):
            return
        runtime.failure_backoffs = dict(previous.failure_backoffs)
        runtime.previous_snapshot = dict(previous.previous_snapshot)
        runtime.expected_starts = dict(previous.expected_starts)
        runtime.seen_active_recovery_subject_ids = previous.seen_active_recovery_subject_ids
        runtime.system_hold_active = previous.system_hold_active
        runtime.git_health_hold_active = previous.git_health_hold_active
        runtime.failure_halt_active = previous.failure_halt_active
        runtime._confirmed_started_task_ids = set(previous._confirmed_started_task_ids)
        runtime._attempted_dispatch_task_ids = set(previous._attempted_dispatch_task_ids)
        runtime.log.transfer_state_from(previous.log)

    def remember_runtime(self, runtime: "WatchProjectRuntime") -> None:
        self.runtimes_by_key[runtime.key] = runtime
        self.local_limit_by_runtime_key[runtime.key] = runtime.config.max_concurrent

    def remember_reconcile(self, state: ProjectRuntimeReconcileResult, *, local_limit: int) -> None:
        self.last_reconcile_by_runtime_key[state.runtime_key] = state
        self.local_limit_by_runtime_key[state.runtime_key] = local_limit

    def project_log_initialization_rotation(self, path: Path) -> tuple[Path, bool]:
        resolved_path = path.resolve()
        return resolved_path, resolved_path not in self.initialized_project_log_paths

    def mark_project_log_initialized(self, resolved_path: Path) -> None:
        self.initialized_project_log_paths.add(resolved_path)

    def disabled_reconcile_states(
        self,
        disabled: Sequence[ExecutionProjectDisabled],
        active_runtime_keys: AbstractSet[str],
    ) -> tuple[ProjectRuntimeReconcileResult, ...]:
        states: list[ProjectRuntimeReconcileResult] = []
        seen: set[str] = set()
        for disabled_project in disabled:
            if disabled_project.selector_key in active_runtime_keys:
                continue
            if disabled_project.selector_key in seen:
                continue
            previous = self.last_reconcile_by_runtime_key.get(disabled_project.selector_key)
...
def _watch_runtime_state_matches(previous: "WatchProjectRuntime", current: "WatchProjectRuntime") -> bool:
    return (
        ...
        and previous.config.project_id == current.config.project_id
        and previous.store.db_path.resolve() == current.store.db_path.resolve()
    )
...
class SupervisorLaunchBudget:
    ...
    supervisor_batch: int
    dry_run: bool = False
    read_only_reconcile_states: Sequence[ProjectRuntimeReconcileResult] = ()
    read_only_limit_by_runtime_key: Mapping[str, int] = field(default_factory=dict)
    ...
    disabled_runtime_keys: set[str] = field(default_factory=set, init=False)
    refresh_count: int = 0

    def __post_init__(self) -> None:
        if self.supervisor_batch < 0:
            raise ValueError("supervisor_batch must be non-negative")
        self._last_reconcile_by_runtime_key = {state.runtime_key: state for state in self.read_only_reconcile_states}
    ...
    @property
    def reconciled_states(self) -> tuple[ProjectRuntimeReconcileResult, ...]:
        with self._lock:
            ...
            for runtime in self.runtimes:
                ...
                try:
                    ...
                    if previous is not None:
                        snapshots.append(previous)
                    continue
                self._last_reconcile_by_runtime_key[runtime.key] = snapshot
                snapshots.append(snapshot)
            for state in self.read_only_reconcile_states:
                if state.runtime_key not in self._last_reconcile_by_runtime_key:
                    self._last_reconcile_by_runtime_key[state.runtime_key] = state
                if state.runtime_key not in {snapshot.runtime_key for snapshot in snapshots}:
                    snapshots.append(state)
            limit_by_runtime_key = {runtime.key: runtime.config.max_concurrent for runtime in self.runtimes}
            limit_by_runtime_key.update(self.read_only_limit_by_runtime_key)
            self._occupancy = aggregate_reconciled_watch_occupancy(
                tuple(snapshots),
    ...
    def disable_runtime(
        ...
        *,
        phase: Literal["health", "reconcile", "analyze", "direct", "dispatch", "observe", "baseline", "pending"],
        exc: BaseException,
    ) -> ExecutionProjectDisabled:
        disabled = _watch_supervisor_operational_disabled_project(runtime, phase=phase, exc=exc)
        self.disabled_runtime_keys.add(runtime.key)
        runtime.log.emit("HOLD", disabled.message)
        if self.operational_disabled_callback is not None:
            self.operational_disabled_callback(disabled)
        return disabled
    ...
    *,
    ...
    with contextlib.ExitStack() as dry_run_attempt_stack:
        ...
        while worker_consuming_starts < limit and launch_budget.occupancy.slots > 0:
            ...
            for runtime in runtimes:
                if runtime.key in launch_budget.disabled_runtime_keys:
            ...
            if selected_runtime is None:
                ...
                continue
            if selected_runtime.key in launch_budget.disabled_runtime_keys:
                results.append(
                    ProjectDispatchResult(
                        runtime_key=choice.project_key,
                        candidate=choice.candidate,
                        ...
                        slot_consuming=False,
                        work_done=False,
                        detail=f"runtime {choice.project_key!r} is disabled for this supervisor pass",
                        task=choice.candidate.task,
                    )
                )
                continue
            try:
                ...
                disabled = launch_budget.disable_runtime(selected_runtime, phase="dispatch", exc=exc)
                result = ProjectDispatchResult(
...
def _watch_cycle_disabled_project(
    runtime: "WatchProjectRuntime",
    *,
    reason: ExecutionProjectDisableReason,
    message: str,
    ...
    return ExecutionProjectDisabled(
        selector_key=runtime.key,
        reason=reason,
        message=message,
        project_id=runtime.config.project_id,
        root_path=runtime.config.project_dir,
        config_path=runtime.config.project_dir / "gza.yaml",
        db_path=runtime.config.db_path,
    )


def _watch_supervisor_runtime_enabled_for_cycle(
    runtime: "WatchProjectRuntime",
    *,
    dry_run: bool,
    ...
    if not _system_can_run_tasks(runtime.config, runtime_context=runtime.runtime_context):
        runtime.log.emit(
            "HOLD",
            "System unavailable: Docker daemon unreachable - holding project, nothing started/failed",
        )
        runtime.system_hold_active = True
        return False, _watch_cycle_disabled_project(
            runtime,
            reason="docker-held",
            message="Docker daemon unreachable; project held until the next supervisor poll boundary.",
        )
    if runtime.system_hold_active:
        runtime.log.emit("RESUME", "Docker available again - resuming")
        runtime.system_hold_active = False
    ...
    if _emit_git_health_hold(
        ...
        return False, _watch_cycle_disabled_project(
            ...
            message="Git worktree health is red; project held until the next supervisor poll boundary.",
        )
    if runtime.git_health_hold_active:
        runtime.log.emit("RESUME", "git worktree health restored - resuming dispatch")
        runtime.git_health_hold_active = False
    return True, None


_WATCH_PROJECT_OPERATIONAL_EXCEPTIONS = (
    ManualMigrationRequired,
    ExecutionProjectActivationError,
    SchemaIntegrityError,
    ConfigError,
...
def _watch_supervisor_operational_disabled_project(
    runtime: "WatchProjectRuntime",
    *,
    phase: Literal["health", "reconcile", "analyze", "direct", "dispatch", "observe", "baseline", "pending"],
    exc: BaseException,
    ...
    if isinstance(exc, ExecutionProjectActivationError):
        reason = exc.reason
    elif isinstance(exc, ManualMigrationRequired):
        reason = "manual_migration_required"
        ...
        reason = "db_unavailable" if _sqlite_error_is_db_unavailable(exc) else "schema_incompatible"
    elif isinstance(exc, (OSError, GitError)):
        reason = "runtime_filesystem_unavailable"
    else:
        reason = "runtime_filesystem_unavailable"
    return _watch_cycle_disabled_project(
        runtime,
        reason=reason,
        message=(
            f"Execution project {phase} phase failed for selector {runtime.key!r} "
            f"(project_id={runtime.config.project_id!r}, root={runtime.config.project_dir}): {exc}"
        ),
    )


def _watch_supervisor_try_project_phase(
    runtime: "WatchProjectRuntime",
    *,
    phase: Literal["health", "reconcile", "analyze", "direct", "dispatch", "observe", "baseline", "pending"],
    operation: Callable[[], T],
) -> tuple[T | None, ExecutionProjectDisabled | None]:
    try:
...
def _watch_supervisor_emit_aggregate_summary(
    *,
    disabled: Sequence[ExecutionProjectDisabled],
    ...
    def emit_summary_line(line: str) -> None:
        if not quiet:
            print(line)
        if aggregate_log_path is None:
            return
        try:
            aggregate_log_path.parent.mkdir(parents=True, exist_ok=True)
            with open(aggregate_log_path, "a") as f:
                f.write(line + "\n")
        except OSError as exc:
            print(
                f"WARN      aggregate watch log sink disabled for {aggregate_log_path}: {exc}",
                file=sys.stderr,
            )

    for disabled_project in disabled:
        emit_summary_line(f"[{disabled_project.selector_key}] {_format_disabled_watch_project(disabled_project)}")
    for local in occupancy.local:
        emit_summary_line(
            f"[{local.runtime_key}] state running={local.running} starting={local.starting} "
            f"available={local.available} anonymous={local.anonymous_worker_count}"
        )
    for direct_result in direct_results:
        if direct_result.work_done or direct_result.blocked or direct_result.detail is not None:
            detail = f": {direct_result.detail}" if direct_result.detail else ""
            emit_summary_line(
                f"[{direct_result.runtime_key}] direct "
                f"work_done={direct_result.work_done} replan={direct_result.needs_replan}{detail}"
            )
    for dispatch_result in dispatch_results.results:
        task_id = dispatch_result.task.id if dispatch_result.task is not None else dispatch_result.candidate.task.id
        emit_summary_line(
            f"[{dispatch_result.runtime_key}] dispatch "
            f"{dispatch_result.candidate.lane} {task_id}: {dispatch_result.status}"
        )
    emit_summary_line(
        "fleet summary: "
        f"running={occupancy.running} starting={occupancy.starting} "
        f"slots={occupancy.slots} disabled={len(disabled)}"
    )


def _emit_watch_supervisor_strategy_startup_warning(
    *,
    strategy_name: str,
    aggregate_log_path: Path | None,
    quiet: bool,
) -> None:
    metadata = WATCH_DISPATCH_STRATEGY_REGISTRY[strategy_name]
    if metadata.startup_warning is None:
        return
    line = f"WARNING   strategy {strategy_name}: {metadata.startup_warning}"
    if not quiet:
        print(line)
    if aggregate_log_path is None:
        return
...
def watch_supervisor_fleet_cycle_is_idle(result: WatchSupervisorFleetCycleResult) -> bool:
    ...
    if result.work_done:
        return False
    if result.runnable_candidate_count > 0 or result.pending > 0:
        return False
    if occupancy is None:
        return result.running == 0 and result.starting_worker_count == 0
    return (
        occupancy.running == 0
        and occupancy.starting == 0
        and occupancy.provisional_reservations == 0
        and result.running == 0
        and result.starting_worker_count == 0
    )


def run_watch_supervisor_fleet_cycle(
    *,
    anchor_store: SqliteTaskStore,
    selection: WatchSupervisorSelection,
    quiet: bool,
    owner_token: str | None,
    existing_lease_set: WatchLeaseSet | None,
    batch: int,
    recovery_slots: int,
    recovery_mode: DispatchSelectionMode | None,
    max_recovery_attempts: int | None,
    ...
    recovery_strategy: WatchDispatchStrategy[ProjectDispatchCandidate] | None = None,
    pending_strategy: WatchDispatchStrategy[ProjectDispatchCandidate] | None = None,
    ...
    try:
        ...
        baseline_disabled_keys: set[str] = set()
        for runtime in runtimes:
        ...
        for runtime in runtimes:
            if runtime.key in baseline_disabled_keys:
                continue
            health_result, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
                phase="health",
                operation=functools.partial(
                    _watch_supervisor_runtime_enabled_for_cycle,
                    runtime,
                    dry_run=dry_run,
                ),
            )
        ...
        for runtime in enabled_runtimes:
            if not dry_run:
                _repair_result, disabled_project = _watch_supervisor_try_project_phase(
                    runtime,
                    phase="reconcile",
        ...
        for runtime in enabled_runtimes:
            ...
            observed, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
                phase="observe",
                operation=functools.partial(
                    runtime.observe_cycle_boundary,
                    restart_failed_mode=normalize_dispatch_selection_mode(recovery_mode) == "recovery_only",
        ...
        for runtime in enabled_runtimes:
            attempts = resolve_watch_supervisor_runtime_recovery_attempts(runtime, max_recovery_attempts)
            direct_result, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
                phase="direct",
                operation=functools.partial(
                    runtime.run_direct_phase,
                    analysis=analyses.get(runtime.key),
                    batch=batch,
                    max_iterations=max_iterations,
                    dry_run=dry_run,
        ...
        limit_by_runtime_key = {runtime.key: runtime.config.max_concurrent for runtime in enabled_runtimes}
        limit_by_runtime_key.update(runtime_state.local_limit_by_runtime_key)
        pre_dispatch_occupancy = aggregate_reconciled_watch_occupancy(
            (
                *(state for state in reconciled_states if state.runtime_key in active_runtime_keys),
                *disabled_reconcile_states,
            ),
            supervisor_batch=batch,
            limit_by_runtime_key=limit_by_runtime_key,
        ...
        weights = {project.key: project.weight for project in selection.projects}
        if recovery_strategy is None and project_order:
            recovery_strategy = create_watch_dispatch_strategy(
                strategy_name,
                project_order=project_order,
                weights=weights,
            )
        if pending_strategy is None and project_order:
        ...
        dispatch_results = WatchSupervisorDispatchResult((), ())
        launch_budget: SupervisorLaunchBudget | None = None
        if enabled_runtimes and recovery_strategy is not None and pending_strategy is not None:
            launch_budget = SupervisorLaunchBudget(
                enabled_runtimes,
                supervisor_batch=batch,
                dry_run=dry_run,
                read_only_reconcile_states=(
                    *(state for state in reconciled_states if state.runtime_key in active_runtime_keys),
                    *disabled_reconcile_states,
                ),
            ...
            for state in launch_budget.reconciled_states:
                local_limit = runtime_state.local_limit_by_runtime_key.get(
                    state.runtime_key,
                    launch_budget.read_only_limit_by_runtime_key.get(state.runtime_key, 0),
                )
                if state.runtime_key in launch_budget.runtime_by_key:
        ...
        active_dispatch_disabled_keys = launch_budget.disabled_runtime_keys if launch_budget is not None else set()
        ...
        for runtime in enabled_runtimes:
            ...
            pending_count, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
                phase="pending",
            ...
            has_head, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
                phase="pending",
        ...
        enabled_runtimes = [runtime for runtime in enabled_runtimes if runtime.key in analyses]
        work_done = any(result.work_done for result in direct_results) or any(
            result.work_done for result in dispatch_results.results
        )
        confirmed_start_count = 0
        halted_by_failure_boundary = False
        for runtime in enabled_runtimes:
            attempts = resolve_watch_supervisor_runtime_recovery_attempts(runtime, max_recovery_attempts)
            observed, disabled_project = _watch_supervisor_try_project_phase(
                runtime,
            ...
            if observed[1]:
                ...
                halted_by_failure_boundary = True
        anonymous_worker_count = sum(local.anonymous_worker_count for local in occupancy.local)
        result = WatchSupervisorFleetCycleResult(
            work_done=work_done or confirmed_start_count > 0 or halted_by_failure_boundary,
            running=occupancy.running,
            pending=pending,
            anonymous_worker_count=anonymous_worker_count,
            starting_worker_count=occupancy.starting,
        ...
        if construction.lease_set is not None and construction.lease_set is not existing_lease_set:
            try:
                construction.lease_set.release()
            except WatchLeaseReleaseError as cleanup_error:
                raise BaseExceptionGroup(
                    "fleet watch cycle failed and watch-supervisor lease release also failed",
                    [primary_error, cleanup_error],
                ) from primary_error
        raise
...
class WatchProjectRuntime:
    ...
    def observe_cycle_boundary(
        self,
        *,
        restart_failed_mode: bool,
        max_recovery_attempts: int,
    ) -> tuple[int, bool]:
        """Observe task transitions and failure boundaries for this runtime."""
        current_snapshot = _task_snapshot(self.store)
        confirmed_start_ids = _emit_transition_events(
            ...
            config=self.config,
            log=self.log,
            restart_failed_mode=restart_failed_mode,
            max_recovery_attempts=max_recovery_attempts,
            scoped_owner_ids=None,
            scoped_selector_recovery_closure_by_owner=None,
        )
        confirmed_count = _process_expected_start_boundary(
            store=self.store,
            config=self.config,
            log=self.log,
        ...
        _reset_failure_backoff_for_completed_owners(
            ...
            failure_backoffs=self.failure_backoffs,
            log=self.log,
            scoped_owner_ids=None,
        )
    ...
    def any_tag(self) -> bool:
        return self.selection.any_tag

    def active_failure_owner_ids(self, *, now: datetime | None = None) -> frozenset[str]:
        return _active_failure_backoff_owner_ids(
            self.failure_backoffs,
    ...
    def repair_merge_unit_state(self) -> None:
...
def _map_planned_recovery_entries_to_actionable_failed(
    ...
    *,
    actionable_failed: Sequence[tuple[LineageOwnerRow, DbTask, FailedRecoveryDecision, dict[str, Any], bool, DbTask]],
    ...
    actionable_by_failed_id = {
        str(failed.id): (row, failed, decision, recovery_action, worker_consuming_recovery, action_task)
    ...
    planned_actionable_failed: list[
        tuple[LineageOwnerRow, DbTask, FailedRecoveryDecision, dict[str, Any], bool, DbTask]
    ] = []
    unmapped_entries: list[DispatchPreviewEntry] = []
    for entry in planned_entries:
        if entry.lane != "recovery" or entry.task.id is None:
            continue
        failed_id = str(entry.task.id)
        planned_failed = actionable_by_failed_id.get(failed_id)
        if planned_failed is None:
            if failed_id in known_non_actionable_failed_ids:
                continue
            unmapped_entries.append(entry)
            continue
        planned_actionable_failed.append(planned_failed)
...
def _emit_recovery_dry_run_report(
    *,
    ...
    hidden_breakdown = (
        f"{rebase} needs_rebase, {resume} resume, {retry} retry" if rebase > 0 else f"{resume} resume, {retry} retry"
    )
    if show_skipped:
        print(
    ...
    *,
    ...
    for failure in failures:
        ...
        log.emit(
            ...
            dedupe_key=(f"owner-backoff:{failure.owner_task_id}:{streak}:{backoff_until.astimezone(UTC).isoformat()}"),
        )
    ...
    log.emit(
        ...
        (
            ...
            "holding affected watch runtime for human intervention"
        ),
    )
    return True
        ...
        for row in recovery_rows:
            ...
            if recovery_action_class == "skip":
                ...
                if parked_attention is not None:
                    ...
                    continue
                if str(recovery_action.get("reason", "")).startswith("recovery_already_") or decision.reason_code in {
                    "recovery_already_pending",
                    "recovery_already_running",
                }:
                    active_recovery_subject_ids.add(str(failed.id))
...
def _watch_failed_recovery_scan_is_current(
    *,
    ...
    if marker is None:
        ...
        stale_units, merged_proof_by_branch = _watch_failed_recovery_scan_target_moved_units(
            git=git,
            units=units,
            target_branch=target_branch,
        ...
        stale_units = list(units)
        merged_proof_by_branch = {}
    else:
        stale_units = _watch_failed_recovery_scan_source_changed_units(
            git=git,
            units=units,
            target_branch=target_branch,
            incomplete_reasons=incomplete_reasons,
    ...
    if stale_units:
        stale_unit_ids = {unit.id for unit in stale_units}
        cohorts = _watch_failed_recovery_scan_cohorts(store, stale_units)
        ...
        for cohort, result in zip(cohorts, results, strict=True):
            ...
            if result.head_sha is None or result.base_sha is None:
                ...
                proof_errors.append(
                    ...
                    f"'{result.base_sha}' instead of current cycle snapshot '{target_sha}'"
                )
        ...
        stale_unit_ids = set()
    if marker is not None and not target_unchanged and not incomplete_reasons:
        for unit in units:
            if unit.id in stale_unit_ids or unit.head_sha is None or unit.base_sha == target_sha:
                continue
            try:
                store.refresh_merge_unit_head(unit.id, unit.head_sha, target_sha)
            except Exception as exc:
                incomplete_reasons.append(
                    f"branch '{unit.source_branch}' failed to persist current target proof: {exc}"
                )
    if stale_units or (marker is not None and not target_unchanged):
...
def _watch_failed_recovery_scan_cohorts(store: SqliteTaskStore, units: Sequence[MergeUnit]) -> list[BranchCohort]:
    return [
        BranchCohort(
            branch=str(unit.source_branch),
            tasks=tuple(store.list_tasks_for_merge_unit(unit.id)),
            merge_unit_id=str(unit.id),
            merge_unit_state=(
                "unmerged"
...
def _watch_failed_recovery_scan_resolve_refs(
    *,
    git: Git,
    units: Sequence[MergeUnit],
) -> Mapping[str, str | None]:
    refs = sorted({str(unit.source_branch) for unit in units})
...
def _watch_failed_recovery_scan_target_moved_units(
    *,
    git: Git,
    units: Sequence[MergeUnit],
    target_branch: str,
        ...
        if not callable(content_equivalent):
            ...
            try:
                content_presence = content_equivalent(
                    [str(unit.head_sha) for unit in unchanged_candidates if unit.head_sha is not None],
                    target_sha,
                )
            except Exception as exc:
                incomplete_reasons.append(
                    f"batched content-equivalence proof unavailable for target '{target_branch}': {exc}"
                )
            else:
                for unit in unchanged_candidates:
                    ...
                    if present is None:
                        ...
                        stale_by_id[unit.id] = unit
                        merged_proof_by_branch[str(unit.source_branch)] = True
                    elif unit.state == "merged" and not present:
                        stale_by_id[unit.id] = unit
                        merged_proof_by_branch[str(unit.source_branch)] = False
                    elif unit.state in {"empty", "redundant"} and not present:
                        stale_by_id[unit.id] = unit
                        merged_proof_by_branch[str(unit.source_branch)] = False
...
def _evaluate_blind_parked_auto_rearm(
    *,
    ...
    scan_current = (
        ...
        else _watch_failed_recovery_scan_is_current(
            store=store,
            git=git,
            target_branch=target_branch,
            target_sha=target_sha,
...
def _dispatch_scoped_watch_once(
    *,
    ...
    any_tag: bool = False,
    quiet: bool = False,
    suppress_main_verify_stdout: bool = False,
...
def _run_cycle(
    *,
    ...
    def _emit_settle_no_progress_attention(
        *,
        deferred: _DeferredWatchDispatchStart,
        no_progress_attention: dict[str, Any] | None,
        attention_key_prefix: str,
    ) -> None:
        if no_progress_attention is None or deferred.subject_task.id is None:
            return
        log.emit_attention(
            attention_key=(
                f"{attention_key_prefix}:{deferred.subject_task.id}:{deferred.action_type}:watch-no-progress"
            ),
            message=_watch_needs_attention_message(deferred.subject_task, no_progress_attention),
        )

    def _settle_deferred_dispatch_starts(
        deferred_starts: list[_DeferredWatchDispatchStart],
        *,
        count_confirmed_start: bool,
        count_recovery_start: bool,
    ) -> None:
        nonlocal confirmed_start_count, recovery_started_this_cycle, work_done
        if not deferred_starts:
            return
        settled_starts = _settle_watch_dispatch_starts(
            config=config,
            store=store,
            pending_starts=[deferred.settle for deferred in deferred_starts],
        )
        for deferred, settle_result in zip(deferred_starts, settled_starts, strict=True):
            started, no_progress_attention = _emit_deferred_watch_dispatch_outcome(
                config=config,
                store=store,
                log=log,
            ...
            if not started:
                ...
                _emit_settle_no_progress_attention(
                    ...
                    attention_key_prefix=(
                        "recovery-attention" if deferred.failed_task is not None else "advance-attention"
                    ),
                )
            ...
            if count_confirmed_start:
                confirmed_start_count += 1
            if count_recovery_start:
                recovery_started_this_cycle = True
            work_done = True
            _emit_settle_no_progress_attention(
                deferred=deferred,
                no_progress_attention=no_progress_attention,
                attention_key_prefix="recovery-attention" if deferred.failed_task is not None else "advance-attention",
            )
    ...
    def _consume_active_main_verify_remediation_terminal_attempt(
        candidate: DbTask | None,
        *,
        reason: str,
        ...
        if dry_run or active_main_verify_terminal_attempt_consumed:
            return False
        if active_main_verify_remediation is None or latest_main_verify_state is None:
            return False
        if active_main_verify_remediation.kind != "fix" or active_main_verify_remediation_task_id is None:
            return False
        active_task = store.get(active_main_verify_remediation_task_id)
        if active_task is None:
            store.clear_main_verify_remediation_active_task(
                signature=active_main_verify_remediation.signature,
                tree_fingerprint=_main_verify_remediation_ledger_fingerprint(
                    active_main_verify_remediation.tree_fingerprint
                ),
                last_observed_head_sha=getattr(latest_main_verify_state, "head_sha", None),
                last_observed_failure=active_main_verify_remediation.failure,
            )
            active_main_verify_remediation = None
            active_main_verify_remediation_task_id = None
            return False
        ...
        if require_candidate_lineage:
            if candidate is None or candidate.id is None:
                return False
            if not _task_is_descendant_of(store, candidate.id, active_main_verify_remediation_task_id):
                return False
        if active_task.status not in {"completed", "unmerged", "dropped"}:
            return False
        ...
        if _main_verify_remediation_task_is_effectively_merged(store, active_task):
            ...
            if _main_verify_remediation_attempt_state_exhausted(attempt_state, attempt_limit=attempt_limit):
                ...
                refreshed_main_verify_state = _persist_main_verify_remediation_exhausted_attention(
                    store=store,
                    log=log,
                    state=latest_main_verify_state,
                    remediation=active_main_verify_remediation,
                    exhausted_task_id=active_main_verify_remediation_task_id,
                    attempt_limit=attempt_limit,
                )
        else:
            consume_result = _consume_unmerged_main_verify_remediation_attempt(
                config=config,
                store=store,
                log=log,
                task=active_task,
            ...
            if not consume_result.consumed:
                return False
            refreshed_main_verify_state = consume_result.refreshed_state
        if refreshed_main_verify_state is not None:
            latest_main_verify_state = refreshed_main_verify_state
            _emit_main_verify_attention(
                log=log,
                state=refreshed_main_verify_state,
                now=datetime.now(UTC),
                git=git,
                target_branch=target_branch,
            )
    ...
    def _fail_closed_for_planned_recovery_entry(
        ...
        *,
        ...
        owner_id = (
            entry.owner_task.id if entry.owner_task is not None and entry.owner_task.id is not None else "unknown"
        )
        log.emit_attention(
            attention_key=f"planned-recovery-dispatch-invariant:{stage}:{task_id}:{reason}",
            message=(
                f"{task_id}: planned recovery entry from {stage} could not be dispatched "
                f"(owner={owner_id}, reason={reason}); watch stopped further dispatch for this pass"
            ),
        )

    def _dispatch_planned_recovery_entries(
        planned_rows: Sequence[tuple[LineageOwnerRow, DbTask, FailedRecoveryDecision, dict[str, Any], bool, DbTask]],
        *,
        pending_slots_for_replan: int,
        allow_replan: bool,
        processed_task_ids: set[str] | None = None,
    ) -> tuple[bool, bool]:
        nonlocal recovery_started_this_cycle, work_done
        local_replan_requested = False
        local_fail_closed = False
        deferred_recovery_starts: list[_DeferredWatchDispatchStart] = []

        for row, failed, decision, recovery_action, worker_consuming_recovery, action_task in planned_rows:
            if failed.id is None:
                continue
            if processed_task_ids is not None:
                processed_task_ids.add(str(failed.id))
            recovery_action_type = str(recovery_action.get("type", ""))
            if worker_consuming_recovery and (_free_worker_start_slots() <= 0 or _free_reserved_recovery_slots() <= 0):
            ...
            if recovery_action_type == "reconcile_branch_divergence":
                ...
                if exec_result.status == "success" and _watch_action_attempted_direct_work(
                    ...
                    work_done = True
                    _consume_worker_slot_if_needed(exec_result, reserve_recovery_slot=True)
                    if allow_replan and pending_slots_for_replan <= 0 and not exec_result.worker_consuming:
                        local_replan_requested = True
                    continue
                continue
            if recovery_action_type == "needs_rebase":
                ...
                if (
                    ...
                    or exec_result.created_task is None
                    or exec_result.created_task.id is None
                ):
                    if (
                        allow_replan
                        and exec_result.status == "success"
                        and pending_slots_for_replan <= 0
                        and not exec_result.worker_consuming
                    ):
                        local_replan_requested = True
                    continue
                ...
                deferred_recovery_starts.append(
                    _DeferredWatchDispatchStart(
                        settle=_PendingWatchDispatchSettle(
                            ...
                            task_before=_snapshot_watch_dispatch_task(exec_result.created_task),
                            start_label=f"{failed.id} -> {recovered_task_id}",
                            dedupe_key=f"recovery-undispatched:needs_rebase:{failed.id}:{recovered_task_id}",
                        ),
                        subject_task=failed,
                        action=recovery_action,
                        action_task_before=action_task,
            ...
            if recovery_action_type == "resume":
                ...
                if decision.launch_mode == "worker":
                    ...
                    recovered_task_before = _snapshot_watch_dispatch_task(resume_recovered_task)
                    recovered_task_id = str(prepared_recovered_task.id)
                    pending_recovery_task_ids.add(recovered_task_id)
                    rc = _spawn_worker_with_failure_log(
                        quiet=quiet,
                ...
                recovered_task_before = _snapshot_watch_dispatch_task(retry_recovered_task)
                recovered_task_id = str(prepared_recovered_task.id)
                pending_recovery_task_ids.add(recovered_task_id)
                rc = (
                ...
                _fail_closed_for_planned_recovery_entry(
                    DispatchPreviewEntry(
                        ...
                        owner_task=row.owner_task,
                        runnable=True,
                        worker_consuming=worker_consuming_recovery,
                        decision=decision,
                        advance_action=recovery_action,
                        lineage_row=row,
                    ),
                    stage="replan" if not allow_replan else "initial-plan",
                    reason=f"unsupported-action:{recovery_action_type or 'unknown'}",
                )
                break
        ...
        _settle_deferred_dispatch_starts(
            deferred_recovery_starts,
            count_confirmed_start=True,
            ...
            if replan_recovery_dispatch and slots > 0 and not planned_recovery_dispatch_failed_closed:
                ...
                if unmapped_replanned_recovery_entries:
                    invariant_violating_entries = tuple(
                        ...
                        if entry.task.id is None
                        or str(entry.task.id) not in explicitly_undispatched_recovery_subject_ids
                    )
                    if invariant_violating_entries:
                        planned_recovery_dispatch_failed_closed = True
                    for entry in invariant_violating_entries:
...
def _emit_git_health_hold(
    *,
    ...
    log.emit_attention(
        ...
        message=git_health_check.state.alert_message or "git worktree health RED - dispatch halted",
    )
    return True
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    if supervisor_selection.multi_project:
        ...
        multi_project_reexec_fingerprint: str | None = None
        ...
        try:
            ...
            batch = supervisor_selection.batch if supervisor_selection.batch is not None else anchor_config.watch.batch
            poll = supervisor_selection.poll if supervisor_selection.poll is not None else anchor_config.watch.poll
            max_idle = (
                supervisor_selection.max_idle
                if supervisor_selection.max_idle_present
                else _resolve_watch_max_idle(args, anchor_config)
            )
            max_iterations = (
                args.max_iterations
                if getattr(args, "max_iterations", None) is not None
                else anchor_config.watch.max_iterations
            )
            requested_dispatch_mode = getattr(args, "dispatch_mode", None)
            if requested_dispatch_mode is None and hasattr(args, "recovery_mode"):
                requested_dispatch_mode = getattr(args, "recovery_mode", None)
            ...
            if requested_dispatch_mode is None and restart_failed:
                requested_dispatch_mode = "recovery_only"
            recovery_slots_arg = supervisor_selection.recovery_slots
            if recovery_slots_arg is None:
                recovery_slots_arg = getattr(args, "restart_failed_batch", None)
            recovery_slots = (
                recovery_slots_arg if recovery_slots_arg is not None else anchor_config.watch.recovery_slots
            )
            dispatch_mode = _normalize_watch_dispatch_selection_mode(
                dispatch_mode=cast(str | None, requested_dispatch_mode),
                recovery_slots=recovery_slots,
            ...
            max_recovery_attempts = (
                args.max_resume_attempts if getattr(args, "max_resume_attempts", None) is not None else None
            )
            if max_recovery_attempts is not None and max_recovery_attempts < 0:
                print("Error: --max-resume-attempts must be non-negative")
                multi_project_result_code = 1
                raise _MultiProjectEarlyResult
            if getattr(args, "task_ids", None):
                print("Error: positional task IDs are not supported in multi-project watch")
                multi_project_result_code = 1
                raise _MultiProjectEarlyResult
            ...
            project_order = tuple(project.key for project in supervisor_selection.projects)
            weights = {project.key: project.weight for project in supervisor_selection.projects}
            strategy_name = supervisor_selection.strategy or "round-robin"
            try:
                recovery_strategy_state, pending_strategy_state = _watch_reexec_strategy_states_from_args(args)
            except ValueError as exc:
                print(f"Error: {exc}")
                multi_project_result_code = 1
                raise _MultiProjectEarlyResult
            recovery_strategy: WatchDispatchStrategy[ProjectDispatchCandidate] = create_watch_dispatch_strategy(
                strategy_name,
                project_order=project_order,
                weights=weights,
                state=recovery_strategy_state,
            )
            pending_strategy: WatchDispatchStrategy[ProjectDispatchCandidate] = create_watch_dispatch_strategy(
                strategy_name,
                project_order=project_order,
                weights=weights,
                state=pending_strategy_state,
            )
            runtime_state = WatchSupervisorRuntimeState()
            aggregate_log_path = _watch_supervisor_aggregate_log_path(anchor_config, supervisor_selection)
            resumed_reexec = bool(getattr(args, "resumed_reexec", False))
            if not resumed_reexec:
                _emit_watch_supervisor_strategy_startup_warning(
            ...
            if not skip_confirm:
                if not sys.stdout.isatty():
                    print(
                        "watch: stdout is not a terminal, so the initial multi-project confirmation "
                        "prompt cannot be shown. Re-run with -y to auto-confirm.",
                        file=sys.stderr,
                    )
                    multi_project_result_code = 1
                    raise _MultiProjectEarlyResult
                preview_result, preview_lease_set = run_watch_supervisor_fleet_cycle(
                    anchor_store=anchor_store,
                    selection=supervisor_selection,
                    quiet=quiet,
                    owner_token=watch_lease_token,
                    existing_lease_set=supervisor_lease_set,
                    batch=batch,
                    recovery_slots=recovery_slots,
                    ...
                    pending_strategy=pending_strategy.clone(),
                    emit_summary=True,
                    runtime_state=runtime_state,
                    aggregate_log_path=aggregate_log_path,
                )
                if preview_lease_set is not None:
                    supervisor_lease_set = preview_lease_set
                    watch_lease_token = preview_lease_set.owner_token
            ...
            while multi_project_result_code is None:
                if multi_project_stop_signal is not None:
                    multi_project_result_code = 128 + multi_project_stop_signal
                    break
                fleet_cycle_result, supervisor_lease_set = run_watch_supervisor_fleet_cycle(
                    anchor_store=anchor_store,
                    selection=supervisor_selection,
                    quiet=quiet,
                    owner_token=watch_lease_token,
                ...
                if dry_run:
                    multi_project_result_code = 0
                    break
                if not watch_supervisor_fleet_cycle_is_idle(fleet_cycle_result):
                    idle_seconds = 0
                else:
                    idle_seconds += poll
                ...
                if multi_project_stop_signal is not None:
                    multi_project_result_code = 128 + multi_project_stop_signal
                    break
                _sleep_interruptibly(poll, lambda: multi_project_stop_signal is not None)

            if multi_project_reexec_fingerprint is not None and multi_project_result_code is None:
                args.watch_strategy_state = _serialize_watch_reexec_strategy_state(
                    recovery_strategy=recovery_strategy,
                    pending_strategy=pending_strategy,
                )
    ...
    def _emit_shutdown_message_once() -> None:
        ...
        if shutdown_message_emitted:
            return
        emitted, errors = _emit_watch_message_safely(
            runtime.log,
            quiet,
            "INFO",
            "shutting down (workers left running)",
        )
        if emitted:
    ...
    try:
        ...
        if dispatch_mode == "recovery_only" and dry_run and scoped_owner_ids is None:
            if _emit_git_health_hold(
                ...
                _dry_run_target_branch = runtime.git.default_branch()
        ...
        def _current_scoped_selector_recovery_closure_by_owner() -> dict[str, frozenset[str]] | None:
            if scoped_owner_ids is None:
                return None
            scoped_target_branch = runtime.git.default_branch()
        ...
        def _process_failure_boundary(
            ...
            *,
            ...
            _reset_failure_backoff_for_completed_owners(
                store=runtime.store,
                completed_ids=completed_ids,
                failure_backoffs=runtime.failure_backoffs,
                log=runtime.log,
                scoped_owner_ids=boundary_scoped_owner_ids,
            )
            unhandled_failures = _collect_unhandled_failures(
        ...
        def _prime_effective_scope_for_boundary() -> None:
            ...
            if pending_first_cycle_plan is None:
                pending_first_cycle_plan = runtime.build_cycle_plan(
                    batch=batch,
                    recovery_slots=recovery_slots,
                    recovery_mode=dispatch_mode,
                    max_recovery_attempts=max_recovery_attempts,
        ...
        while result_code is None:
            ...
            if not _system_can_run_tasks(runtime.config, runtime_context=runtime.runtime_context):
                ...
                if scoped_owner_ids is not None:
                    ...
                    if scope_message is not None:
                        runtime.log.emit("INFO", scope_message)
                    if scoped_activity.active_count == 0:
                        runtime.log.emit("INFO", SCOPED_WATCH_COMPLETE_MESSAGE)
                        break
                    runtime.log.emit(
                        "HOLD",
                        (
            ...
            pre_cycle_snapshot = _task_snapshot(runtime.store)
            ...
            cycle_confirmed_start_count = _process_expected_start_boundary(
                ...
                config=runtime.config,
                log=runtime.log,
                expected_starts=runtime.expected_starts,
            ...
            if _process_failure_boundary(
                ...
                runtime.previous_snapshot = pre_cycle_snapshot
                break
            runtime.previous_snapshot = pre_cycle_snapshot
            excluded_owner_ids = _active_failure_owner_ids()
            ...
            if cycle_result.expected_starts:
                runtime.expected_starts.update(cycle_result.expected_starts)
            runtime.seen_active_recovery_subject_ids = cycle_result.active_recovery_subject_ids | frozenset(
                str(task.based_on)
                for task in runtime.store.get_all()
                if task.status in {"pending", "in_progress"} and task.based_on is not None
            )

            current_snapshot = _task_snapshot(runtime.store)
            ...
            if _process_failure_boundary(
                ...
                runtime.previous_snapshot = current_snapshot
                break
            runtime.previous_snapshot = current_snapshot
            ...
            if not cycle_result.work_done:
                idle_seconds += poll
                if max_idle is not None and idle_seconds >= max_idle:
                    runtime.log.emit("INFO", f"max idle time reached ({max_idle}s), exiting")
                    break
        ...
        for restore_error in _restore_installed_watch_signal_handlers(installed_signal_handlers):
            ...
            cleanup_errors.extend(
                _emit_watch_error_safely(
                    runtime.log,
                    quiet,
                    f"watch signal handler restore failed: {restore_error}",
                )
            )
        if watch_session_recorded:
    ...
    if check.merges_halted:
        ...
        print(
            format_main_verify_status_message(
                ...
                fallback=f"main verify {status}{phase}; merges halted",
            )
        )
        return 1
...
def cmd_queue(args: argparse.Namespace) -> int:
    ...
    visible_runnable_entries = (
        runnable_preview_entries if display_limit is None else runnable_preview_entries[:display_limit]
    )
    ...
    visible_pending_task_ids = {entry.task.id for entry in visible_pending_entries if entry.task.id is not None}
    ...
    visible_runnable_pending_rows = [row for row in _runnable_rows if row.task.id in visible_pending_task_ids]
    visible_quiet_rows = list(quiet_rows)
    ...
    if quiet_rendered_rows:
        console.print()
        console.print(
            build_queue_summary("Quiet lane: `gza queue` shows held tasks without giving them runnable positions.")
        )
