    ...
    *,
    ...
    if action_type == "resume":
        ...
        if launch_mode == "iterate":
            ...
            result.success_message = f"Started iterate resume for {prepared_resume_task.id}"
            return result
    ...
    if action_type == "retry":
        ...
        if launch_mode == "iterate":
            ...
            if prepared_retry_task is None:
                assert prepare_error is not None
                return prepare_error
            rc = context.spawn_iterate_worker(prepared_retry_task, task.task_type or "task")
            worker_label = "iterate"
        else:
            prepared_retry_task, prepare_error = _prepare_background_start(
                context=context,
        ...
        result = _spawn_result(
            action_type=action_type,
            rc=rc,
            handled_task_id=prepared_retry_task.id,
            worker_label=worker_label,
            created_task=prepared_retry_task,
        )
        if reuse_existing:
            result.success_message = f"Reused pending retry task {prepared_retry_task.id}"
        else:
            result.success_message = f"Created retry task {prepared_retry_task.id}"
        return result
